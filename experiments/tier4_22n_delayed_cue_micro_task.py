#!/usr/bin/env python3
"""Tier 4.22n tiny delayed-cue custom-runtime micro-task gate.

Tier 4.22m proved that a tiny fixed-pattern task micro-loop could run on the custom runtime on real SpiNNaker. Tier 4.22n takes the next deliberately small step: a delayed-cue-like stream where each prediction waits behind intervening decisions before its target matures.
...
TIER = "Tier 4.22n - Tiny Delayed-Cue Custom-Runtime Micro-Task"
RUNNER_REVISION = "tier4_22n_delayed_cue_micro_task_20260501_0001"
TIER4_22M_LATEST = CONTROLLED / "tier4_22m_latest_manifest.json"
DEFAULT_OUTPUT = CONTROLLED / "tier4_22n_20260501_delayed_cue_micro_task_prepared"
UPLOAD_PACKAGE_NAME = "cra_422v"
STABLE_EBRAINS_UPLOAD = ROOT / "ebrains_jobs" / UPLOAD_PACKAGE_NAME
DEPRECATED_EBRAINS_UPLOADS = [ROOT / "ebrains_jobs" / "cra_422v_old"]

FP_SHIFT = 15
FP_ONE = 1 << FP_SHIFT
TASK_LEARNING_RATE = 0.125
TASK_TAIL_WINDOW = 4
PENDING_GAP_DEPTH = 2
TASK_SEQUENCE = [
    {
        ...
        "purpose": "delayed-cue signed feature/target event",
    }
    for idx in range(12)
]
...
def generate_task_reference(
    ...
    return {
        "status": "pass",
        "task": "delayed_cue_signed_micro_task",
        "fixed_point": "s16.15",
...
def write_reference_artifacts(output_dir: Path, reference: dict[str, Any]) -> dict[str, str]:
    reference_json = output_dir / "tier4_22n_task_reference.json"
    reference_csv = output_dir / "tier4_22n_task_reference_rows.csv"
...
def prepare_bundle(output_dir: Path) -> tuple[Path, str, dict[str, str]]:
    ...
    for script in [
        "tier4_22n_delayed_cue_micro_task.py",
    ...
    command = f"{UPLOAD_PACKAGE_NAME}/experiments/tier4_22n_delayed_cue_micro_task.py --mode run-hardware --output-dir tier4_22n_job_output"
    readme = bundle / "README_TIER4_22N_JOB.md"
    readme.write_text(
...
def write_report(path: Path, result: dict[str, Any]) -> None:
    ...
    for key in [
        "tier4_22m_status", "mode", "reference_status", "reference_sequence_length", "reference_accuracy", "reference_tail_accuracy",
...
def write_latest(output_dir: Path, manifest: Path, report: Path, status: str, mode: str) -> None:
    ...
    payload = {
        ...
        "report": str(report),
        "canonical": False,
        "claim": "Latest Tier 4.22n tiny delayed-cue custom-runtime micro-task; pass means a tiny delayed pending-queue on-chip loop matched local fixed-point reference and task metrics only.",
    }
    write_json(CONTROLLED / "tier4_22n_latest_manifest.json", payload)


def finalize(output_dir: Path, result: dict[str, Any]) -> int:
    manifest = output_dir / "tier4_22n_results.json"
    report = output_dir / "tier4_22n_report.md"
...
def _base_local_result(args: argparse.Namespace, output_dir: Path) -> tuple[str, str | None, dict[str, Any], dict[str, str], dict[str, Any], list[dict[str, Any]]]:
    tier4_22m_status, tier4_22m_manifest = latest_status(TIER4_22M_LATEST)
    ...
    return tier4_22m_status, tier4_22m_manifest, reference, reference_artifacts, main_syntax, source_checks


def local(args: argparse.Namespace, output_dir: Path) -> int:
    tier4_22m_status, tier4_22m_manifest, reference, reference_artifacts, main_syntax, source_checks = _base_local_result(args, output_dir)
    metrics = reference.get("metrics", {})
    criteria = [
        criterion("Tier 4.22m task micro-loop pass exists", tier4_22m_status, "== pass", tier4_22m_status == "pass"),
    ...
    result = {
        ...
        "summary": {
            "mode": "local",
            "tier4_22m_status": tier4_22m_status,
            "tier4_22m_manifest": tier4_22m_manifest,
            ...
            "next_step_if_passed": "Prepare cra_422v and run the Tier 4.22n tiny delayed-cue micro-task on EBRAINS.",
        },
        "criteria": criteria,
        "reference": reference,
...
def run_hardware(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    env_path = output_dir / "tier4_22n_environment.json"
    target_path = output_dir / "tier4_22n_target_acquisition.json"
    load_path = output_dir / "tier4_22n_load_result.json"
    task_path = output_dir / "tier4_22n_task_micro_loop_result.json"
    task_csv = output_dir / "tier4_22n_task_micro_loop_rows.csv"
    ...
    criteria = [
        ...
        criterion("Tier 4.22m task micro-loop pass exists or fresh bundle", tier4_22m_status, "== pass OR missing in fresh EBRAINS bundle", tier4_22m_status in {"pass", "missing"}),
        criterion("local task fixed-point reference generated", reference.get("status"), "== pass", reference.get("status") == "pass"),
    ...
    result = {
        ...
        "summary": {
            ...
            "claim_boundary": "Tiny on-chip delayed-cue-like micro-task only; not full CRA task learning or speedup evidence.",
...
def ingest(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    raw_manifest_path = source / "tier4_22n_results.json"
    if not raw_manifest_path.exists():
        raise SystemExit(f"missing tier4_22n_results.json in ingest dir: {source}")
    raw_manifest = read_json(raw_manifest_path)

    artifacts: dict[str, str] = {}
    artifacts["raw_remote_manifest_json"] = _copy_latest(source, output_dir, "tier4_22n_results.json", target_name="remote_tier4_22n_results_raw.json")
    artifacts["raw_remote_report_md"] = _copy_latest(source, output_dir, "tier4_22n_report.md", target_name="remote_tier4_22n_report_raw.md")
    exact_artifact_names = {
        "tier4_22n_environment.json": "environment_json",
        "tier4_22n_target_acquisition.json": "target_acquisition_json",
        "tier4_22n_load_result.json": "load_result_json",
        "tier4_22n_task_micro_loop_result.json": "task_micro_loop_result_json",
        "tier4_22n_task_micro_loop_rows.csv": "task_micro_loop_rows_csv",
        "tier4_22n_task_reference.json": "reference_json",
        "tier4_22n_task_reference_rows.csv": "reference_csv",
        "tier4_22n_latest_manifest.json": "remote_latest_manifest_json",
    }
    ...
    ingested["summary"].update(
        {
            ...
            "claim_boundary": "Ingested returned EBRAINS artifacts; a pass is a tiny signed delayed-cue-like custom-runtime micro-task only, not full CRA task learning or speedup evidence.",
        }
    )
...
def main() -> int:
    ...
    default = DEFAULT_OUTPUT if args.mode == "prepare" else CONTROLLED / f"tier4_22n_{stamp}_{args.mode.replace('-', '_')}"
