...
resonant-heavy specialist allocation:

* 4 standard bounded recurrent/LIF-style excitatory units + 12 resonant branches

The point is to ask whether a resonant-heavy specialist subpool can preserve
...
TIER = "Tier 5.20d - Resonant-Heavy Hybrid LIF Polyp Diagnostic"
RUNNER_REVISION = "tier5_20d_resonant_heavy_polyp_diagnostic_20260508_0001"
DEFAULT_OUTPUT_DIR = OUTPUT_ROOT / "tier5_20d_20260508_resonant_heavy_polyp_diagnostic"

HYBRID_4_12 = "hybrid_4_lif_12_resonant"
HYBRID_4_12_FLAT = "hybrid_4_lif_12_flat_tau_sham"
HYBRID_4_12_RATE = "hybrid_4_lif_12_rate_only_sham"
HYBRID_4_12_SHUFFLED = "hybrid_4_lif_12_shuffled_branch_sham"
REQUIRED_MODELS = [
    V23,
    V22,
    FULL_RESONANT,
...
def run_task_models(task: Any, *, seed: int, args: argparse.Namespace, capture_timeseries: bool) -> tuple[list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
    ...
    model_bundles = {
        ...
        HYBRID_4_12: hybrid_features(task.observed, seed=seed, train_end=task.train_end, lif_units=4, branch_count=12, branch_mode="resonant", args=args),
        HYBRID_4_12_FLAT: hybrid_features(task.observed, seed=seed, train_end=task.train_end, lif_units=4, branch_count=12, branch_mode="flat_tau", args=args),
        HYBRID_4_12_RATE: hybrid_features(task.observed, seed=seed, train_end=task.train_end, lif_units=4, branch_count=12, branch_mode="rate_only", args=args),
        HYBRID_4_12_SHUFFLED: hybrid_features(task.observed, seed=seed, train_end=task.train_end, lif_units=4, branch_count=12, branch_mode="shuffled", args=args),
    }
    ...
    diagnostics = {
        "task": task.name,
        "seed": int(seed),
        "same_budget_variant": "4_lif_12_resonant",
        "v23_feature_count": int(model_bundles[V23].features.shape[1]),
        "hybrid_4_12_feature_count": int(model_bundles[HYBRID_4_12].features.shape[1]),
    }
...
def classify(summary_rows: list[dict[str, Any]], aggregate_summary: list[dict[str, Any]], tasks: list[str]) -> dict[str, Any]:
    candidate = evaluate_candidate(
        candidate=HYBRID_4_12,
        shams=[HYBRID_4_12_FLAT, HYBRID_4_12_RATE, HYBRID_4_12_SHUFFLED],
        tasks=tasks,
        summary_rows=summary_rows,
        aggregate_summary=aggregate_summary,
    )
    if candidate["useful_signal"] and candidate["no_material_regression_vs_v2_3"]:
        outcome = "resonant_heavy_candidate_for_integration_gate"
        recommendation = "Proceed to optional-polyp integration gate for 4 LIF / 12 resonant branches."
    elif candidate["no_material_regression_vs_v2_3"] and candidate["sham_separation_count"] >= 1:
        outcome = "resonant_heavy_research_scaffold_not_promoted"
        recommendation = "Keep 4/12 as a documented scaffold; do not integrate until stronger task value appears."
    else:
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    criteria = [
        ...
        criterion("same budget 4/12 variant declared", "4 LIF + 12 resonant", "sum == 16 excitatory units", True),
        ...
        criterion("4/12 shams present", [HYBRID_4_12_FLAT, HYBRID_4_12_RATE, HYBRID_4_12_SHUFFLED], "all present", all(model in models for model in [HYBRID_4_12_FLAT, HYBRID_4_12_RATE, HYBRID_4_12_SHUFFLED])),
    ...
    resonant_heavy_contract = {
        "mechanism": "resonant_heavy_hybrid_lif_branches",
        "tested_layout": {"standard_lif_excitatory_units": 4, "resonant_branch_units": 12, "total_excitatory_budget": 16},
        "repair_target": "test whether the resonant-heavy specialist dose preserves v2.3 while retaining any variable-delay/anomaly value",
        "nonclaims": [
    ...
    payload = {
        ...
        "resonant_heavy_contract": resonant_heavy_contract,
        "claim_boundary": (
            "Tier 5.20d is a software-only resonant-heavy diagnostic after "
            "5.20a/5.20b. It tests a same-budget 4 LIF / 12 resonant branch "
            "internal-polyp proxy against v2.3, v2.2, full-resonant, "
            "lag/reservoir/ESN controls, and 4/12 shams. It is not a canonical "
            "organism change, not hardware evidence, and not a baseline freeze."
        ),
    }
    write_json(output_dir / "tier5_20d_results.json", payload)
    write_json(output_dir / "tier5_20d_resonant_heavy_contract.json", resonant_heavy_contract)
    write_csv_rows(output_dir / "tier5_20d_summary.csv", summary_rows)
    write_csv_rows(output_dir / "tier5_20d_aggregate.csv", aggregate_rows)
    write_csv_rows(output_dir / "tier5_20d_aggregate_summary.csv", aggregate_summary)
    if capture_timeseries:
        write_csv_rows(output_dir / "tier5_20d_timeseries.csv", all_timeseries)
    write_report(output_dir / "tier5_20d_report.md", payload)
    latest = {
        "tier": TIER,
        "status": payload["status"],
        "generated_at_utc": payload["generated_at_utc"],
        "output_dir": str(output_dir),
        "results_json": str(output_dir / "tier5_20d_results.json"),
        "report_md": str(output_dir / "tier5_20d_report.md"),
        "summary_csv": str(output_dir / "tier5_20d_summary.csv"),
        "classification": classification["outcome"],
    }
    write_json(OUTPUT_ROOT / "tier5_20d_latest_manifest.json", latest)
    return payload
