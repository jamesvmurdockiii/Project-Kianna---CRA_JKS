#!/usr/bin/env python3
"""Tier 4.22m minimal custom-runtime task micro-loop gate.

Tier 4.22l proved that a tiny sequence of chip-owned pending-horizon/readout
updates matched a local s16.15 fixed-point reference on real SpiNNaker. Tier
4.22m takes the next deliberately small step: a deterministic task-like stream
where the custom runtime emits a pre-update prediction, receives delayed credit,
and improves tail sign accuracy across repeated events.
...
TIER = "Tier 4.22m - Minimal Custom-Runtime Task Micro-Loop"
RUNNER_REVISION = "tier4_22m_custom_runtime_task_micro_loop_20260501_0001"
TIER4_22L_LATEST = CONTROLLED / "tier4_22l_latest_manifest.json"
DEFAULT_OUTPUT = CONTROLLED / "tier4_22m_20260501_custom_runtime_task_micro_loop_prepared"
UPLOAD_PACKAGE_NAME = "cra_422u"
STABLE_EBRAINS_UPLOAD = ROOT / "ebrains_jobs" / UPLOAD_PACKAGE_NAME
DEPRECATED_EBRAINS_UPLOADS = [ROOT / "ebrains_jobs" / "cra_422u_old"]
...
TASK_SEQUENCE = [
    {
        ...
        "purpose": "fixed-pattern signed feature/target event",
    }
    for idx in range(12)
]
...
def generate_task_reference(
    ...
    return {
        "status": "pass",
        "task": "fixed_pattern_signed_micro_loop",
        "fixed_point": "s16.15",
        "equation": "prediction=w*feature+bias; score sign(prediction) before update; error=target-prediction; w+=lr*error*feature; b+=lr*error",
        "learning_rate": float(learning_rate),
        "learning_rate_raw": lr_raw,
...
def write_reference_artifacts(output_dir: Path, reference: dict[str, Any]) -> dict[str, str]:
    reference_json = output_dir / "tier4_22m_task_reference.json"
    reference_csv = output_dir / "tier4_22m_task_reference_rows.csv"
...
def prepare_bundle(output_dir: Path) -> tuple[Path, str, dict[str, str]]:
    ...
    for script in [
        "tier4_22m_custom_runtime_task_micro_loop.py",
    ...
    command = f"{UPLOAD_PACKAGE_NAME}/experiments/tier4_22m_custom_runtime_task_micro_loop.py --mode run-hardware --output-dir tier4_22m_job_output"
    readme = bundle / "README_TIER4_22M_JOB.md"
    readme.write_text(
...
def write_report(path: Path, result: dict[str, Any]) -> None:
    ...
    for key in [
        "tier4_22l_status", "mode", "reference_status", "reference_sequence_length", "reference_accuracy", "reference_tail_accuracy",
        "reference_final_weight", "reference_final_bias", "hardware_target_configured", "spinnaker_hostname", "selected_dest_cpu",
...
def write_latest(output_dir: Path, manifest: Path, report: Path, status: str, mode: str) -> None:
    ...
    payload = {
        ...
        "report": str(report),
        "canonical": False,
        "claim": "Latest Tier 4.22m minimal custom-runtime task micro-loop; pass means a tiny task-like on-chip loop matched local fixed-point reference and task metrics only.",
    }
    write_json(CONTROLLED / "tier4_22m_latest_manifest.json", payload)


def finalize(output_dir: Path, result: dict[str, Any]) -> int:
    manifest = output_dir / "tier4_22m_results.json"
    report = output_dir / "tier4_22m_report.md"
...
def _base_local_result(args: argparse.Namespace, output_dir: Path) -> tuple[str, str | None, dict[str, Any], dict[str, str], dict[str, Any], list[dict[str, Any]]]:
    tier4_22l_status, tier4_22l_manifest = latest_status(TIER4_22L_LATEST)
    reference = generate_task_reference(learning_rate=float(args.learning_rate))
    ...
    return tier4_22l_status, tier4_22l_manifest, reference, reference_artifacts, main_syntax, source_checks


def local(args: argparse.Namespace, output_dir: Path) -> int:
    tier4_22l_status, tier4_22l_manifest, reference, reference_artifacts, main_syntax, source_checks = _base_local_result(args, output_dir)
    metrics = reference.get("metrics", {})
    criteria = [
        criterion("Tier 4.22l tiny learning parity pass exists", tier4_22l_status, "== pass", tier4_22l_status == "pass"),
        ...
        criterion("reference sequence length", reference.get("sequence_length"), "== 12", reference.get("sequence_length") == 12),
        ...
        criterion("reference final weight positive", reference.get("final_readout_weight_raw"), "> 0", int(reference.get("final_readout_weight_raw") or 0) > 0),
        criterion("reference final bias neutral", reference.get("final_readout_bias_raw"), "== 0", int(reference.get("final_readout_bias_raw") if reference.get("final_readout_bias_raw") is not None else -1) == 0),
    ] + source_checks
    ...
    result = {
        ...
        "summary": {
            "mode": "local",
            "tier4_22l_status": tier4_22l_status,
            "tier4_22l_manifest": tier4_22l_manifest,
...
def run_hardware(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    env_path = output_dir / "tier4_22m_environment.json"
    target_path = output_dir / "tier4_22m_target_acquisition.json"
    load_path = output_dir / "tier4_22m_load_result.json"
    task_path = output_dir / "tier4_22m_task_micro_loop_result.json"
    task_csv = output_dir / "tier4_22m_task_micro_loop_rows.csv"
    ...
    criteria = [
        ...
        criterion("Tier 4.22l tiny parity pass exists or fresh bundle", tier4_22l_status, "== pass OR missing in fresh EBRAINS bundle", tier4_22l_status in {"pass", "missing"}),
        criterion("local task fixed-point reference generated", reference.get("status"), "== pass", reference.get("status") == "pass"),
    ...
    result = {
        ...
        "summary": {
            ...
            "claim_boundary": "Minimal on-chip fixed-pattern task micro-loop only; not full CRA task learning or speedup evidence.",
...
def ingest(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    raw_manifest_path = source / "tier4_22m_results.json"
    if not raw_manifest_path.exists():
        raise SystemExit(f"missing tier4_22m_results.json in ingest dir: {source}")
    raw_manifest = read_json(raw_manifest_path)

    artifacts: dict[str, str] = {}
    artifacts["raw_remote_manifest_json"] = _copy_latest(source, output_dir, "tier4_22m_results.json", target_name="remote_tier4_22m_results_raw.json")
    artifacts["raw_remote_report_md"] = _copy_latest(source, output_dir, "tier4_22m_report.md", target_name="remote_tier4_22m_report_raw.md")
    exact_artifact_names = {
        "tier4_22m_environment.json": "environment_json",
        "tier4_22m_target_acquisition.json": "target_acquisition_json",
        "tier4_22m_load_result.json": "load_result_json",
        "tier4_22m_task_micro_loop_result.json": "task_micro_loop_result_json",
        "tier4_22m_task_micro_loop_rows.csv": "task_micro_loop_rows_csv",
        "tier4_22m_task_reference.json": "reference_json",
        "tier4_22m_task_reference_rows.csv": "reference_csv",
        "tier4_22m_latest_manifest.json": "remote_latest_manifest_json",
    }
    ...
    ingested["summary"].update(
        {
            ...
            "claim_boundary": "Ingested returned EBRAINS artifacts; a pass is a minimal signed fixed-pattern custom-runtime task micro-loop only, not full CRA task learning or speedup evidence.",
        }
    )
...
def main() -> int:
    ...
    default = DEFAULT_OUTPUT if args.mode == "prepare" else CONTROLLED / f"tier4_22m_{stamp}_{args.mode.replace('-', '_')}"
