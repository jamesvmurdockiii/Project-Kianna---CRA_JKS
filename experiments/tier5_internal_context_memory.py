...
external host-side scaffold could solve those tasks. Tier 5.10d now asks the
next question: can CRA's internal host-side context-memory path solve the same
tasks when fed raw observations, and do reset/shuffled/wrong-memory ablations
remove the benefit?

This is a software mechanism diagnostic. A pass here authorizes treating the
internal context-memory path as a candidate mechanism subject to compact
regression. It is not hardware evidence, not sleep/replay evidence, and not
native on-chip memory evidence.
"""
...
TIER = "Tier 5.10d - Internal Context Memory Implementation Diagnostic"
...
def parse_variants(raw: str) -> list[MemoryVariant]:
    ...
    if missing:
        raise argparse.ArgumentTypeError(f"unknown Tier 5.10d variants: {', '.join(missing)}")
    ...
    if not required.issubset(present):
        raise argparse.ArgumentTypeError(
            "Tier 5.10d requires v1_4_raw, external_context_memory_scaffold, and internal_context_memory"
        )
    return selected
...
def evaluate_tier(
    *,
    ...
    summary = {
        ...
        "claim_boundary": "Software diagnostic only: internal host-side context binding, not native on-chip memory, sleep/replay, or hardware evidence.",
    }
...
def plot_memory_edges(comparisons: list[dict[str, Any]], path: Path) -> None:
    ...
    ax.set_title("Tier 5.10d Internal Context Memory Diagnostic")
    ax.set_xticks(x)
...
def fairness_contract(args: argparse.Namespace, variants: list[MemoryVariant], models: list[str]) -> dict[str, Any]:
    return {
        ...
        "fairness_rules": [
            ...
            "Tier 5.10d is internal host-side context memory, not native on-chip memory",
        ],
        "tasks": args.tasks,
        "steps": args.steps,
        "seeds": seeds_from_args(args),
...
def write_latest(output_dir: Path, report_path: Path, manifest_path: Path, summary_csv: Path, status: str) -> None:
    latest_path = ROOT / "controlled_test_output" / "tier5_10d_latest_manifest.json"
    payload = {
        ...
        "canonical": False,
        "claim": "Latest Tier 5.10d internal context-memory diagnostic; passing is not hardware or native on-chip memory evidence.",
    }
...
def run_tier(args: argparse.Namespace, output_dir: Path, variants: list[MemoryVariant]) -> dict[str, Any]:
    ...
    for seed in seeds_from_args(args):
        ...
        for task in tasks:
            ...
            for variant in variants:
                print(f"[tier5.10d] task={task.stream.name} variant={variant.name} seed={seed}", flush=True)
            ...
            for model in models:
                print(f"[tier5.10d] task={task.stream.name} model={model} seed={seed}", flush=True)
            ...
            for control in CONTROL_MODELS:
                print(f"[tier5.10d] task={task.stream.name} control={control} seed={seed}", flush=True)
    ...
    summary_csv = output_dir / "tier5_10d_summary.csv"
    comparisons_csv = output_dir / "tier5_10d_comparisons.csv"
    fairness_json = output_dir / "tier5_10d_fairness_contract.json"
    plot_path = output_dir / "tier5_10d_memory_edges.png"
    ...
    return {
        "name": "internal_context_memory_mechanism",
        "status": status,
        "summary": {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.description = "Run Tier 5.10d internal context-memory implementation diagnostic."
    parser.set_defaults(
        backend="nest",
        tasks=DEFAULT_TASKS,
        steps=720,
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_dir = args.output_dir or (ROOT / "controlled_test_output" / f"tier5_10d_{timestamp}")
    output_dir.mkdir(parents=True, exist_ok=True)
    result = run_tier(args, output_dir, variants)
    manifest_path = output_dir / "tier5_10d_results.json"
    report_path = output_dir / "tier5_10d_report.md"
    summary_csv = output_dir / "tier5_10d_summary.csv"
    comparisons_csv = output_dir / "tier5_10d_comparisons.csv"
    fairness_json = output_dir / "tier5_10d_fairness_contract.json"
    manifest = {
        ...
        "artifacts": {
            ...
            "memory_edges_png": str(output_dir / "tier5_10d_memory_edges.png"),
        },
    }
