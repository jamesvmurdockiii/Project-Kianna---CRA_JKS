...
Tier 5.10e showed that the internal host-side context-memory path survives
longer gaps, denser distractors, and hidden recurrence pressure. Tier 5.10f
asks a harder question: does the same mechanism survive when the stream contains
intervening contexts, overlapping pending decisions, and explicit interference
that can overwrite a single-slot memory?

This is a software stress diagnostic for the v1.5 memory-retention baseline. A
pass would strengthen the internal memory claim under capacity/interference
pressure. A fail would not falsify memory; it would identify the measured
blocker that justifies multi-slot memory, consolidation, or sleep/replay work.
It is not hardware evidence and not native on-chip memory evidence.
"""
...
TIER = "Tier 5.10f - Memory Capacity / Interference Stressor"
...
def build_tasks(args: argparse.Namespace, seed: int) -> list[MemoryTask]:
    ...
    if missing:
        raise argparse.ArgumentTypeError(f"unknown Tier 5.10f tasks: {', '.join(missing)}")
...
def parse_variants(raw: str) -> list[MemoryVariant]:
    ...
    if missing:
        raise argparse.ArgumentTypeError(f"unknown Tier 5.10f variants: {', '.join(missing)}")
    ...
    if not required.issubset(present):
        raise argparse.ArgumentTypeError(
            "Tier 5.10f requires v1_4_raw, external_context_memory_scaffold, and internal_context_memory"
        )
    return selected
...
def plot_memory_edges(comparisons: list[dict[str, Any]], path: Path) -> None:
    ...
    ax.set_title("Tier 5.10f Memory Capacity / Interference Stressor")
    ax.set_xticks(x)
...
def fairness_contract(args: argparse.Namespace, variants: list[MemoryVariant], models: list[str]) -> dict[str, Any]:
    return {
        ...
        "fairness_rules": [
            ...
            "Tier 5.10f is internal host-side context memory under capacity/interference stress, not native on-chip memory or sleep/replay",
        ],
        "tasks": args.tasks,
        "steps": args.steps,
        "seeds": seeds_from_args(args),
...
def write_latest(output_dir: Path, report_path: Path, manifest_path: Path, summary_csv: Path, status: str) -> None:
    latest_path = ROOT / "controlled_test_output" / "tier5_10f_latest_manifest.json"
    payload = {
        ...
        "canonical": False,
        "claim": "Latest Tier 5.10f memory capacity/interference stressor; passing is not hardware or native on-chip memory evidence.",
    }
...
def run_tier(args: argparse.Namespace, output_dir: Path, variants: list[MemoryVariant]) -> dict[str, Any]:
    ...
    for seed in seeds_from_args(args):
        ...
        for task in tasks:
            ...
            for variant in variants:
                print(f"[tier5.10f] task={task.stream.name} variant={variant.name} seed={seed}", flush=True)
            ...
            for model in models:
                print(f"[tier5.10f] task={task.stream.name} model={model} seed={seed}", flush=True)
            ...
            for control in CONTROL_MODELS:
                print(f"[tier5.10f] task={task.stream.name} control={control} seed={seed}", flush=True)
    ...
    summary_csv = output_dir / "tier5_10f_summary.csv"
    comparisons_csv = output_dir / "tier5_10f_comparisons.csv"
    fairness_json = output_dir / "tier5_10f_fairness_contract.json"
    plot_path = output_dir / "tier5_10f_memory_edges.png"
    ...
    return {
        "name": "memory_capacity_interference_stressor",
        "status": status,
        "summary": {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.description = "Run Tier 5.10f memory capacity/interference stressor."
    parser.set_defaults(
        backend="nest",
        tasks=DEFAULT_TASKS,
        steps=720,
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_dir = args.output_dir or (ROOT / "controlled_test_output" / f"tier5_10f_{timestamp}")
    output_dir.mkdir(parents=True, exist_ok=True)
    result = run_tier(args, output_dir, variants)
    manifest_path = output_dir / "tier5_10f_results.json"
    report_path = output_dir / "tier5_10f_report.md"
    summary_csv = output_dir / "tier5_10f_summary.csv"
    comparisons_csv = output_dir / "tier5_10f_comparisons.csv"
    fairness_json = output_dir / "tier5_10f_fairness_contract.json"
    manifest = {
        ...
        "artifacts": {
            ...
            "memory_edges_png": str(output_dir / "tier5_10f_memory_edges.png"),
        },
    }
