...
Tier 5.11b showed strong replay repair signal but failed promotion because a
shuffled replay sham partially matched prioritized replay on partial-key
reentry. Tier 5.11c gives replay one focused repair cycle: the opportunity,
timing, and write counts remain matched, but the shams become sharper
wrong-key, key-label-permuted, shuffled-order, random, priority-only, and
no-consolidation controls.
...
TIER = "Tier 5.11c - Replay Sham-Separation Repair"
...
def parse_variants(raw: str) -> list[ReplayVariant]:
    ...
    if unknown:
        raise SystemExit(f"Unknown Tier 5.11c variants: {unknown}; available={sorted(available)}")
    required = {
        "v1_6_no_replay",
        "prioritized_replay",
    ...
    if missing:
        raise SystemExit(f"Tier 5.11c requires {sorted(required)}; missing={sorted(missing)}")
    ...
    *,
    ...
    for rank, item in enumerate(selected):
        replay = organism.replay_context_memory_episode(
            ...
            source=f"tier5_11c_{variant.replay_mode}",
        )
        events.append(
            {
    ...
    if not args.smoke:
        criteria.extend(
            [
                ...
                criterion("shuffled-order replay does not match prioritized tail", min(edge_shuffled_order) if edge_shuffled_order else None, ">=", args.min_control_tail_edge, bool(edge_shuffled_order) and min(edge_shuffled_order) >= float(args.min_control_tail_edge)),
                ...
                criterion("wrong-key replay does not match prioritized tail", min(edge_wrong_key) if edge_wrong_key else None, ">=", args.min_control_tail_edge, bool(edge_wrong_key) and min(edge_wrong_key) >= float(args.min_control_tail_edge)),
                criterion("key-label-permuted replay does not match prioritized tail", min(edge_key_label) if edge_key_label else None, ">=", args.min_control_tail_edge, bool(edge_key_label) and min(edge_key_label) >= float(args.min_control_tail_edge)),
                criterion("priority-only ablation does not match prioritized tail", min(edge_priority_only) if edge_priority_only else None, ">=", args.min_control_tail_edge, bool(edge_priority_only) and min(edge_priority_only) >= float(args.min_control_tail_edge)),
                ...
                criterion("sham replay write counts match prioritized", min(matched_write_controls.values()) if matched_write_controls else None, "==", replay_writes, bool(matched_write_controls) and all(abs(value - replay_writes) <= EPS for value in matched_write_controls.values())),
            ]
        )
    summary = {
...
def plot_replay_edges(comparisons: list[dict[str, Any]], path: Path) -> None:
    ...
    ax.set_title("Tier 5.11c Replay Sham-Separation Repair")
    ax.set_xticks(x)
...
def fairness_contract(args: argparse.Namespace, variants: list[ReplayVariant], models: list[str]) -> dict[str, Any]:
    return {
        ...
        "fairness_rules": [
            ...
            "shuffled-order/random/wrong-key/key-label-permuted/priority-only/no-consolidation controls match replay opportunity but remove prioritized binding semantics",
...
def write_report(path: Path, result: dict[str, Any], aggregates: list[dict[str, Any]], comparisons: list[dict[str, Any]], args: argparse.Namespace, output_dir: Path) -> None:
    ...
    lines = [
        ...
        f"- prioritized min tail edge vs shuffled-order: `{tier_summary.get('prioritized_min_tail_edge_vs_shuffled_order')}`",
        f"- prioritized min tail edge vs random: `{tier_summary.get('prioritized_min_tail_edge_vs_random')}`",
        f"- prioritized min tail edge vs wrong-key: `{tier_summary.get('prioritized_min_tail_edge_vs_wrong_key')}`",
        f"- prioritized min tail edge vs key-label-permuted: `{tier_summary.get('prioritized_min_tail_edge_vs_key_label_permuted')}`",
        f"- prioritized min tail edge vs priority-only: `{tier_summary.get('prioritized_min_tail_edge_vs_priority_only')}`",
...
def write_latest(output_dir: Path, report_path: Path, manifest_path: Path, summary_csv: Path, status: str) -> None:
    latest_path = ROOT / "controlled_test_output" / "tier5_11c_latest_manifest.json"
    payload = {
        ...
        "canonical": False,
        "claim": "Latest Tier 5.11c replay sham-separation repair; not hardware replay or native on-chip memory.",
    }
...
def run_tier(args: argparse.Namespace, output_dir: Path, variants: list[ReplayVariant]) -> dict[str, Any]:
    ...
    for seed in seeds_from_args(args):
        for task in build_tasks(args, seed):
            ...
            for variant in variants:
                print(f"[tier5.11c] task={task.stream.name} variant={variant.name} seed={seed}", flush=True)
            ...
            for model in models:
                print(f"[tier5.11c] task={task.stream.name} model={model} seed={seed}", flush=True)
            ...
            for control in CONTROL_MODELS:
                print(f"[tier5.11c] task={task.stream.name} control={control} seed={seed}", flush=True)
    ...
    summary_csv = output_dir / "tier5_11c_summary.csv"
    comparisons_csv = output_dir / "tier5_11c_comparisons.csv"
    replay_events_csv = output_dir / "tier5_11c_replay_events.csv"
    fairness_json = output_dir / "tier5_11c_fairness_contract.json"
    plot_path = output_dir / "tier5_11c_replay_edges.png"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.description = "Run Tier 5.11c replay sham-separation repair."
    parser.set_defaults(
        backend="nest",
        tasks=DEFAULT_TASKS,
        steps=960,
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_dir = args.output_dir or (ROOT / "controlled_test_output" / f"tier5_11c_{timestamp}")
    output_dir.mkdir(parents=True, exist_ok=True)
    result = run_tier(args, output_dir, variants)
    manifest_path = output_dir / "tier5_11c_results.json"
    report_path = output_dir / "tier5_11c_report.md"
    summary_csv = output_dir / "tier5_11c_summary.csv"
    comparisons_csv = output_dir / "tier5_11c_comparisons.csv"
    replay_events_csv = output_dir / "tier5_11c_replay_events.csv"
    fairness_json = output_dir / "tier5_11c_fairness_contract.json"
    manifest = {
        ...
        "artifacts": {
            ...
            "replay_edges_png": str(output_dir / "tier5_11c_replay_edges.png"),
        },
    }
