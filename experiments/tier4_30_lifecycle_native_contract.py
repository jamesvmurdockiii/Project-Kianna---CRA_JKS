...
def gate_specs() -> list[GateSpec]:
    return [
        ...
        GateSpec(
            ...
            scope="lifecycle ownership, message semantics, active-mask sync, and failure classes across learning/context/route/memory/lifecycle cores",
            pass_criteria="single-writer lifecycle ownership, explicit MCPL/multicast-target messages, final active-mask sync, and exact canonical/boundary parity",
            fail_criteria="ambiguous ownership, missing failure classes, mask desync, corrupted lineage, or unmatched event count",
            artifacts="core map, message contract, failure classes, split trace, local/reference diff",
        ),
        GateSpec(
            gate="Tier 4.30d multi-core lifecycle runtime source audit/local C host test",
            mode="local source/runtime",
            scope="lifecycle-core runtime profile plus inter-core lifecycle message/readback stubs",
            pass_criteria="source guards and local C host tests prove the 4.30c contract before EBRAINS packaging",
            fail_criteria="missing lifecycle-core profile, stale/duplicate event counters absent, readback schema drift, or source/runtime parity failure",
            artifacts="source-audit JSON, host-test output, message/readback stubs, failure-class guard report",
        ),
        GateSpec(
            gate="Tier 4.30f lifecycle sham-control subset",
