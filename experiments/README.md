...
Current source of truth: the generated evidence registry contains `157`
canonical evidence bundles. The active software gate is Tier 5.45a
healthy-NEST rebaseline scoring; current progress is tracked in
`docs/TIER5_45A_SHARD_EXECUTION_PLAN.md`. The current predictive benchmark
baseline remains `CRA_EVIDENCE_BASELINE_v2.6`; `v2.7` is a diagnostic
healthy-NEST organism-development snapshot and does not supersede `v2.6` for
predictive usefulness.

The paragraphs below preserve the historical tier progression for experiment
orientation. When a count or active-status statement conflicts with the
registry, `controlled_test_output/STUDY_REGISTRY.json`, `README.md`, and
`docs/MASTER_EXECUTION_PLAN.md` are authoritative.
...
the tracked evidence suite is generated by the registry and currently contains
`157` canonical bundles. The generated registry is the authority for the exact
current list:
...
Tier 4.29f is complete: the compact native mechanism evidence-regression gate passed over 4.29a-e and froze `CRA_NATIVE_MECHANISM_BRIDGE_v0.3`. Tier 7.0 standard dynamical benchmarks are complete as a diagnostic: CRA v2.1 ranked 5/5 on the tested continuous-valued sequence baselines. Tier 7.0b localized the gap to a recoverable state-signal/default-readout failure. Tier 7.0c bounded continuous readout/interface repair passed its integrity criteria, improved raw CRA, and beat shuffled/frozen controls, but lag-only online LMS still explained most of the benchmark gain. Tier 7.0d then confirmed that lag regression explains this benchmark path under the prior interface, so no continuous-readout mechanism was promoted. Tier 5.19 / 7.0e then defined the temporal-substrate contract. Tier 5.19a passed as a local software reference, Tier 5.19b passed as a stricter sham gate with classification `fading_memory_supported_recurrence_unproven`, and Tier 5.19c passed the narrowed fading-memory compact-regression gate with full NEST compact regression. Tier 7.0j froze `v2.3` as a generic host-side bounded recurrent continuous-state baseline; topology-specific recurrence, ESN superiority, hardware/native recurrence, language, planning, AGI, and ASI remain unproven. Tier 7.4c now freezes `v2.4` as the current host-side software cost-aware policy/action baseline after the locked 7.4b expected-utility candidate passed full NEST compact regression. Tier 4.30-readiness passed as a local engineering audit (`16/16`), selecting the initial lifecycle-native path: static pools, masks, lineage, and trophic counters layered on `CRA_NATIVE_MECHANISM_BRIDGE_v0.3`, with v2.2 used as software reference only. Tier 4.30 then passed as the lifecycle-native static-pool contract (`14/14`), defining command schemas, readback schemas, event semantics, gate sequence, and failure classes. Tier 4.30a passed as the local deterministic lifecycle reference (`20/20`) with canonical 32-event and boundary 64-event traces plus sham outputs. Tier 4.30b passed source/runtime audit (`13/13`), and Tier 4.30b-hw passed after ingest correction: the raw remote runner failed only because rev-0001 checked cumulative `readback_bytes`; returned artifacts show compact `payload_len=68` and exact lifecycle state parity. Tier 4.30c then passed as the multi-core lifecycle state split contract/local reference (22/22), defining the five-core ownership model, MCPL/multicast-target lifecycle messages, active-mask sync, and distributed failure classes. Tier 4.30d passed as the local runtime source audit (14/14), adding the dedicated lifecycle_core profile, lifecycle inter-core stubs/counters, active-mask/count/lineage sync bookkeeping, and local C host tests against the split contract. Tier 4.30e then passed as the multi-core lifecycle hardware smoke after EBRAINS ingest: board `10.11.226.145`, raw remote status `pass`, ingest status `pass`, 75/75 hardware criteria, 5/5 ingest criteria, five profile builds/loads/readbacks, canonical/boundary lifecycle parity, and duplicate/stale lifecycle event rejection. Tier 4.30f then passed as the compact lifecycle sham-control hardware subset after EBRAINS ingest: board `10.11.227.9`, raw remote status `pass`, ingest status `pass`, 185/185 hardware criteria, 5/5 ingest criteria, 35 returned artifacts preserved, enabled canonical behavior preserved, and fixed-pool/random-replay/active-mask-shuffle/no-trophic/no-dopamine controls separated on their predeclared fields. Tier 4.30g then passed as a local task-benefit/resource bridge contract (`9/9`): enabled lifecycle opened the bounded task gate, all five controls closed it, and resource/readback fields were declared. Tier 4.30g-hw then passed on real SpiNNaker after EBRAINS ingest: board `10.11.242.97`, raw remote status `pass`, ingest status `pass`, `285/285` hardware criteria, `5/5` ingest criteria, `36` returned artifacts preserved, enabled lifecycle opened the bounded task gate, all five predeclared controls closed it, and resource/readback accounting returned. `CRA_LIFECYCLE_NATIVE_BASELINE_v0.4` is now frozen with a host-ferried lifecycle bridge boundary. Tier 4.31a native temporal-substrate readiness passed locally (`24/24`), predeclaring seven causal fixed-point EMA traces as the first native v2.2 temporal subset with derived deltas/novelty and no hidden recurrent state. Tier 4.31b then passed locally (`16/16`), proving fixed-point parity against the v2.2 fading-memory reference with zero selected saturations and separated lag-only, zero-state, frozen-state, shuffled-state, reset-interval, shuffled-target, and no-plasticity controls. Tier 4.31c then passed locally (`17/17`), adding C-owned seven-EMA temporal state, compact 48-byte readback, command codes `39-42`, behavior-backed shams, profile ownership guards, and local C host tests. Tier 4.31d-hw passed and was ingested from EBRAINS with `59/59` remote hardware criteria, compact temporal payload length `48`, and enabled/zero/frozen/reset controls all passing. Tier 4.31e passed the native replay/eligibility decision closeout (`15/15`), deferring native replay buffers, sleep-like replay, and native macro eligibility until measured blockers exist. Tier 4.32 then passed the native-runtime mapping/resource model (`23/23`), selecting MCPL-first scale stress, documenting positive returned profile ITCM/DTCM headroom, authorizing Tier 4.32a single-chip multi-core scale stress next, and withholding native-scale baseline freeze. Tier 4.32a then passed the single-chip scale-stress preflight (`19/19`), authorizing only the 4/5-core single-shard hardware stress points and blocking replicated 8/12/16-core stress until shard-aware MCPL routing exists. Tier 4.32a-r0 then passed the protocol truth audit (`10/10`), blocking the planned MCPL-first hardware package until confidence-bearing and shard-aware MCPL lookup is repaired. Tier 4.32a-r1 passed the local MCPL lookup repair (`14/14`): value/meta reply packets preserve confidence/hit/status, keys carry shard identity, cross-shard controls pass, and full/zero/half-confidence learning controls pass over MCPL. Tier 4.32a-hw then passed after EBRAINS ingest. Tier 4.32a-hw-replicated then passed after EBRAINS ingest: board `10.11.215.121`, raw status `pass`, ingest status `pass`, `185/185` raw hardware criteria, `9/9` ingest criteria, `80` returned artifacts, and all point08/point12/point16 replicated-shard stress points passed with zero stale replies, duplicate replies, timeouts, or synthetic fallback. Tier 4.32b static reef partition smoke/resource mapping then passed locally (`25/25`), defining `quad_mechanism_partition_v0`, rejecting one-polyp-one-chip, and authorizing Tier 4.32c. Tier 4.32c inter-chip feasibility contract then passed locally (`19/19`), defining board/chip/core/role/partition/shard/seq identity, remote split-role MCPL lookup paths, compact readback ownership, failure classes, and the exact two-chip split-role single-shard smoke target; true two-partition cross-chip learning remains blocked until origin/target shard semantics are defined. Tier 4.32d-r0 route/source/package audit then passed locally, blocking the first 4.32d upload until inter-chip route repair; Tier 4.32d-r1 then passed as local route repair/QA, authorizing the Tier 4.32d communication/readback hardware smoke. Tier 4.32d then passed on EBRAINS and was ingested, and Tier 4.32e then passed the first two-chip learning-bearing micro-task after EBRAINS ingest: board `10.11.205.161`, two cases, 32 events per case, 96/96 lookup replies per case, zero stale replies, zero duplicate replies, zero timeouts, zero synthetic fallback, enabled LR 0.25 readout `32768/0`, and no-learning LR 0.0 readout `0/0`. Tier 4.32f then passed locally at `controlled_test_output/tier4_32f_20260507_multichip_resource_lifecycle_decision/` with `22/22` criteria, selecting lifecycle traffic with resource counters and authorizing Tier 4.32g-r0 route/source repair audit before another multi-chip hardware package; at that point learning scale, lifecycle scaling, multi-shard learning, speedup, benchmarks, and native-scale baseline freeze remained blocked pending 4.32g/4.32h.
...
readout repaired scalar scoring, but v2.3 still did not win. Tier 7.1e then
passed at
...
no-action collapse. Tier 7.4d then passed at
`controlled_test_output/tier7_4d_20260509_cost_aware_policy_action_heldout_contract/`,
locking the held-out/public action-cost scoring contract for v2.4 before any
scoring. Tier 7.4e then passed at
`controlled_test_output/tier7_4e_20260509_cost_aware_policy_action_heldout_preflight/`,
verifying source/split/cost/schema readiness before scoring. Tier 7.4f then
passed at
...
event-coverage non-confirmation. Tier 7.4h then passed at
...
and not hardware/native transfer. Tier 7.5d then passed at
...
Tier 7.6c then passed at `controlled_test_output/tier7_6c_20260509_long_horizon_planning_attribution_closeout/`, preserving the scaffold signal but blocking promotion/freeze/hardware transfer pending reduced-feature generalization. Tier 7.6d then passed at `controlled_test_output/tier7_6d_20260509_reduced_feature_planning_generalization/`, repairing the prior weak families under aliased/coarse feature access. Tier 7.6e then passed at `controlled_test_output/tier7_6e_20260509_planning_promotion_compact_regression/`, froze `CRA_EVIDENCE_BASELINE_v2.5` after full NEST compact regression, and Tier 7.7a then passed at `controlled_test_output/tier7_7a_20260509_v2_5_standardized_scoreboard_contract/`, locking the v2.5 public/standardized scoreboard contract before scoring. Tier 7.7b then passed at `controlled_test_output/tier7_7b_20260509_v2_5_standardized_scoreboard_scoring_gate/` with outcome `standardized_progress_pass`: v2.5 improved the locked aggregate versus v2.3, but the gain was Mackey-Glass-driven and ESN/online-linear/ridge baselines still beat v2.5 on aggregate. Tier 7.7c then passed at `controlled_test_output/tier7_7c_20260509_standardized_long_run_failure_contract/`, locking the long-run/failure-localization matrix before Tier 7.7d scoring. Tier 7.7d then passed as a blocker-classification harness at `controlled_test_output/tier7_7d_20260509_standardized_long_run_failure_scoring_gate/`: the Mackey-Glass signal persisted, but the complete long-run scoreboard is blocked because NARMA10 became non-finite at 16000 and 32000. Tier 7.7e must repair/preflight the finite stream before rerunning.
...
`cra_432a_hw/experiments/tier4_32a_hw_single_shard_scale_stress.py --mode
run-hardware --output-dir tier4_32a_hw_job_output`. This is preparation only,
not hardware evidence; the result must be returned and ingested before
replicated stress, multi-chip work, or a native-scale baseline freeze reopens.
Tier 4.32a-hw then passed after EBRAINS ingest at
`controlled_test_output/tier4_32a_hw_20260507_hardware_pass_ingested/`: board
`10.11.215.185`, raw status `pass`, ingest status `pass`, `31/31` raw hardware
...
`14/14` criteria, and the EBRAINS run later passed after ingest at
`controlled_test_output/tier4_32a_hw_replicated_20260507_hardware_pass_ingested/`.
This is single-chip replicated-shard hardware evidence only. Tier 4.32b static
reef partition smoke/resource mapping then passed locally at
`controlled_test_output/tier4_32b_20260507_static_reef_partition_smoke/`; Tier
4.32c inter-chip feasibility contract then passed locally at
`controlled_test_output/tier4_32c_20260507_interchip_feasibility_contract/` with
`19/19` criteria. Tier 4.32d-r0 route/source/package audit then passed locally
at `controlled_test_output/tier4_32d_r0_20260507_interchip_route_source_audit/`
with `10/10` criteria and blocked the first 4.32d upload until inter-chip route
repair. Tier 4.32d-r1 then passed locally at
...
explicit host-side context binding reaches `1.0` task accuracy on the repaired
...
- The run learned the fixed-pattern capsule in the limited sense required here:
  overall strict accuracy is above threshold, tail strict accuracy reaches `1.0`,
