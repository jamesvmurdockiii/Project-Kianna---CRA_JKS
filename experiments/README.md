...
Tier 4.29f is complete: the compact native mechanism evidence-regression gate passed over 4.29a-e and froze `CRA_NATIVE_MECHANISM_BRIDGE_v0.3`. Tier 7.0 standard dynamical benchmarks are complete as a diagnostic: CRA v2.1 ranked 5/5 on the tested continuous-valued sequence baselines. Tier 7.0b localized the gap to a recoverable state-signal/default-readout failure. Tier 7.0c bounded continuous readout/interface repair passed its integrity criteria, improved raw CRA, and beat shuffled/frozen controls, but lag-only online LMS still explained most of the benchmark gain. Tier 7.0d then confirmed that lag regression explains this benchmark path under the prior interface, so no continuous-readout mechanism was promoted. Tier 5.19 / 7.0e then defined the temporal-substrate contract. Tier 5.19a passed as a local software reference, Tier 5.19b passed as a stricter sham gate with classification `fading_memory_supported_recurrence_unproven`, and Tier 5.19c passed the narrowed fading-memory compact-regression gate with full NEST compact regression. The current frozen software baseline is `v2.2`, bounded to host-side fading-memory temporal state; bounded nonlinear recurrence, universal benchmark superiority, and hardware/on-chip temporal dynamics remain unproven. The active plan is now Tier 4.30-readiness audit before lifecycle-native contract work.
