...
smallest plausible specialist allocation:

* 14 standard bounded recurrent/LIF-style excitatory units + 2 resonant branches

The point is to ask whether a tiny resonant specialist subpool can preserve
...
TIER = "Tier 5.20c - Minimal Hybrid Resonant/LIF Polyp Diagnostic"
RUNNER_REVISION = "tier5_20c_minimal_resonant_polyp_diagnostic_20260508_0001"
DEFAULT_OUTPUT_DIR = OUTPUT_ROOT / "tier5_20c_20260508_minimal_resonant_polyp_diagnostic"

HYBRID_14_2 = "hybrid_14_lif_2_resonant"
HYBRID_14_2_FLAT = "hybrid_14_lif_2_flat_tau_sham"
HYBRID_14_2_RATE = "hybrid_14_lif_2_rate_only_sham"
HYBRID_14_2_SHUFFLED = "hybrid_14_lif_2_shuffled_branch_sham"
REQUIRED_MODELS = [
    V23,
    V22,
    FULL_RESONANT,
...
def run_task_models(task: Any, *, seed: int, args: argparse.Namespace, capture_timeseries: bool) -> tuple[list[dict[str, Any]], list[dict[str, Any]], dict[str, Any]]:
    ...
    model_bundles = {
        ...
        HYBRID_14_2: hybrid_features(task.observed, seed=seed, train_end=task.train_end, lif_units=14, branch_count=2, branch_mode="resonant", args=args),
        HYBRID_14_2_FLAT: hybrid_features(task.observed, seed=seed, train_end=task.train_end, lif_units=14, branch_count=2, branch_mode="flat_tau", args=args),
        HYBRID_14_2_RATE: hybrid_features(task.observed, seed=seed, train_end=task.train_end, lif_units=14, branch_count=2, branch_mode="rate_only", args=args),
        HYBRID_14_2_SHUFFLED: hybrid_features(task.observed, seed=seed, train_end=task.train_end, lif_units=14, branch_count=2, branch_mode="shuffled", args=args),
    }
    ...
    diagnostics = {
        "task": task.name,
        "seed": int(seed),
        "same_budget_variant": "14_lif_2_resonant",
        "v23_feature_count": int(model_bundles[V23].features.shape[1]),
        "hybrid_14_2_feature_count": int(model_bundles[HYBRID_14_2].features.shape[1]),
    }
...
def classify(summary_rows: list[dict[str, Any]], aggregate_summary: list[dict[str, Any]], tasks: list[str]) -> dict[str, Any]:
    candidate = evaluate_candidate(
        candidate=HYBRID_14_2,
        shams=[HYBRID_14_2_FLAT, HYBRID_14_2_RATE, HYBRID_14_2_SHUFFLED],
        tasks=tasks,
        summary_rows=summary_rows,
        aggregate_summary=aggregate_summary,
    )
    if candidate["useful_signal"] and candidate["no_material_regression_vs_v2_3"]:
        outcome = "minimal_resonant_candidate_for_integration_gate"
        recommendation = "Proceed to optional-polyp integration gate for 14 LIF / 2 resonant branches."
    elif candidate["no_material_regression_vs_v2_3"] and candidate["sham_separation_count"] >= 1:
        outcome = "minimal_resonant_research_scaffold_not_promoted"
        recommendation = "Keep 14/2 as a documented scaffold; do not integrate until stronger task value appears."
    else:
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    criteria = [
        ...
        criterion("same budget 14/2 variant declared", "14 LIF + 2 resonant", "sum == 16 excitatory units", True),
        ...
        criterion("14/2 shams present", [HYBRID_14_2_FLAT, HYBRID_14_2_RATE, HYBRID_14_2_SHUFFLED], "all present", all(model in models for model in [HYBRID_14_2_FLAT, HYBRID_14_2_RATE, HYBRID_14_2_SHUFFLED])),
    ...
    minimal_contract = {
        "mechanism": "minimal_hybrid_resonant_lif_branches",
        "tested_layout": {"standard_lif_excitatory_units": 14, "resonant_branch_units": 2, "total_excitatory_budget": 16},
        "repair_target": "test whether the smallest resonant specialist dose preserves v2.3 while retaining any variable-delay/anomaly value",
        "nonclaims": [
    ...
    payload = {
        ...
        "claim_boundary": (
            "Tier 5.20c is a software-only minimal-dose diagnostic after "
            "5.20a/5.20b. It tests a same-budget 14 LIF / 2 resonant branch "
            "internal-polyp proxy against v2.3, v2.2, full-resonant, "
            "lag/reservoir/ESN controls, and 14/2 shams. It is not a canonical "
            "organism change, not hardware evidence, and not a baseline freeze."
        ),
    }
    write_json(output_dir / "tier5_20c_results.json", payload)
    write_json(output_dir / "tier5_20c_minimal_contract.json", minimal_contract)
    write_csv_rows(output_dir / "tier5_20c_summary.csv", summary_rows)
    write_csv_rows(output_dir / "tier5_20c_aggregate.csv", aggregate_rows)
    write_csv_rows(output_dir / "tier5_20c_aggregate_summary.csv", aggregate_summary)
    if capture_timeseries:
        write_csv_rows(output_dir / "tier5_20c_timeseries.csv", all_timeseries)
    write_report(output_dir / "tier5_20c_report.md", payload)
    latest = {
        "tier": TIER,
        "status": payload["status"],
        "generated_at_utc": payload["generated_at_utc"],
        "output_dir": str(output_dir),
        "results_json": str(output_dir / "tier5_20c_results.json"),
        "report_md": str(output_dir / "tier5_20c_report.md"),
        "summary_csv": str(output_dir / "tier5_20c_summary.csv"),
        "classification": classification["outcome"],
    }
    write_json(OUTPUT_ROOT / "tier5_20c_latest_manifest.json", latest)
    return payload
