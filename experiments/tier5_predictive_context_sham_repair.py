...
predictive signal can solve the streams. Tier 5.12c repairs the sham-separation gate from 5.12c and tests the next bounded
...
TIER = "Tier 5.12c - Predictive Context Sham-Separation Repair"
DEFAULT_TASKS = "masked_input_prediction,event_stream_prediction,sensor_anomaly_prediction"
DEFAULT_VARIANTS = "v1_7_reactive,external_predictive_scaffold,internal_predictive_context,wrong_predictive_context,shuffled_predictive_context,permuted_predictive_context,no_write_predictive_context"
EPS = 1e-12
...
VARIANTS: tuple[PredictiveVariant, ...] = (
    ...
    PredictiveVariant(
        ...
        group="alternate_code_control",
        runner="internal",
        feature_mode="wrong",
        hypothesis="Diagnostic control: predictive context is systematically inverted. A stable sign inversion can still carry usable information, so this is reported but not used as the primary sham gate.",
        overrides={
            "learning.delayed_readout_learning_rate": 0.20,
            "learning.macro_eligibility_enabled": False,
            "learning.context_memory_enabled": True,
    ...
    PredictiveVariant(
        name="permuted_predictive_context",
        group="predictive_ablation",
        runner="internal",
        feature_mode="permuted",
        hypothesis="Control: predictive context writes a per-seed permuted key/label binding rather than the causal visible precursor label.",
        overrides={
            "learning.delayed_readout_learning_rate": 0.20,
            "learning.macro_eligibility_enabled": False,
            "learning.context_memory_enabled": True,
...
def parse_variants(raw: str) -> list[PredictiveVariant]:
    ...
    if missing:
        raise argparse.ArgumentTypeError(f"unknown Tier 5.12c variants: {', '.join(missing)}")
    ...
    if not required.issubset(present):
        raise argparse.ArgumentTypeError(
            "Tier 5.12c requires v1_7_reactive, external_predictive_scaffold, and internal_predictive_context"
        )
    return selected
...
def permuted_label_map(task: PredictiveTask, seed: int) -> dict[int, int]:
    labels = trial_label_map(task)
    trials = sorted(labels)
    values = [labels[trial] for trial in trials]
    rng = np.random.default_rng(seed + 9512)
    if len(values) > 1:
        for _ in range(8):
            shuffled = list(values)
            rng.shuffle(shuffled)
            if any(int(shuffled[idx]) != int(values[idx]) for idx in range(len(values))):
                values = shuffled
                break
    return {trial: int(values[idx]) for idx, trial in enumerate(trials)}


def predictive_metadata_for_step(
    task: PredictiveTask,
    step: int,
    label_by_trial: dict[int, int],
    *,
    variant: PredictiveVariant,
    permuted_label_by_trial: dict[int, int],
) -> dict[str, Any]:
    event = str(task.event_type[step])
    trial = int(task.trial_id[step])
    label = int(label_by_trial.get(trial, 0))
    if variant.feature_mode == "permuted":
        label = int(permuted_label_by_trial.get(trial, 0))
...
def run_cra_predictive_variant(
    ...
    *,
    ...
    permuted_label_by_trial = permuted_label_map(task, seed)
    ...
    try:
        ...
        for step in range(task.stream.steps):
            ...
            metadata = predictive_metadata_for_step(
                task,
                step,
                label_by_trial,
                variant=variant,
                permuted_label_by_trial=permuted_label_by_trial,
            )
...
def evaluate_tier(
    *,
    ...
    science_criteria = [
        ...
        criterion(
            "information-destroying predictive shams are worse than candidate",
            ...
            "Stable wrong-sign coding is reported separately because it can remain learnably informative; the gate uses shuffled/permuted/no-write shams.",
        ),
        criterion(
    ...
    summary = {
        ...
        "excluded_default_task": "hidden_regime_switching is excluded from default 5.12c because this mechanism requires visible precursor writes; latent regime inference is future work.",
    }
...
def plot_predictive_context(comparisons: list[dict[str, Any]], path: Path) -> None:
    ...
    ax.set_title("Tier 5.12c Internal Predictive Context Diagnostic")
    ax.set_xticks(x)
...
def fairness_contract(args: argparse.Namespace, variants: list[PredictiveVariant], models: list[str]) -> dict[str, Any]:
    return {
        ...
        "fairness_rules": [
            ...
            "shuffled/permuted/no-write predictive-context shams must not match the candidate",
            "wrong-sign predictive context is reported as an alternate-code diagnostic, not an information-destroying sham",
            "feedback_due_step must be greater than or equal to prediction step",
            "passing Tier 5.12c does not freeze v1.8 without compact regression",
        ],
        "tasks": args.tasks,
        "steps": args.steps,
        "seeds": seeds_from_args(args),
...
def run_tier(args: argparse.Namespace, output_dir: Path) -> dict[str, Any]:
    ...
    for seed in seeds_from_args(args):
        for task in build_predictive_tasks(args, seed=args.task_seed + seed):
            ...
            for variant in variants:
                print(f"[tier5.12c] task={task.stream.name} variant={variant.name} seed={seed}", flush=True)
            ...
            for model in [*models, *CONTROL_MODELS]:
                print(f"[tier5.12c] task={task.stream.name} model={model} seed={seed}", flush=True)
    ...
    summary_csv = output_dir / "tier5_12c_summary.csv"
    comparisons_csv = output_dir / "tier5_12c_comparisons.csv"
    fairness_json = output_dir / "tier5_12c_fairness_contract.json"
    plot_path = output_dir / "tier5_12c_predictive_context.png"
...
def write_latest(output_dir: Path, report_path: Path, manifest_path: Path, summary_csv: Path, status: str) -> None:
    latest_path = ROOT / "controlled_test_output" / "tier5_12c_latest_manifest.json"
    payload = {
        ...
        "summary_csv": str(summary_csv),
        "canonical": False,
        "claim": "Latest Tier 5.12c predictive-context sham-separation repair; passing authorizes compact regression/promotion review only.",
    }
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.description = "Run Tier 5.12c predictive-context sham-separation repair."
    parser.set_defaults(
        backend="nest",
        tasks=DEFAULT_TASKS,
        steps=720,
    ...
    parser.add_argument("--min-candidate-accuracy", type=float, default=0.80)
    parser.add_argument("--min-candidate-tail-accuracy", type=float, default=0.85)
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_dir = args.output_dir or (ROOT / "controlled_test_output" / f"tier5_12c_{timestamp}")
    output_dir.mkdir(parents=True, exist_ok=True)
    result = run_tier(args, output_dir)
    manifest_path = output_dir / "tier5_12c_results.json"
    report_path = output_dir / "tier5_12c_report.md"
    summary_csv = output_dir / "tier5_12c_summary.csv"
    ...
    print(f"Tier 5.12c status: {result['status']}")
