...
Tier 5.11a showed a measured need: frozen v1.6 bounded keyed memory fails
silent reentry tail stress while unbounded keyed memory and oracle scaffold solve
it. Tier 5.11b tests whether prioritized offline replay can repair that need
without leakage, and whether shuffled/random/no-consolidation replay controls
remove the benefit.

This is software evidence only. It is not hardware replay, not on-chip replay,
and not a baseline freeze by itself.
"""
...
TIER = "Tier 5.11b - Prioritized Replay / Consolidation Intervention"
DEFAULT_VARIANTS = "v1_6_no_replay,prioritized_replay,shuffled_replay,random_replay,no_consolidation_replay,unbounded_keyed_control,oracle_context_scaffold"
EPS = 1e-12
...
VARIANTS: tuple[ReplayVariant, ...] = (
    ...
    ReplayVariant(
        ...
        hypothesis="Replay event count and keys are matched, but signs are deliberately shuffled across keys.",
        overrides={
            "learning.delayed_readout_learning_rate": 0.20,
            "learning.macro_eligibility_enabled": False,
            "learning.context_memory_enabled": True,
...
def parse_variants(raw: str) -> list[ReplayVariant]:
    ...
    if unknown:
        raise SystemExit(f"Unknown Tier 5.11b variants: {unknown}; available={sorted(available)}")
    required = {"v1_6_no_replay", "prioritized_replay", "shuffled_replay", "random_replay", "no_consolidation_replay", "unbounded_keyed_control", "oracle_context_scaffold"}
    missing = required.difference(requested)
    if missing:
        raise SystemExit(f"Tier 5.11b requires {sorted(required)}; missing={sorted(missing)}")
...
def replay_selection(
    *,
    ...
    if variant.replay_mode == "shuffled":
        signs = [int(item["sign"]) for item in base]
        if len(signs) == 1:
            signs = [-signs[0]]
        else:
            signs = signs[1:] + signs[:1]
            if all(signs[idx] == int(base[idx]["sign"]) for idx in range(len(base))):
                signs = [-int(s) for s in signs]
        return [{**item, "sign": int(signs[idx]), "shuffled_from_key": base[(idx + 1) % len(base)]["key"]} for idx, item in enumerate(base)]
    ...
    *,
    ...
    for rank, item in enumerate(selected):
        replay = organism.replay_context_memory_episode(
            ...
            source=f"tier5_11b_{variant.replay_mode}",
        )
        events.append(
            {
...
def build_comparisons(aggregates: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for task in sorted({row["task"] for row in aggregates}):
        ...
        shuffled = by_task_model.get((task, "shuffled_replay"), {})
        ...
        rows.append(
            {
                ...
                "shuffled_all_accuracy": shuffled.get("all_accuracy_mean"),
                ...
                "prioritized_all_delta_vs_shuffled": prio_acc - float(shuffled.get("all_accuracy_mean") or 0.0),
                ...
                "shuffled_tail_accuracy": shuffled.get("tail_accuracy_mean"),
                ...
                "prioritized_tail_delta_vs_shuffled": prio_tail - float(shuffled.get("tail_accuracy_mean") or 0.0),
                ...
                "shuffled_replay_consolidations": shuffled.get("replay_consolidations_sum"),
...
def evaluate_tier(
    *,
    ...
    edge_shuffled = [float(row.get("prioritized_tail_delta_vs_shuffled") or 0.0) for row in comparisons]
    ...
    criteria = [
        ...
        criterion("prioritized replay selected episodes", replay_events, ">", 0, replay_events > 0),
        criterion("prioritized replay consolidated episodes", replay_writes, ">", 0, replay_writes > 0),
    ]
    if not args.smoke:
        criteria.extend(
            [
                criterion("prioritized replay minimum all accuracy", min(prio_all) if prio_all else None, ">=", args.min_prioritized_all_accuracy, bool(prio_all) and min(prio_all) >= float(args.min_prioritized_all_accuracy)),
                criterion("prioritized replay minimum tail accuracy", min(prio_tail) if prio_tail else None, ">=", args.min_prioritized_tail_accuracy, bool(prio_tail) and min(prio_tail) >= float(args.min_prioritized_tail_accuracy)),
                criterion("prioritized replay improves tail over no replay", min(tail_delta_no) if tail_delta_no else None, ">=", args.min_tail_improvement_vs_no_replay, bool(tail_delta_no) and min(tail_delta_no) >= float(args.min_tail_improvement_vs_no_replay)),
                criterion("prioritized replay closes all-accuracy gap toward unbounded", min(all_closure) if all_closure else None, ">=", args.min_gap_closure, bool(all_closure) and min(all_closure) >= float(args.min_gap_closure)),
                criterion("prioritized replay closes tail gap toward unbounded", min(tail_closure) if tail_closure else None, ">=", args.min_tail_gap_closure, bool(tail_closure) and min(tail_closure) >= float(args.min_tail_gap_closure)),
                criterion("shuffled replay does not match prioritized tail", min(edge_shuffled) if edge_shuffled else None, ">=", args.min_control_tail_edge, bool(edge_shuffled) and min(edge_shuffled) >= float(args.min_control_tail_edge)),
                criterion("random replay does not match prioritized tail", min(edge_random) if edge_random else None, ">=", args.min_random_tail_edge, bool(edge_random) and min(edge_random) >= float(args.min_random_tail_edge)),
    ...
    summary = {
        ...
        "prioritized_min_tail_edge_vs_shuffled": min(edge_shuffled) if edge_shuffled else None,
...
def plot_replay_edges(comparisons: list[dict[str, Any]], path: Path) -> None:
    ...
    prio_tail = [float(row.get("prioritized_tail_accuracy") or 0.0) for row in comparisons]
    shuffled_tail = [float(row.get("shuffled_tail_accuracy") or 0.0) for row in comparisons]
    ...
    ax.bar(x - 0.5 * width, prio_tail, width, label="prioritized replay", color="#2f855a")
    ax.bar(x + 0.5 * width, shuffled_tail, width, label="shuffled replay", color="#b7791f")
    ax.bar(x + 1.5 * width, random_tail, width, label="random replay", color="#805ad5")
    ax.set_title("Tier 5.11b Prioritized Replay Intervention")
    ax.set_xticks(x)
...
def fairness_contract(args: argparse.Namespace, variants: list[ReplayVariant], models: list[str]) -> dict[str, Any]:
    return {
        ...
        "replay_controls": ["shuffled_replay", "random_replay", "no_consolidation_replay"],
        ...
        "fairness_rules": [
            ...
            "prioritized replay selects rare/old observed contexts without reading future labels or targets",
            "shuffled/random/no-consolidation controls match replay opportunity but remove prioritized consolidation semantics",
...
def write_report(path: Path, result: dict[str, Any], aggregates: list[dict[str, Any]], comparisons: list[dict[str, Any]], args: argparse.Namespace, output_dir: Path) -> None:
    ...
    lines = [
        ...
        "## Summary Metrics",
        "",
        f"- prioritized replay events: `{tier_summary.get('prioritized_replay_events_sum')}`",
        f"- prioritized replay consolidations: `{tier_summary.get('prioritized_replay_consolidations_sum')}`",
        f"- no-consolidation writes: `{tier_summary.get('no_consolidation_writes_sum')}`",
        f"- prioritized min all accuracy: `{tier_summary.get('prioritized_min_all_accuracy')}`",
        f"- prioritized min tail accuracy: `{tier_summary.get('prioritized_min_tail_accuracy')}`",
        f"- prioritized min tail delta vs no replay: `{tier_summary.get('prioritized_min_tail_delta_vs_no_replay')}`",
        f"- prioritized min all gap closure: `{tier_summary.get('prioritized_min_all_gap_closure')}`",
        f"- prioritized min tail gap closure: `{tier_summary.get('prioritized_min_tail_gap_closure')}`",
        f"- prioritized min tail edge vs shuffled: `{tier_summary.get('prioritized_min_tail_edge_vs_shuffled')}`",
        f"- prioritized min tail edge vs random: `{tier_summary.get('prioritized_min_tail_edge_vs_random')}`",
        f"- prioritized min tail edge vs no-consolidation: `{tier_summary.get('prioritized_min_tail_edge_vs_no_consolidation')}`",
        "",
        "## Task Comparisons",
        "",
    ...
    for row in comparisons:
        lines.append(
            ...
            f"{markdown_value(row.get('shuffled_tail_accuracy'))} | "
            ...
            f"{markdown_value(row.get('prioritized_tail_delta_vs_shuffled'))} | "
            f"{markdown_value(row.get('prioritized_tail_delta_vs_random'))} | "
...
def write_latest(output_dir: Path, report_path: Path, manifest_path: Path, summary_csv: Path, status: str) -> None:
    latest_path = ROOT / "controlled_test_output" / "tier5_11b_latest_manifest.json"
    payload = {
        ...
        "canonical": False,
        "claim": "Latest Tier 5.11b prioritized replay/consolidation intervention; not hardware replay or native on-chip memory.",
    }
...
def run_tier(args: argparse.Namespace, output_dir: Path, variants: list[ReplayVariant]) -> dict[str, Any]:
    ...
    for seed in seeds_from_args(args):
        for task in build_tasks(args, seed):
            ...
            for variant in variants:
                print(f"[tier5.11b] task={task.stream.name} variant={variant.name} seed={seed}", flush=True)
            ...
            for model in models:
                print(f"[tier5.11b] task={task.stream.name} model={model} seed={seed}", flush=True)
            ...
            for control in CONTROL_MODELS:
                print(f"[tier5.11b] task={task.stream.name} control={control} seed={seed}", flush=True)
    ...
    summary_csv = output_dir / "tier5_11b_summary.csv"
    comparisons_csv = output_dir / "tier5_11b_comparisons.csv"
    replay_events_csv = output_dir / "tier5_11b_replay_events.csv"
    fairness_json = output_dir / "tier5_11b_fairness_contract.json"
    plot_path = output_dir / "tier5_11b_replay_edges.png"
    ...
    return {
        "name": "prioritized_replay_intervention",
        "status": status,
        "summary": {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.description = "Run Tier 5.11b prioritized replay / consolidation intervention."
    parser.set_defaults(
        backend="nest",
        tasks=DEFAULT_TASKS,
        steps=960,
    ...
    parser.add_argument("--min-random-tail-edge", type=float, default=0.20)
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_dir = args.output_dir or (ROOT / "controlled_test_output" / f"tier5_11b_{timestamp}")
    output_dir.mkdir(parents=True, exist_ok=True)
    result = run_tier(args, output_dir, variants)
    manifest_path = output_dir / "tier5_11b_results.json"
    report_path = output_dir / "tier5_11b_report.md"
    summary_csv = output_dir / "tier5_11b_summary.csv"
    comparisons_csv = output_dir / "tier5_11b_comparisons.csv"
    replay_events_csv = output_dir / "tier5_11b_replay_events.csv"
    fairness_json = output_dir / "tier5_11b_fairness_contract.json"
    manifest = {
        ...
        "artifacts": {
            ...
            "replay_edges_png": str(output_dir / "tier5_11b_replay_edges.png"),
        },
    }
