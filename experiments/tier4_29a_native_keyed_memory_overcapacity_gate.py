...
Tests the native SpiNNaker keyed context memory mechanism under stress with
explicit controls: wrong-key events, overwrite events, and slot-shuffle patterns.
Uses a four-core MCPL distributed runner (context/route/memory/learning cores).
...
Claim boundary:
  Native keyed context memory works for multi-slot tables (8–12 slots).
  Wrong-key, overwrite, and shuffle controls pass.
  This is a single-mechanism bridge; not routing, replay, predictive binding,
...
# Tier constants
# ---------------------------------------------------------------------------
TIER = "Tier 4.29a — Native Keyed-Memory Overcapacity Gate"
RUNNER_REVISION = "tier4_29a_native_keyed_memory_overcapacity_20260503_0001"
UPLOAD_PACKAGE_NAME = "cra_429b"
...
# Overwrite: key 201 changes from +1.0 to -1.0 between phases
OVERWRITE_KEY = 201
OVERWRITE_OLD = 1.0
OVERWRITE_NEW = -1.0
...
def _generate_task_sequence() -> list[dict[str, Any]]:
    """Generate the 32-event signed stream with shuffle, wrong-key, and overwrite."""
    slots = dict(CONTEXT_SLOTS)
    events: list[dict[str, Any]] = []

    def add_event(step: int, key: int, cue: float, target: float, purpose: str) -> None:
        events.append({
            "step": step,
            "context_key": key,
            "route_key": NEUTRAL_ROUTE_KEY,
    ...
    # Phase 1: initial reads in order
    for i, (key, val) in enumerate(slots.items()):
        add_event(i + 1, key, 1.0, val, f"phase1_order_read_{key}")

    # Phase 1: shuffled reverse reads with flipped cue (last 4 slots only)
    shuffled_keys = [208, 207, 206, 205]
    for i, key in enumerate(shuffled_keys):
        val = slots[key]
        add_event(9 + i, key, -1.0, -val, f"phase1_shuffle_read_{key}")

    # Phase 1: wrong-key events
    add_event(13, WRONG_KEYS[0], 1.0, 0.0, "phase1_wrong_key_1")
    add_event(14, WRONG_KEYS[1], 1.0, 0.0, "phase1_wrong_key_2")

    # Phase 1: re-read first two slots
    add_event(15, 201, 1.0, slots[201], "phase1_re_read_201")
    add_event(16, 202, 1.0, slots[202], "phase1_re_read_202")

    # --- OVERWRITE between phases ---
    slots[OVERWRITE_KEY] = OVERWRITE_NEW

    # Phase 2: overwrite verification
    add_event(17, OVERWRITE_KEY, 1.0, OVERWRITE_NEW, "phase2_overwrite_verify")
    add_event(18, OVERWRITE_KEY, -1.0, -OVERWRITE_NEW, "phase2_overwrite_verify_neg")

    # Phase 2: scattered reads (shuffle control)
    scattered = [203, 204, 205, 206]
    for i, key in enumerate(scattered):
        val = slots[key]
        cue = 1.0 if i % 2 == 0 else -1.0
        add_event(19 + i, key, cue, cue * val, f"phase2_scatter_{key}")

    # Phase 2: wrong-key events
    add_event(23, WRONG_KEYS[2], 1.0, 0.0, "phase2_wrong_key_1")
    add_event(24, WRONG_KEYS[3], 1.0, 0.0, "phase2_wrong_key_2")

    # Phase 2: mixed re-reads in new order
    mixed = [202, 203, 204, 207, 208, 205]
    for i, key in enumerate(mixed):
        val = slots[key]
        cue = 1.0 if i % 3 != 0 else -1.0
        add_event(25 + i, key, cue, cue * val, f"phase2_mixed_{key}")

    # Phase 2: additional wrong-key events
    add_event(31, 995, 1.0, 0.0, "phase2_wrong_key_3")
    add_event(32, 994, 1.0, 0.0, "phase2_wrong_key_4")
...
def _compute_reference() -> dict[str, Any]:
    ...
    # Track simulated slot table (linear scan, LRU not needed for 8 slots)
    ...
    for idx, event in enumerate(TASK_SEQUENCE):
        ...
        # Apply overwrite between phases
        if step == 17:
            slots[OVERWRITE_KEY] = OVERWRITE_NEW
        ...
        # Feature = context * route * memory * cue  (route=memory=1.0)
        feature = ctx_val * cue
    ...
    # Count wrong-key events
    wrong_key_events = [e for e in TASK_SEQUENCE if e["context_key"] in WRONG_KEYS]
    wrong_key_count = len(wrong_key_events)

    # Count overwrite events (events using overwritten key after step 16)
    overwrite_events = [
    ...
    return {
        ...
        "overwrite_events": len(overwrite_events),
        "slot_writes": len(CONTEXT_SLOTS) + 1,  # 8 initial + 1 overwrite
        "slot_hits": len(TASK_SEQUENCE) - wrong_key_count,
        "slot_misses": wrong_key_count,
...
def build_aplx_for_profile(profile: str, output_dir: Path) -> dict[str, Any]:
    ...
    (output_dir / f"tier4_29a_build_{profile}_stdout.txt").write_text(build["stdout"])
    (output_dir / f"tier4_29a_build_{profile}_stderr.txt").write_text(build["stderr"])
...
def mode_local(args: argparse.Namespace) -> dict[str, Any]:
    ...
    default_dir = f"tier4_29a_seed{args.seed}_job_output"
    ...
    print(f"  wrong_key={ref['wrong_key_count']}, overwrite_events={ref['overwrite_events']}, slot_hits={ref['slot_hits']}, slot_misses={ref['slot_misses']}")
    ...
    criteria = [
        ...
        {"name": "reference slot hits match", "value": ref["slot_hits"], "rule": "== 26", "passed": ref["slot_hits"] == 26},
        {"name": "reference slot misses match", "value": ref["slot_misses"], "rule": "== 6", "passed": ref["slot_misses"] == 6},
        {"name": "reference wrong-key count match", "value": ref["wrong_key_count"], "rule": "== 6", "passed": ref["wrong_key_count"] == 6},
        {"name": "reference overwrite events match", "value": ref["overwrite_events"], "rule": ">= 2", "passed": ref["overwrite_events"] >= 2},
    ]
    ...
    output_path = output_dir / "tier4_29a_local_results.json"
...
def mode_prepare(args: argparse.Namespace) -> dict[str, Any]:
    ...
    scripts = [
        "tier4_29a_native_keyed_memory_overcapacity_gate.py",
    ...
    # 4. Write README
    print("\n[4/4] Writing README...")
    command_single = f"{UPLOAD_PACKAGE_NAME}/experiments/tier4_29a_native_keyed_memory_overcapacity_gate.py --mode run-hardware --seed 42"
    command_multi = f"{UPLOAD_PACKAGE_NAME}/experiments/tier4_29a_native_keyed_memory_overcapacity_gate.py --mode run-hardware --seeds 42,43,44"
...
"""
    ...
    readme = bundle / "README_TIER4_29A_JOB.md"
    ...
    output_path = output_dir / "tier4_29a_prepare_results.json"
    output_path.write_text(json.dumps(result, indent=2))
    print(f"\nArtifact: {output_path}")
    print(f"\n{'=' * 60}")
    print(f"OVERALL: {status.upper()}")
...
def four_core_hardware_loop(
    ...
    try:
        ...
        # Write neutral route and memory slots
        route_writes = []
        ok = route_ctrl.write_route_slot(NEUTRAL_ROUTE_KEY, 1.0, 1.0, dest_x, dest_y, ROUTE_CORE_P)
        route_writes.append({"key": NEUTRAL_ROUTE_KEY, "success": ok.get("success") is True})
...
def mode_run_hardware(args: argparse.Namespace) -> dict[str, Any]:
    ...
    write_json(output_dir / "tier4_29a_environment.json", env_report)
    write_json(output_dir / "tier4_29a_target_acquisition.json", base.public_target_acquisition(target))
    ...
    try:
        if target.get("status") == "pass" and hostname:
            ...
            for name, load_info in loads.items():
                write_json(output_dir / f"tier4_29a_{name}_load.json", load_info)
        ...
        if (target.get("status") == "pass" and hostname
            ...
            write_json(output_dir / "tier4_29a_task.json", task_result)
    except Exception as exc:
        hardware_exception = {
    ...
    criteria = [
        ...
        # Tier 4.29a specific criteria
        criterion("wrong-key events fail cleanly (context misses)", ctx_final.get("slot_misses"), f"== {ref['slot_misses']}", ctx_final.get("slot_misses") == ref["slot_misses"]),
        criterion("overwrite events use new value (context writes)", ctx_final.get("slot_writes"), f"== {ref['slot_writes']}", ctx_final.get("slot_writes") == ref["slot_writes"]),
    ...
    write_json(output_dir / "tier4_29a_hardware_results.json", result)
...
def mode_ingest(args: argparse.Namespace) -> dict[str, Any]:
    print("Tier 4.29a — Ingest hardware results")
    print("=" * 60)
    ...
    hw_results_path = hw_dir / "tier4_29a_hardware_results.json"
    ...
    output_path = output_dir / "tier4_29a_ingest_results.json"
    output_path.write_text(json.dumps(result, indent=2))
    ...
    (output_dir / "tier4_29a_ingest_report.md").write_text(report)

    print(f"OVERALL: {status.upper()}")
    print(f"Passed: {passed}/{total}")
    print(f"Ingest report: {output_dir / 'tier4_29a_ingest_report.md'}")
    return result
    ...
    try:
        if mode == "local":
            ...
            if args.seeds:
                ...
                combined_path = Path(f"tier4_29a_multi_seed_job_output") / "tier4_29a_combined_results.json"
        ...
        crash_path = Path(f"tier4_29a_seed{args.seed}_job_output") / "tier4_29a_crash_report.json"
