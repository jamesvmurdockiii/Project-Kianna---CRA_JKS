# CRA Codebase Contract

This document is the operating contract for any AI agent, human collaborator, or
...
The project is not allowed to rely on vibes, hype, or single-run demos. Every
...
10. Do not make the user upload generated evidence to EBRAINS. Job upload folders
...
12. The repo must stay readable enough that another competent researcher can
    reproduce the result without private chat history.
...
Any AI agent working here must reason like a careful research engineer, not like
a demo builder.
...
Do not optimize for looking right. Optimize for making the project more true.
...
4. Giving the user a JobManager command.
5. Ingesting results.
...
`Noncanonical` does not mean garbage. It means the result is not a formal
...
14. Run validation.
15. Tell the user what happened, what it means, what it does not mean, and the
    exact next step.
...
Before telling the user what to upload or run, verify the prepared package like
an auditor, not from memory.
...
9. Confirm the job README filename, title, tier number, uploaded folder name,
   ...
   the wrong tier, fix the runner/package before telling the user to upload it.
...
The user downloads EBRAINS returned outputs to `/Users/james/Downloads`.
...
Do not delete files from Downloads unless the user explicitly asks. Copy or
normalize what is needed into `controlled_test_output/`.

### 9.4 JobManager Result Ingest SOP

When the user says an EBRAINS/JobManager run finished or errored and files were
...
17. Report to the user using this shape:
...
reading the returned files. Never make the user upload `controlled_test_output/`
...
10. If image size is the issue, prefer real profile/resource-budget fixes over
    ...
    `-Os`, section garbage collection, and document the profile.
...
Do not create experiment scripts that require private chat context, manually
...
4. Verify the command starts with the actual folder the user must upload.
...
| EBRAINS package mistakes | Tier prepare mode plus Section 9.1.1 package QA | The upload folder, README, command, artifacts, and report must agree before the user is told to upload. |
...
currently practical. Do not rely on memory, chat history, or "we know what we
mean."
...
| It is hype. | Claim boundaries, failures preserved, nonclaims stated. |
...
When reporting to the user, separate these clearly:
...
Do not make the user do investigative work that the agent can do. Do not ask the
user to upload GB-scale generated folders. Do not tell the user to run shell
...
18. Telling the user to upload a similarly named older `cra_*` folder.

## 21. Session Start Checklist For Any AI Agent

At the start of a new work session:

1. Confirm repo root. Preferred root is `/Users/james/JKS:CRA` unless the user
...
8. Inspect returned files in `/Users/james/Downloads` only if the user says new
...
This section exists so another AI agent can continue the project at the same
...
4. The user asks whether the current approach is scientifically correct.
...
research becomes part of the repo, not private context.

### 23.6 User Interaction Standard

The user should not have to manage the agent.

The agent must:

1. Give short progress updates during substantial work.
2. Be explicit when confused and then investigate.
3. Avoid making the user upload unnecessary folders.
...
Before giving a new AI agent write access to serious code, evidence, EBRAINS
...
```text
The user downloaded 20 files from a failed EBRAINS run. What do you do?
```
...
Use this section when handing the repo to another AI agent. The goal is to force
...
This lets the user catch misorientation immediately.
