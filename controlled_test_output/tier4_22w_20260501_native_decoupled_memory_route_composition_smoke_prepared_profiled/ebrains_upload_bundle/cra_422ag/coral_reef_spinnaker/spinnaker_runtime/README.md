...
Prepared package `ebrains_jobs/cra_422af` is source/package readiness only
until returned EBRAINS hardware artifacts pass.
...
Tier 4.22w extends this surface with independent-key composition through `CMD_SCHEDULE_DECOUPLED_MEMORY_ROUTE_CONTEXT_PENDING`. It is prepared as `ebrains_jobs/cra_422af` and sends `context_key`, `route_key`, `memory_key`, `cue`, and `delay` so the chip computes `feature=context[context_key]*route[route_key]*memory[memory_key]*cue`. This is still a tiny custom-runtime primitive, not full native v2.1 memory/routing or full CRA task learning.
