...
| bundle native context and keyed-route command constants exist | `CMD_WRITE_CONTEXT/CMD_WRITE_ROUTE_SLOT/CMD_SCHEDULE_KEYED_ROUTE_CONTEXT_PENDING` | `custom runtime must expose native context plus keyed route-state primitives` | yes |
...
| bundle native keyed routed schedule forms feature on chip | `feature = FP_MUL(FP_MUL(context_value, route_value), cue)` | `host must send key+cue, while the runtime retrieves context+route slot by key and computes the scalar feature` | yes |
