...
Tier 4.22p is prepared as the next tiny reentry gate using the same command
surface. It uses a 30-event A-B-A stream: target follows feature in regime A,
reverses in regime B, then returns to the original A rule. Local fixed-point
reference and prepare gates passed with `pending_gap_depth=2`, max pending
depth `3`, accuracy `0.8666666667`, tail accuracy `1.0`, final
`readout_weight_raw=30810`, and final `readout_bias_raw=-1`. The upload folder
is `ebrains_jobs/cra_422y`. This is not hardware evidence until returned
`run-hardware` artifacts are ingested and pass.
