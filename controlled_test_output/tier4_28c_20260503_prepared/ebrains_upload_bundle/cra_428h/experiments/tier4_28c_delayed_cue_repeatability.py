...
Extends 4.28b from a one-seed delayed-cue probe to three-seed repeatability.
The runner, task sequence, four-core MCPL scaffold, and pass criteria are
identical to 4.28b. Only the tier label and runner revision change.
...
Claim boundary:
  ...
  Three-seed repeatability validates task transfer robustness before mechanism
  maturation. Not multi-chip scaling, not speedup evidence, not v2.1 transfer.
"""
...
TIER = "Tier 4.28c — Delayed-Cue Three-Seed Repeatability"
RUNNER_REVISION = "tier4_28c_delayed_cue_repeatability_20260503_0001"
UPLOAD_PACKAGE_NAME = "cra_428h"
...
def build_aplx_for_profile(profile: str, output_dir: Path) -> dict[str, Any]:
    ...
    (output_dir / f"tier4_28c_build_{profile}_stdout.txt").write_text(build["stdout"])
    (output_dir / f"tier4_28c_build_{profile}_stderr.txt").write_text(build["stderr"])
...
def mode_local(args: argparse.Namespace) -> dict[str, Any]:
    ...
    default_dir = f"tier4_28c_seed{args.seed}_job_output"
    ...
    output_path = output_dir / "tier4_28c_local_results.json"
...
def mode_prepare(args: argparse.Namespace) -> dict[str, Any]:
    ...
    # Copy runner scripts
    scripts = [
        "tier4_28c_delayed_cue_repeatability.py",
    ...
    # 4. Write README
    print("\n[4/4] Writing README...")
    command_single = f"{UPLOAD_PACKAGE_NAME}/experiments/tier4_28c_delayed_cue_repeatability.py --mode run-hardware --seed 42"
    command_multi  = f"{UPLOAD_PACKAGE_NAME}/experiments/tier4_28c_delayed_cue_repeatability.py --mode run-hardware --seeds 42,43,44"
...
"""
    ...
    readme = bundle / "README_TIER4_28c_JOB.md"
    ...
    output_path = output_dir / "tier4_28c_prepare_results.json"
    output_path.write_text(json.dumps(result, indent=2))
    print(f"\nArtifact: {output_path}")
    print(f"\n{'=' * 60}")
    print(f"OVERALL: {status.upper()}")
...
def mode_run_hardware(args: argparse.Namespace) -> dict[str, Any]:
    ...
    write_json(output_dir / "tier4_28c_environment.json", env_report)
    write_json(output_dir / "tier4_28c_target_acquisition.json", base.public_target_acquisition(target))
    ...
    try:
        if target.get("status") == "pass" and hostname:
            ...
            for name, load_info in loads.items():
                write_json(output_dir / f"tier4_28c_{name}_load.json", load_info)
        ...
        if (target.get("status") == "pass" and hostname
            ...
            write_json(output_dir / "tier4_28c_task.json", task_result)
    except Exception as exc:
        hardware_exception = {
    ...
    write_json(output_dir / "tier4_28c_hardware_results.json", result)
...
def mode_ingest(args: argparse.Namespace) -> dict[str, Any]:
    print("Tier 4.28c — Ingest hardware results")
    print("=" * 60)
    ...
    hw_results_path = hw_dir / "tier4_28c_hardware_results.json"
    ...
    output_path = output_dir / "tier4_28c_ingest_results.json"
    output_path.write_text(json.dumps(result, indent=2))
    ...
    (output_dir / "tier4_28c_ingest_report.md").write_text(report)

    print(f"OVERALL: {status.upper()}")
    print(f"Passed: {passed}/{total}")
    print(f"Ingest report: {output_dir / 'tier4_28c_ingest_report.md'}")
    return result
    ...
    try:
        if mode == "local":
            ...
            if args.seeds:
                ...
                combined_path = Path(f"tier4_28c_multi_seed_job_output") / "tier4_28c_combined_results.json"
        ...
        crash_path = Path(f"tier4_28c_seed{args.seed}_job_output") / "tier4_28c_crash_report.json"
