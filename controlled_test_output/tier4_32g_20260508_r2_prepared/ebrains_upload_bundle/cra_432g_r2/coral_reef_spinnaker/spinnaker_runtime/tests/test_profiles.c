...
static void test_expected_handlers(void) {
    ...
    assert(_dispatch(CMD_LIFECYCLE_EVENT, 5, LIFECYCLE_EVENT_DEATH, 3) == 0);
    ...
    assert(_dispatch(CMD_LIFECYCLE_TROPHIC_UPDATE, 4, FP_ONE / 4, 0) == 0);
...
static void test_unexpected_handlers(void) {
    ...
    assert(_dispatch(CMD_LIFECYCLE_INIT, MAX_LIFECYCLE_SLOTS, 2, 42) == 0xFF);
    assert(_dispatch(CMD_LIFECYCLE_READ_STATE, 0, 0, 0) == 0xFF);
    assert(_dispatch(CMD_LIFECYCLE_SHAM_MODE, LIFECYCLE_SHAM_FIXED_POOL, 0, 0) == 0xFF);
