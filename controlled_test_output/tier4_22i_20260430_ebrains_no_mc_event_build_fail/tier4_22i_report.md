...
| --- | --- | --- | --- |
| runner revision current | `tier4_22i_custom_runtime_roundtrip_20260430_0001` | `expected current source` | yes |
...
| runtime callback event shim exists | `CRA_MC_PACKET_EVENT` | `defines compatibility across Spin1API event macro names` | yes |
| runtime callback registration uses shim | `spin1_callback_on(CRA_MC_PACKET_EVENT` | `no direct brittle MC_PACKET_RX callback registration` | yes |
| runtime host stub catches brittle callback usage | `MCPL_PACKET_RECEIVED fallback` | `stub omits MC_PACKET_RX so direct use fails local syntax compile` | yes |
