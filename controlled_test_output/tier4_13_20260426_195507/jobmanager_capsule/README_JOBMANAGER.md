...
The runner also applies a narrow NumPy 2 compatibility shim for spinnman 7.4.x host-side memory-upload checksums. This does not change the CRA model or pass criteria; it only prevents valid 32-bit SpiNNaker words from being cast as scalar uint8 values during upload bookkeeping.

On failure, the capsule exports `seed_<seed>_failure_traceback.txt`, `seed_<seed>_backend_diagnostics.json`, and recent `reports/` directories so the next hardware blocker has a full stack/provenance trail.
