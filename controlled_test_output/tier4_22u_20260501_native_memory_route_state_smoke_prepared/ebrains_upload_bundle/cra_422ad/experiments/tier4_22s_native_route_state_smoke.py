#!/usr/bin/env python3
"""Tier 4.22s tiny native route-state custom-runtime smoke gate.

Tier 4.22r proved that the custom runtime can own keyed context state
and compute feature=context*cue on real SpiNNaker. Tier 4.22s takes the
next deliberately small step: the host writes keyed context and a chip-owned
route scalar, then asks the custom runtime to retrieve both and form
feature=context*route*cue on chip before scheduling delayed credit.
...
TIER = "Tier 4.22s - Tiny Native Route-State Custom-Runtime Smoke"
RUNNER_REVISION = "tier4_22s_native_route_state_smoke_20260501_0001"
TIER4_22R_LATEST = CONTROLLED / "tier4_22r_latest_manifest.json"
DEFAULT_OUTPUT = CONTROLLED / "tier4_22s_20260501_native_route_state_smoke_prepared"
UPLOAD_PACKAGE_NAME = "cra_422ab"
STABLE_EBRAINS_UPLOAD = ROOT / "ebrains_jobs" / UPLOAD_PACKAGE_NAME
DEPRECATED_EBRAINS_UPLOADS = [ROOT / "ebrains_jobs" / "cra_422ab_old"]
...
NATIVE_ROUTED_PLAN = [
    ("ctx_A", 1, 1, 1, "ctx_A_write_route_pos"),
    ("ctx_A", None, None, -1, "ctx_A_recall_route_pos"),
    ("ctx_B", -1, None, 1, "ctx_B_write_route_pos"),
    ("ctx_A", None, None, 1, "ctx_A_reentry_route_pos"),
    ("ctx_B", None, -1, -1, "ctx_B_recall_route_neg"),
    ("ctx_A", None, None, -1, "ctx_A_recall_route_neg"),
    ("ctx_C", 1, None, 1, "ctx_C_write_route_neg"),
    ("ctx_B", None, None, 1, "ctx_B_reentry_route_neg"),
    ("ctx_C", None, 1, -1, "ctx_C_recall_route_pos"),
    ("ctx_A", None, None, 1, "ctx_A_reentry_route_pos_again"),
]


def build_native_route_state_sequence() -> list[dict[str, Any]]:
    slots: dict[str, int] = {}
    ...
    for cycle in range(3):
        for key, context_update, route_update, cue, purpose in NATIVE_ROUTED_PLAN:
            if context_update is not None:
                slots[key] = int(context_update)
                context_writes += 1
            if route_update is not None:
                route_value = int(route_update)
                route_writes += 1
            cue_value = int(cue if cycle % 2 == 0 else -cue)
            context_value = int(slots.get(key, 1))
            context_reads += 1
            route_reads += 1
            feature = float(context_value * route_value * cue_value)
            sequence.append({
                "step": len(sequence) + 1,
                "feature": feature,
                "target": feature,
...
TASK_SEQUENCE = build_native_route_state_sequence()
...
def generate_task_reference(
    ...
    return {
        "status": "pass",
        "task": "native_route_state_signed_micro_task",
        "fixed_point": "s16.15",
        "equation": "chip feature=context_slot_lookup(key)*route_state*visible_cue; schedule prediction=w*feature+bias; keep pending across gap; mature oldest with target; w+=lr*(target-prediction)*feature; b+=lr*(target-prediction)",
        "native_context": {
            ...
            "feature_source": "chip_context_route_lookup_feature_transform",
        },
        "native_route": {
            "route_writes": int(max([int(row.get("native_route_writes") or 0) for row in rows], default=0)),
            "route_reads": int(max([int(row.get("native_route_reads") or 0) for row in rows], default=0)),
...
def write_reference_artifacts(output_dir: Path, reference: dict[str, Any]) -> dict[str, str]:
    reference_json = output_dir / "tier4_22s_task_reference.json"
    reference_csv = output_dir / "tier4_22s_task_reference_rows.csv"
...
def bridge_summary(reference: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "native_route_writes": route.get("route_writes"),
        "native_route_reads": route.get("route_reads"),
        "native_route_values": route.get("route_values"),
        "native_feature_source": context.get("feature_source") or route.get("feature_source"),
    }
...
def bridge_criteria(reference: dict[str, Any], *, label: str = "reference") -> list[dict[str, Any]]:
    ...
    return [
        ...
        criterion(f"{label} native route writes observed", route.get("route_writes"), "> 0", int(route.get("route_writes") or 0) > 0),
        criterion(f"{label} native route reads observed", route.get("route_reads"), "== 30", int(route.get("route_reads") or 0) == 30),
        ...
        criterion(
            ...
            context.get("feature_source") or route.get("feature_source"),
            "== chip_context_route_lookup_feature_transform",
            (context.get("feature_source") or route.get("feature_source")) == "chip_context_route_lookup_feature_transform",
        ),
    ]
...
def task_source_checks(
    ...
    *,
    ...
    checks.extend(
        [
            criterion(
                f"{label} native context and route command constants exist",
                "CMD_WRITE_CONTEXT/CMD_SCHEDULE_CONTEXT_PENDING/CMD_WRITE_ROUTE/CMD_SCHEDULE_ROUTED_CONTEXT_PENDING",
                "custom runtime must expose native context plus native route state primitives",
            ...
            criterion(
                f"{label} native route write/read handlers exist",
                "cra_state_write_route/cra_state_read_route",
                "route state must be owned by the C runtime, not a host-only host scalar",
                "cra_state_write_route(value, confidence, g_timestep)" in host_source and "cra_state_read_route(&route_value, &route_confidence)" in host_source,
            ),
            criterion(
                f"{label} native routed schedule forms feature on chip",
                "feature = FP_MUL(FP_MUL(context_value, route_value), cue)",
                "host must send key+cue, while the runtime retrieves context+route and computes the scalar feature",
                "feature = FP_MUL(FP_MUL(context_value, route_value), cue)" in host_source and "CMD_SCHEDULE_ROUTED_CONTEXT_PENDING" in controller_source,
            ),
            criterion(
...
def prepare_bundle(output_dir: Path) -> tuple[Path, str, dict[str, str]]:
    ...
    command = f"{UPLOAD_PACKAGE_NAME}/experiments/tier4_22s_native_route_state_smoke.py --mode run-hardware --output-dir tier4_22s_job_output"
    readme = bundle / "README_TIER4_22S_JOB.md"
    readme.write_text(
    ...
    try:
        ...
        for expected in expected_rows:
            ...
            if expected.get("native_route_update") is not None:
                ...
                time.sleep(float(args.command_delay_seconds))
            schedule = ctrl.schedule_routed_context_pending_decision(
                key=int(expected["native_context_key_id"]),
        ...
        final_route_state = ctrl.read_route(args.dest_x, args.dest_y, dest_cpu)
        observed_route_writes = max(
            [
                int_or((row.get("route_write") or {}).get("route_writes"), 0)
                for row in rows
        ...
        ok = (
            ...
            and bool(final_route_state.get("success"))
            and int(observed_route_writes) == int(reference.get("native_route", {}).get("route_writes") or -2)
            and int_or(final_route_state.get("route_reads"), -1) >= len(expected_rows)
        ...
        return {
            ...
            "observed_route_writes": int(observed_route_writes),
...
def write_report(path: Path, result: dict[str, Any]) -> None:
    ...
    for key in [
        "tier4_22r_status", "mode", "reference_status", "reference_sequence_length", "reference_accuracy", "reference_tail_accuracy",
        ...
        "native_route_writes", "native_route_reads", "native_route_values", "native_feature_source",
        "final_pending_created", "final_pending_matured", "final_reward_events", "final_decisions", "final_readout_weight", "final_readout_bias",
        "final_route_value", "final_route_writes", "final_route_reads",
...
def write_latest(output_dir: Path, manifest: Path, report: Path, status: str, mode: str) -> None:
    ...
    payload = {
        ...
        "report": str(report),
        "canonical": False,
        "claim": "Latest Tier 4.22s tiny native route-state custom-runtime micro-task; pass means a tiny native route-state pending-queue on-chip loop matched local fixed-point reference and task metrics only.",
    }
    write_json(CONTROLLED / "tier4_22s_latest_manifest.json", payload)


def finalize(output_dir: Path, result: dict[str, Any]) -> int:
    manifest = output_dir / "tier4_22s_results.json"
    report = output_dir / "tier4_22s_report.md"
...
def _base_local_result(args: argparse.Namespace, output_dir: Path) -> tuple[str, str | None, dict[str, Any], dict[str, str], dict[str, Any], list[dict[str, Any]]]:
    tier4_22r_status, tier4_22r_manifest = latest_status(TIER4_22R_LATEST)
    ...
    return tier4_22r_status, tier4_22r_manifest, reference, reference_artifacts, main_syntax, source_checks


def local(args: argparse.Namespace, output_dir: Path) -> int:
    tier4_22r_status, tier4_22r_manifest, reference, reference_artifacts, main_syntax, source_checks = _base_local_result(args, output_dir)
    metrics = reference.get("metrics", {})
    criteria = [
        criterion("Tier 4.22r native context-state smoke pass exists", tier4_22r_status, "== pass", tier4_22r_status == "pass"),
        ...
        *bridge_criteria(reference),
        criterion("reference final weight positive after native route stream", reference.get("final_readout_weight_raw"), "> 0", int(reference.get("final_readout_weight_raw") or 0) > 0),
    ...
    result = {
        ...
        "summary": {
            "mode": "local",
            "tier4_22r_status": tier4_22r_status,
            "tier4_22r_manifest": tier4_22r_manifest,
            ...
            **bridge_summary(reference),
            "next_step_if_passed": f"Prepare {UPLOAD_PACKAGE_NAME} and run the Tier 4.22s tiny native route-state micro-task on EBRAINS.",
        },
        "criteria": criteria,
        "reference": reference,
...
def run_hardware(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    env_path = output_dir / "tier4_22s_environment.json"
    target_path = output_dir / "tier4_22s_target_acquisition.json"
    load_path = output_dir / "tier4_22s_load_result.json"
    task_path = output_dir / "tier4_22s_task_micro_loop_result.json"
    task_csv = output_dir / "tier4_22s_task_micro_loop_rows.csv"
    ...
    write_csv(task_csv, [{k: v for k, v in row.items() if k not in {"context_write", "route_write", "schedule", "mature", "state_after_schedule", "state_after_mature"}} for row in task_result.get("rows", [])] if isinstance(task_result, dict) else [])

    final_state = task_result.get("final_state", {}) if isinstance(task_result, dict) else {}
    final_route_state = task_result.get("final_route_state", {}) if isinstance(task_result, dict) else {}
    observed_route_writes = int_or(task_result.get("observed_route_writes"), -1) if isinstance(task_result, dict) else -1
    ...
    criteria = [
        ...
        criterion("Tier 4.22r native context-state smoke pass exists or fresh bundle", tier4_22r_status, "== pass OR missing in fresh EBRAINS bundle", tier4_22r_status in {"pass", "missing"}),
        criterion("local task fixed-point reference generated", reference.get("status"), "== pass", reference.get("status") == "pass"),
        *bridge_criteria(reference),
        ...
        *_source_checks_for_root(ROOT, label="runtime"),
        ...
        criterion("all route writes succeeded", [row.get("route_write_success") for row in rows if row.get("native_route_update") is not None], "all True", bool(rows) and all(row.get("route_write_success") for row in rows)),
        ...
        criterion("chip-retrieved route values match local reference", [row.get("route_value_raw_delta") for row in rows], f"abs(delta) <= {tolerance}", bool(rows) and all(abs(int(row.get("route_value_raw_delta", 10**12))) <= tolerance for row in rows)),
        criterion("chip-retrieved route confidence matches local reference", [row.get("route_confidence_raw_delta") for row in rows], f"abs(delta) <= {tolerance}", bool(rows) and all(abs(int(row.get("route_confidence_raw_delta", 10**12))) <= tolerance for row in rows)),
        ...
        criterion("route readback final succeeds", final_route_state.get("success"), "== True", bool(final_route_state.get("success"))),
        criterion("observed route writes final", observed_route_writes, f"== {reference.get('native_route', {}).get('route_writes')}", observed_route_writes == int(reference.get("native_route", {}).get("route_writes") or -2)),
        criterion("route reads final", final_route_state.get("route_reads"), f">= {reference.get('sequence_length')}", int_or(final_route_state.get("route_reads"), -1) >= int(reference.get("sequence_length") or 10**9)),
    ...
    result = {
        ...
        "summary": {
            ...
            **bridge_summary(reference),
            ...
            "final_route_value": final_route_state.get("route_value"),
            "final_route_writes": observed_route_writes,
            "final_route_reads": final_route_state.get("route_reads"),
            "claim_boundary": "Tiny native route-state primitive layered on native keyed context only; not full native/on-chip v2 memory/routing, full CRA task learning, speedup evidence, scaling, or final autonomy.",
            "next_step_if_passed": "Ingest returned Tier 4.22s files; if it passes, continue with the next native custom-runtime integration gate, likely bounded memory/routing composition.",
        },
        "criteria": criteria,
        "reference": reference,
...
def _observed_route_writes_from_rows(rows: list[dict[str, Any]]) -> int:
    return max(
        [
            int_or((row.get("route_write") or {}).get("route_writes"), 0)
            for row in rows
...
def _is_route_writes_false_fail(raw_manifest: dict[str, Any]) -> tuple[bool, str, int]:
    """Detect the first 4.22s returned-run bookkeeping false fail.

    The original 4.22s runner incorrectly required route_writes to appear in
    the final CMD_READ_ROUTE reply. The protocol intentionally returns
    value/confidence/read count there; route write counters are already present
    in the acknowledged CMD_WRITE_ROUTE rows. This correction is only valid
    when all real hardware/task parity checks passed and the only failures are
    the top-level task status plus that bad criterion.
    """
    ...
    allowed = {"minimal task micro-loop pass", "route writes final"}
    ...
    expected_route_writes = int(reference.get("native_route", {}).get("route_writes") or -1)
    observed_route_writes = _observed_route_writes_from_rows(rows)
    final_state = task.get("final_state", {}) if isinstance(task.get("final_state"), dict) else {}
    final_route_state = task.get("final_route_state", {}) if isinstance(task.get("final_route_state"), dict) else {}
    ...
    checks = [
        ...
        observed_route_writes == expected_route_writes,
        bool(final_route_state.get("success")),
        int_or(final_route_state.get("route_reads"), -1) >= int(reference.get("sequence_length") or 10**9),
    ...
    if all(checks):
        return True, "corrected route_writes final false fail; route writes are proven by CMD_WRITE_ROUTE row counters", observed_route_writes
    return False, "route-write false-fail correction checks did not all pass", observed_route_writes
...
def ingest(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    raw_manifest_path = source / "tier4_22s_results.json"
    if not raw_manifest_path.exists():
        raise SystemExit(f"missing tier4_22s_results.json in ingest dir: {source}")
    raw_manifest = read_json(raw_manifest_path)

    artifacts: dict[str, str] = {}
    artifacts["raw_remote_manifest_json"] = _copy_latest(source, output_dir, "tier4_22s_results.json", target_name="remote_tier4_22s_results_raw.json")
    artifacts["raw_remote_report_md"] = _copy_latest(source, output_dir, "tier4_22s_report.md", target_name="remote_tier4_22s_report_raw.md")
    exact_artifact_names = {
        "tier4_22s_environment.json": "environment_json",
        "tier4_22s_target_acquisition.json": "target_acquisition_json",
        "tier4_22s_load_result.json": "load_result_json",
        "tier4_22s_task_micro_loop_result.json": "task_micro_loop_result_json",
        "tier4_22s_task_micro_loop_rows.csv": "task_micro_loop_rows_csv",
        "tier4_22s_task_reference.json": "reference_json",
        "tier4_22s_task_reference_rows.csv": "reference_csv",
        "tier4_22s_latest_manifest.json": "remote_latest_manifest_json",
    }
    ...
    false_fail_corrected, correction_note, observed_route_writes = _is_route_writes_false_fail(raw_manifest)
    ...
    if false_fail_corrected:
        ...
        for item in ingested.get("criteria", []):
            ...
            if item.get("name") == "route writes final" and not item.get("passed"):
                item["passed"] = True
                item["value"] = observed_route_writes
                item["note"] = correction_note
    ...
    ingested["summary"].update(
        {
            ...
            "observed_route_writes": observed_route_writes if observed_route_writes >= 0 else None,
            "final_route_writes": observed_route_writes if false_fail_corrected else raw_manifest.get("summary", {}).get("final_route_writes"),
            ...
            "claim_boundary": "Ingested returned EBRAINS artifacts; a pass is a tiny signed native route-state custom-runtime micro-task only, not full CRA task learning or speedup evidence.",
        }
    )
    ...
    if false_fail_corrected:
        ingested.setdefault("criteria", []).append(
            criterion(
                "route writes false-fail correction valid",
                observed_route_writes,
                "observed CMD_WRITE_ROUTE counter == expected route writes and all real task parity checks passed",
                True,
                correction_note,
            )
        )
...
def main() -> int:
    ...
    default = DEFAULT_OUTPUT if args.mode == "prepare" else CONTROLLED / f"tier4_22s_{stamp}_{args.mode.replace('-', '_')}"
