...
def preflight_criteria(args: argparse.Namespace, mode: str) -> list[dict[str, Any]]:
    ...
    return [
        ...
        criterion("Tier 4.20a transfer audit available", {"status": audit_status, "manifest": audit_manifest}, "status == pass", audit_status == "pass"),
...
def write_latest(output_dir: Path, report_path: Path, manifest_path: Path, status: str) -> None:
    write_json(
        CONTROLLED / "tier4_20b_latest_manifest.json",
        {
            ...
            "status": status,
            "canonical": False,
            "claim": "Latest Tier 4.20b v2.1 one-seed chunked hardware probe; promote only after returned hardware review.",
        },
    )
...
def run_hardware(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    criteria.extend(
        [
            criterion("child Tier 4.16 command exited cleanly", proc.returncode, "== 0", proc.returncode == 0),
    ...
    result = {
        ...
        "summary": {
            ...
            "child_return_code": proc.returncode,
            **child_summary,
        },
        "criteria": criteria,
        "artifacts": artifacts,
