...
evidence suite is **28 entries total**:
...
+ 1 circuit motif causality suite = 28
```
...
Tier 5.12a, Tier 5.12c, Tier 5.12d, Tier 6.1, Tier 6.3, and
Tier 6.4 have been implemented and run. Tier 4.12 proves NEST/Brian2 parity plus SpiNNaker
...
Tier 4.20b is now prepared as that one-seed probe at
`controlled_test_output/tier4_20b_20260429_205214_prepared/`. The prepared
JobManager capsule delegates real SpiNNaker execution to the proven Tier 4.16
chunked-host runner and records the v2.1 bridge profile explicitly. It is not a
hardware pass until returned `run-hardware` artifacts show real spike readback,
zero fallback, zero `sim.run` failures, and zero readback failures.
...
- `tier4_20b_v2_1_hardware_probe.py`: Tier 4.20b prepared one-seed v2.1 chunked SpiNNaker bridge probe; prepares/runs/ingests a JobManager capsule, delegates low-level hardware execution to the proven Tier 4.16 chunked-host runner, and keeps macro eligibility excluded.
