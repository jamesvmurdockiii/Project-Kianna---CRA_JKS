...
def check_registry() -> tuple[Check, dict[str, Any]]:
    ...
    if registry.get("evidence_count") != 26:
        failures.append(f"evidence_count={registry.get('evidence_count')} expected 26")
    if registry.get("expanded_test_entry_count") != 28:
        failures.append(
            f"expanded_test_entry_count={registry.get('expanded_test_entry_count')} expected 28"
        )
    ...
    return Check(
        ...
        passed=not failures,
        details="; ".join(failures) if failures else "26 canonical bundles, 28 entries, 0 missing artifacts, 0 failed criteria",
    ), registry
...
def build_report(checks: list[Check], registry: dict[str, Any]) -> str:
    ...
    lines.extend(
        [
            ...
            "- Tier 4.20b is prepared hardware-probe evidence only; the JobManager capsule is ready for a one-seed v2.1 chunked-host bridge probe, but it is not hardware evidence until returned run-hardware artifacts pass, and even then it is not full native/on-chip v2.1 execution.",
