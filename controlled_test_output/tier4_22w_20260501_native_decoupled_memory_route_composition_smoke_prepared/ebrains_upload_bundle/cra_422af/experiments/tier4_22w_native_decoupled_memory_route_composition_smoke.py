#!/usr/bin/env python3
"""Tier 4.22w tiny native decoupled memory-route composition custom-runtime smoke gate.

Tier 4.22u proved that the custom runtime can own keyed context, keyed route
slots, and keyed memory slots and compute
feature=context[key]*route[key]*memory[key]*cue on real SpiNNaker.
Tier 4.22w adds an explicit decoupled-key schedule command. The host can now send context_key, route_key, memory_key, cue, and delay; the runtime must retrieve independently keyed context, route, and memory slots and produce feature=context[context_key]*route[route_key]*memory[memory_key]*cue on chip before delayed credit.
...
TIER = "Tier 4.22w - Tiny Native Decoupled Memory-Route Composition Custom-Runtime Smoke"
RUNNER_REVISION = "tier4_22w_native_decoupled_memory_route_composition_smoke_20260501_0001"
TIER4_22V_LATEST = CONTROLLED / "tier4_22v_latest_manifest.json"
DEFAULT_OUTPUT = CONTROLLED / "tier4_22w_20260501_native_decoupled_memory_route_composition_smoke_prepared"
UPLOAD_PACKAGE_NAME = "cra_422af"
STABLE_EBRAINS_UPLOAD = ROOT / "ebrains_jobs" / UPLOAD_PACKAGE_NAME
DEPRECATED_EBRAINS_UPLOADS = [ROOT / "ebrains_jobs" / "cra_422af_old"]
...
DECOUPLED_MEMORY_ROUTED_PLAN = [
    ("ctx_A", "route_A", "mem_A", 1, 1, 1, 1, "seed_AAA"),
    ("ctx_B", "route_A", "mem_B", -1, None, -1, 1, "seed_BAB"),
    ("ctx_C", "route_B", "mem_A", 1, -1, None, -1, "seed_CBA"),
    ("ctx_D", "route_C", "mem_C", -1, 1, 1, -1, "seed_DCC"),
    ("ctx_A", "route_B", "mem_C", None, None, None, 1, "compose_ABC"),
    ("ctx_C", "route_A", "mem_B", None, None, None, -1, "compose_CAB"),
    ("ctx_B", "route_C", "mem_A", None, None, None, 1, "compose_BCA"),
    ("ctx_D", "route_B", "mem_B", None, None, None, -1, "compose_DBB"),
    ("ctx_A", "route_D", "mem_D", None, -1, -1, 1, "introduce_route_memory_D"),
    ("ctx_D", "route_D", "mem_A", None, None, None, 1, "compose_DDA"),
    ("ctx_B", "route_A", "mem_D", None, None, None, -1, "compose_BAD"),
    ("ctx_C", "route_C", "mem_C", None, None, None, 1, "compose_CCC"),
    ("ctx_A", "route_C", "mem_B", -1, None, 1, -1, "context_A_memory_B_flip"),
    ("ctx_B", "route_D", "mem_C", None, 1, None, 1, "route_D_flip"),
    ("ctx_C", "route_B", "mem_D", None, None, 1, -1, "memory_D_flip"),
    ("ctx_D", "route_A", "mem_A", 1, None, None, 1, "context_D_flip_reentry"),
]


def build_native_decoupled_memory_route_composition_sequence() -> list[dict[str, Any]]:
    slots: dict[str, int] = {}
    ...
    for cycle in range(3):
        for context_key, route_key, memory_key, context_update, route_update, memory_update, cue, purpose in DECOUPLED_MEMORY_ROUTED_PLAN:
            if context_update is not None:
                slots[context_key] = int(context_update)
                context_writes += 1
            if route_update is not None:
                route_slots[route_key] = int(route_update)
                route_writes += 1
                max_route_slot_count = max(max_route_slot_count, len(route_slots))
            if memory_update is not None:
                memory_slots[memory_key] = int(memory_update)
                memory_writes += 1
                max_memory_slot_count = max(max_memory_slot_count, len(memory_slots))
            cue_value = int(cue if cycle % 2 == 0 else -cue)
            context_value = int(slots.get(context_key, 1))
            context_reads += 1
            route_value = int(route_slots.get(route_key, 1))
            route_reads += 1
            memory_value = int(memory_slots.get(memory_key, 1))
            memory_reads += 1
            ...
            sequence.append({
                ...
                "native_context_key_id": CONTEXT_KEY_IDS[context_key],
                "native_context_update": context_update,
                "native_context_value": context_value,
                "native_context_confidence": 1.0,
                "native_route_key": route_key,
                "native_route_key_id": ROUTE_KEY_IDS[route_key],
                "native_route_update": route_update,
                "native_route_value": route_value,
                "native_route_confidence": 1.0,
                "native_memory_key": memory_key,
                "native_memory_key_id": MEMORY_KEY_IDS[memory_key],
...
TASK_SEQUENCE = build_native_decoupled_memory_route_composition_sequence()
...
def generate_task_reference(
    ...
    for event in sequence or TASK_SEQUENCE:
        ...
        row = {
            ...
            "native_route_key": event.get("native_route_key"),
            "native_route_key_id": event.get("native_route_key_id"),
            ...
            "native_memory_key": event.get("native_memory_key"),
            "native_memory_key_id": event.get("native_memory_key_id"),
    ...
    return {
        ...
        "native_route": {
            "route_keys": sorted({str(row.get("native_route_key")) for row in rows if row.get("native_route_key") is not None}),
            "route_key_ids": sorted({int(row.get("native_route_key_id")) for row in rows if row.get("native_route_key_id") is not None}),
        ...
        "native_memory": {
            "memory_keys": sorted({str(row.get("native_memory_key")) for row in rows if row.get("native_memory_key") is not None}),
            "memory_key_ids": sorted({int(row.get("native_memory_key_id")) for row in rows if row.get("native_memory_key_id") is not None}),
...
def write_reference_artifacts(output_dir: Path, reference: dict[str, Any]) -> dict[str, str]:
    reference_json = output_dir / "tier4_22w_task_reference.json"
    reference_csv = output_dir / "tier4_22w_task_reference_rows.csv"
...
def bridge_summary(reference: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "native_route_keys": route.get("route_keys"),
        "native_route_key_ids": route.get("route_key_ids"),
        ...
        "native_memory_keys": memory.get("memory_keys"),
        "native_memory_key_ids": memory.get("memory_key_ids"),
...
def prepare_bundle(output_dir: Path) -> tuple[Path, str, dict[str, str]]:
    ...
    for script in [
        ...
        "tier4_22v_native_memory_route_reentry_composition_smoke.py",
    ...
    command = f"{UPLOAD_PACKAGE_NAME}/experiments/tier4_22w_native_decoupled_memory_route_composition_smoke.py --mode run-hardware --output-dir tier4_22w_job_output"
    readme = bundle / "README_TIER4_22W_JOB.md"
    readme.write_text(
...
def task_micro_loop(hostname: str, args: argparse.Namespace, *, dest_cpu: int, reference: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        for expected in expected_rows:
            ...
            if expected.get("native_route_update") is not None:
                route_write = ctrl.write_route_slot(
                    key=int(expected["native_route_key_id"]),
            ...
            if expected.get("native_memory_update") is not None:
                memory_write = ctrl.write_memory_slot(
                    key=int(expected["native_memory_key_id"]),
            ...
            schedule = ctrl.schedule_decoupled_memory_route_context_pending_decision(
                context_key=int(expected["native_context_key_id"]),
                route_key=int(expected["native_route_key_id"]),
                memory_key=int(expected["native_memory_key_id"]),
            ...
            row = {
                ...
                "native_route_key": expected.get("native_route_key"),
                "native_route_key_id": expected.get("native_route_key_id"),
                "native_route_update": expected.get("native_route_update"),
                "native_route_value": expected.get("native_route_value"),
                "native_memory_key": expected.get("native_memory_key"),
                "native_memory_key_id": expected.get("native_memory_key_id"),
                ...
                "keyed_context_key_match": int_or(schedule.get("keyed_context_key"), -1) == int(expected["native_context_key_id"]),
                "observed_keyed_route_key": schedule.get("keyed_route_key"),
                "keyed_route_key_match": int_or(schedule.get("keyed_route_key"), -1) == int(expected["native_route_key_id"]),
                "observed_keyed_memory_key": schedule.get("keyed_memory_key"),
                "keyed_memory_key_match": int_or(schedule.get("keyed_memory_key"), -1) == int(expected["native_memory_key_id"]),
        ...
        route_key_ids = sorted({int(row["native_route_key_id"]) for row in expected_rows})
        memory_key_ids = sorted({int(row["native_memory_key_id"]) for row in expected_rows})
...
def write_report(path: Path, result: dict[str, Any]) -> None:
    ...
    for key in [
        "tier4_22v_status", "mode", "reference_status", "reference_sequence_length", "reference_accuracy", "reference_tail_accuracy",
        ...
        "native_route_keys", "native_route_key_ids", "native_route_slot_writes", "native_route_slot_reads", "native_route_max_slot_count", "native_route_values", "native_feature_source",
        "native_memory_keys", "native_memory_key_ids", "native_memory_slot_writes", "native_memory_slot_reads", "native_memory_max_slot_count", "native_memory_values",
...
def write_latest(output_dir: Path, manifest: Path, report: Path, status: str, mode: str) -> None:
    ...
    payload = {
        ...
        "report": str(report),
        "canonical": False,
        "claim": "Latest Tier 4.22w tiny native decoupled memory-route composition custom-runtime micro-task; pass means a tiny native decoupled memory-route composition pending-queue on-chip loop matched local fixed-point reference and task metrics only.",
    }
    write_json(CONTROLLED / "tier4_22w_latest_manifest.json", payload)


def finalize(output_dir: Path, result: dict[str, Any]) -> int:
    manifest = output_dir / "tier4_22w_results.json"
    report = output_dir / "tier4_22w_report.md"
...
def _base_local_result(args: argparse.Namespace, output_dir: Path) -> tuple[str, str | None, dict[str, Any], dict[str, str], dict[str, Any], list[dict[str, Any]]]:
    tier4_22v_status, tier4_22v_manifest = latest_status(TIER4_22V_LATEST)
    ...
    return tier4_22v_status, tier4_22v_manifest, reference, reference_artifacts, main_syntax, source_checks


def local(args: argparse.Namespace, output_dir: Path) -> int:
    tier4_22v_status, tier4_22v_manifest, reference, reference_artifacts, main_syntax, source_checks = _base_local_result(args, output_dir)
    metrics = reference.get("metrics", {})
    criteria = [
        criterion("Tier 4.22v native memory-route reentry/composition smoke pass exists", tier4_22v_status, "== pass", tier4_22v_status == "pass"),
    ...
    result = {
        ...
        "summary": {
            "mode": "local",
            "tier4_22v_status": tier4_22v_status,
            "tier4_22v_manifest": tier4_22v_manifest,
            ...
            **bridge_summary(reference),
            "next_step_if_passed": f"Prepare {UPLOAD_PACKAGE_NAME} and run the Tier 4.22w tiny native decoupled memory-route composition micro-task on EBRAINS.",
        },
        "criteria": criteria,
        "reference": reference,
...
def run_hardware(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    env_path = output_dir / "tier4_22w_environment.json"
    target_path = output_dir / "tier4_22w_target_acquisition.json"
    load_path = output_dir / "tier4_22w_load_result.json"
    task_path = output_dir / "tier4_22w_task_micro_loop_result.json"
    task_csv = output_dir / "tier4_22w_task_micro_loop_rows.csv"
    ...
    criteria = [
        ...
        criterion("Tier 4.22v native memory-route reentry/composition smoke pass exists or fresh bundle", tier4_22v_status, "== pass OR missing in fresh EBRAINS bundle", tier4_22v_status in {"pass", "missing"}),
        criterion("local task fixed-point reference generated", reference.get("status"), "== pass", reference.get("status") == "pass"),
        *bridge_criteria(reference),
...
def _is_route_slot_writes_false_fail(raw_manifest: dict[str, Any]) -> tuple[bool, str, int]:
    ...
    expected_memory_slot_writes = int(reference.get("native_memory", {}).get("memory_slot_writes") or -1)
...
def ingest(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    raw_manifest_path = source / "tier4_22w_results.json"
    if not raw_manifest_path.exists():
        raise SystemExit(f"missing tier4_22w_results.json in ingest dir: {source}")
    raw_manifest = read_json(raw_manifest_path)

    artifacts: dict[str, str] = {}
    artifacts["raw_remote_manifest_json"] = _copy_latest(source, output_dir, "tier4_22w_results.json", target_name="remote_tier4_22w_results_raw.json")
    artifacts["raw_remote_report_md"] = _copy_latest(source, output_dir, "tier4_22w_report.md", target_name="remote_tier4_22w_report_raw.md")
    exact_artifact_names = {
        "tier4_22w_environment.json": "environment_json",
        "tier4_22w_target_acquisition.json": "target_acquisition_json",
        "tier4_22w_load_result.json": "load_result_json",
        "tier4_22w_task_micro_loop_result.json": "task_micro_loop_result_json",
        "tier4_22w_task_micro_loop_rows.csv": "task_micro_loop_rows_csv",
        "tier4_22w_task_reference.json": "reference_json",
        "tier4_22w_task_reference_rows.csv": "reference_csv",
        "tier4_22w_latest_manifest.json": "remote_latest_manifest_json",
    }
...
def main() -> int:
    ...
    default = DEFAULT_OUTPUT if args.mode == "prepare" else CONTROLLED / f"tier4_22w_{stamp}_{args.mode.replace('-', '_')}"
