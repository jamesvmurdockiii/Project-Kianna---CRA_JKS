...
Tier 4.22q is locally passed and EBRAINS-prepared as the next tiny bridge
smoke. It does not add new wire commands; instead, the host runner uses a
keyed-context plus route-state transform to generate a 30-event signed stream,
then sends that stream through the existing `CMD_SCHEDULE_PENDING` and
`CMD_MATURE_PENDING` surface. The local reference has context/route updates
...
final autonomy until returned EBRAINS artifacts pass and are ingested.
