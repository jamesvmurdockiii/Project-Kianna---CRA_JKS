#!/usr/bin/env python3
"""Tier 4.22t tiny native keyed route-state custom-runtime smoke gate.

Tier 4.22s proved that the custom runtime can own keyed context plus a global
route scalar and compute feature=context*route*cue on real SpiNNaker. Tier
4.22t takes the next deliberately small step: the host writes keyed context and
keyed route slots, then asks the custom runtime to retrieve both by key and form
feature=context[key]*route[key]*cue on chip before scheduling delayed credit.
...
TIER = "Tier 4.22t - Tiny Native Keyed Route-State Custom-Runtime Smoke"
RUNNER_REVISION = "tier4_22t_native_keyed_route_state_smoke_20260501_0001"
TIER4_22S_LATEST = CONTROLLED / "tier4_22s_latest_manifest.json"
DEFAULT_OUTPUT = CONTROLLED / "tier4_22t_20260501_native_keyed_route_state_smoke_prepared"
UPLOAD_PACKAGE_NAME = "cra_422ac"
STABLE_EBRAINS_UPLOAD = ROOT / "ebrains_jobs" / UPLOAD_PACKAGE_NAME
DEPRECATED_EBRAINS_UPLOADS = [ROOT / "ebrains_jobs" / "cra_422ac_old"]
...
def build_native_keyed_route_state_sequence() -> list[dict[str, Any]]:
    slots: dict[str, int] = {}
...
TASK_SEQUENCE = build_native_keyed_route_state_sequence()
...
def generate_task_reference(
    ...
    return {
        "status": "pass",
        "task": "native_keyed_route_state_signed_micro_task",
        "fixed_point": "s16.15",
        "equation": "chip feature=context_slot_lookup(key)*route_slot_lookup(key)*visible_cue; schedule prediction=w*feature+bias; keep pending across gap; mature oldest with target; w+=lr*(target-prediction)*feature; b+=lr*(target-prediction)",
        "native_context": {
            ...
            "feature_source": "chip_context_keyed_route_lookup_feature_transform",
        },
        "native_route": {
...
def write_reference_artifacts(output_dir: Path, reference: dict[str, Any]) -> dict[str, str]:
    reference_json = output_dir / "tier4_22t_task_reference.json"
    reference_csv = output_dir / "tier4_22t_task_reference_rows.csv"
...
def bridge_criteria(reference: dict[str, Any], *, label: str = "reference") -> list[dict[str, Any]]:
    ...
    return [
        ...
        criterion(
            ...
            "== chip_context_keyed_route_lookup_feature_transform",
            (context.get("feature_source") or route.get("feature_source")) == "chip_context_keyed_route_lookup_feature_transform",
        ),
    ]
...
def task_source_checks(
    ...
    *,
    ...
    checks.extend(
        [
            criterion(
                f"{label} native context and keyed-route command constants exist",
                "CMD_WRITE_CONTEXT/CMD_WRITE_ROUTE_SLOT/CMD_SCHEDULE_KEYED_ROUTE_CONTEXT_PENDING",
                "custom runtime must expose native context plus keyed route-state primitives",
            ...
            criterion(
                f"{label} native keyed routed schedule forms feature on chip",
                "feature = FP_MUL(FP_MUL(context_value, route_value), cue)",
                "host must send key+cue, while the runtime retrieves context+route slot by key and computes the scalar feature",
                "feature = FP_MUL(FP_MUL(context_value, route_value), cue)" in host_source and "CMD_SCHEDULE_KEYED_ROUTE_CONTEXT_PENDING" in controller_source,
            ),
            criterion(
...
def prepare_bundle(output_dir: Path) -> tuple[Path, str, dict[str, str]]:
    ...
    command = f"{UPLOAD_PACKAGE_NAME}/experiments/tier4_22t_native_keyed_route_state_smoke.py --mode run-hardware --output-dir tier4_22t_job_output"
    readme = bundle / "README_TIER4_22T_JOB.md"
    readme.write_text(
    ...
    try:
        ...
        for expected in expected_rows:
            ...
            if expected.get("native_route_update") is not None:
                ...
                time.sleep(float(args.command_delay_seconds))
            schedule = ctrl.schedule_keyed_route_context_pending_decision(
                key=int(expected["native_context_key_id"]),
            ...
            row = {
                ...
                "keyed_route_key_match": int_or(schedule.get("keyed_route_key"), -1) == int(expected["native_context_key_id"]),
...
def write_latest(output_dir: Path, manifest: Path, report: Path, status: str, mode: str) -> None:
    ...
    payload = {
        ...
        "report": str(report),
        "canonical": False,
        "claim": "Latest Tier 4.22t tiny native keyed route-state custom-runtime micro-task; pass means a tiny native keyed route-state pending-queue on-chip loop matched local fixed-point reference and task metrics only.",
    }
    write_json(CONTROLLED / "tier4_22t_latest_manifest.json", payload)


def finalize(output_dir: Path, result: dict[str, Any]) -> int:
    manifest = output_dir / "tier4_22t_results.json"
    report = output_dir / "tier4_22t_report.md"
...
def _base_local_result(args: argparse.Namespace, output_dir: Path) -> tuple[str, str | None, dict[str, Any], dict[str, str], dict[str, Any], list[dict[str, Any]]]:
    tier4_22r_status, tier4_22r_manifest = latest_status(TIER4_22S_LATEST)
    ...
    result = {
        ...
        "summary": {
            ...
            **bridge_summary(reference),
            "next_step_if_passed": f"Prepare {UPLOAD_PACKAGE_NAME} and run the Tier 4.22t tiny native keyed route-state micro-task on EBRAINS.",
        },
        "criteria": criteria,
        "reference": reference,
...
def run_hardware(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    env_path = output_dir / "tier4_22t_environment.json"
    target_path = output_dir / "tier4_22t_target_acquisition.json"
    load_path = output_dir / "tier4_22t_load_result.json"
    task_path = output_dir / "tier4_22t_task_micro_loop_result.json"
    task_csv = output_dir / "tier4_22t_task_micro_loop_rows.csv"
    ...
    result = {
        ...
        "summary": {
            ...
            **bridge_summary(reference),
            ...
            "claim_boundary": "Tiny native keyed keyed route-state primitive layered on native keyed context only; not full native/on-chip v2 memory/routing, full CRA task learning, speedup evidence, scaling, or final autonomy.",
...
def _is_route_slot_writes_false_fail(raw_manifest: dict[str, Any]) -> tuple[bool, str, int]:
    """Detect the first 4.22t returned-run bookkeeping false fail.
...
def ingest(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    raw_manifest_path = source / "tier4_22t_results.json"
    if not raw_manifest_path.exists():
        raise SystemExit(f"missing tier4_22t_results.json in ingest dir: {source}")
    raw_manifest = read_json(raw_manifest_path)

    artifacts: dict[str, str] = {}
    artifacts["raw_remote_manifest_json"] = _copy_latest(source, output_dir, "tier4_22t_results.json", target_name="remote_tier4_22t_results_raw.json")
    artifacts["raw_remote_report_md"] = _copy_latest(source, output_dir, "tier4_22t_report.md", target_name="remote_tier4_22t_report_raw.md")
    exact_artifact_names = {
        "tier4_22t_environment.json": "environment_json",
        "tier4_22t_target_acquisition.json": "target_acquisition_json",
        "tier4_22t_load_result.json": "load_result_json",
        "tier4_22t_task_micro_loop_result.json": "task_micro_loop_result_json",
        "tier4_22t_task_micro_loop_rows.csv": "task_micro_loop_rows_csv",
        "tier4_22t_task_reference.json": "reference_json",
        "tier4_22t_task_reference_rows.csv": "reference_csv",
        "tier4_22t_latest_manifest.json": "remote_latest_manifest_json",
    }
    ...
    ingested["summary"].update(
        {
            ...
            "claim_boundary": "Ingested returned EBRAINS artifacts; a pass is a tiny signed native keyed route-state custom-runtime micro-task only, not full CRA task learning or speedup evidence.",
        }
    )
...
def main() -> int:
    ...
    default = DEFAULT_OUTPUT if args.mode == "prepare" else CONTROLLED / f"tier4_22t_{stamp}_{args.mode.replace('-', '_')}"
