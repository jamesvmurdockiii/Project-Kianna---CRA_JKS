...
Correction: raw EBRAINS wrapper reported `FAIL` only because the fresh source bundle did not include `controlled_test_output/tier4_20b_latest_manifest.json`. The child Tier 4.16 hardware repeat passed all six task/seed runs with zero fallback, zero `sim.run` failures, zero summary/readback failures, and nonzero real spike readback.
...
The raw returned EBRAINS manifest is preserved under `raw_hardware_artifacts/` and `../tier4_20c_20260430_v2_1_bridge_three_seed_raw_false_fail/`. The corrected PASS does not hide the wrapper issue; it reclassifies the run because every child hardware criterion passed and the only failing criterion was a non-portable local-prerequisite check.
