...
[![Evidence](https://img.shields.io/badge/canonical%20evidence-45%20bundles-blue.svg)](STUDY_EVIDENCE_INDEX.md)
...
| Latest software benchmark diagnostic | Tier 7.0b localized the Tier 7.0 standard-benchmark gap to a recoverable state-signal/default-readout failure; CRA state probes improved substantially over raw CRA output. |
| Active next gate | Tier 7.0c bounded continuous readout/interface repair: use the diagnosed state signal without leakage, with ablations and baseline comparison, before any hardware migration. |
| Canonical registry | 45 evidence bundles, 0 missing expected artifacts, 0 failed criteria. |
...
| 7.0-7.0b | Standard dynamical benchmarks and failure analysis: Mackey-Glass, Lorenz, NARMA10, aggregate geometric-mean MSE, and CRA state/readout probes. | Software diagnostics only; CRA v2.1 underperformed simple continuous-regression sequence baselines, and 7.0b localized the gap to readout/interface rather than proving a promoted repair. |
