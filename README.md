...
| Active next gate | Tier 4.32g two-chip lifecycle traffic/resource hardware smoke is prepared from [`controlled_test_output/tier4_32g_20260507_prepared`](controlled_test_output/tier4_32g_20260507_prepared) with `16/16` prepare criteria and upload folder [`ebrains_jobs/cra_432g`](ebrains_jobs/cra_432g). Run exactly `cra_432g/experiments/tier4_32g_multichip_lifecycle_traffic_resource_smoke.py --mode run-hardware --output-dir tier4_32g_job_output` in EBRAINS JobManager, then ingest returned artifacts. Speedup, benchmark, lifecycle-scaling, true two-partition, multi-shard, and native-scale baseline-freeze claims remain blocked. |
