...
| Latest 4.32g hardware return | First Tier 4.32g EBRAINS return was ingested at [`controlled_test_output/tier4_32g_20260507_hardware_fail_ingested`](controlled_test_output/tier4_32g_20260507_hardware_fail_ingested). A second attempted rerun was ingested at [`controlled_test_output/tier4_32g_20260508_old_package_return_ingested`](controlled_test_output/tier4_32g_20260508_old_package_return_ingested) and showed the same runner revision `...0001`, proving EBRAINS ran the stale `cra_432g` package rather than the repaired r1 package. Both strict gates failed, but the mechanism path succeeded: source event request `1`, source trophic request `1`, source mask sync received `1`, expected active mask `1`, lifecycle event ACKs `2`, lifecycle mask sync sent `1`, death count `1`, trophic update count `1`, zero stale/duplicate/missing-ack counters, and zero synthetic fallback. Failure class: stale package plus cleanup/control-surface issue in rev-0001. |
| Active next gate | Tier 4.32g-r2 two-chip lifecycle traffic/resource hardware smoke is prepared from [`controlled_test_output/tier4_32g_20260508_r2_prepared`](controlled_test_output/tier4_32g_20260508_r2_prepared) with `16/16` prepare criteria and cache-proof upload folder [`ebrains_jobs/cra_432g_r2`](ebrains_jobs/cra_432g_r2). Run exactly `cra_432g_r2/experiments/tier4_32g_multichip_lifecycle_traffic_resource_smoke.py --mode run-hardware --output-dir tier4_32g_job_output` in EBRAINS JobManager, then ingest returned artifacts. Speedup, benchmark, lifecycle-scaling, true two-partition, multi-shard, and native-scale baseline-freeze claims remain blocked. |
