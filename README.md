...
| Active next gate | Tier 7.7r native temporal-basis repair/reframing contract: split 7.7q into two honest paths: bounded engineering/interface promotion if the temporal basis reliably helps without regressions, versus stricter CRA-specific mechanism promotion only if it beats/separates from random-projection and nonlinear-lag controls. |
| Canonical registry | 140 evidence bundles, 0 missing expected artifacts, 0 failed criteria. |
