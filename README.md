...
| Latest prepared EBRAINS package | No new EBRAINS package is pending after Tier 4.30f ingest. The next package should be created only after the Tier 4.30g contract/local gate is defined and passes. |
| Active next gate | Define Tier 4.30g as a compact native lifecycle task-benefit/resource bridge: enabled lifecycle versus predeclared controls on a small task-bearing hardware capsule, with resource/readback accounting and no baseline freeze unless task-effect evidence passes. |
...
| 4.22-4.30f | Custom SpiNNaker runtime progression from roundtrip/load tests to four-core MCPL tasks, keyed memory, routing/composition, predictive binding, confidence-gated learning, host-scheduled replay/consolidation, lifecycle static-pool metadata, multi-core lifecycle source/runtime gates, five-profile lifecycle hardware smoke, and lifecycle sham-control hardware subset. | Native hardware mechanism evidence for the tested capsules only; 4.29f is an audit over hardware passes, 4.30c is local split-contract evidence, 4.30d is local C/source evidence, 4.30e is a hardware smoke only, and 4.30f is compact lifecycle sham-control hardware evidence only. None of these prove lifecycle task benefit, speedup, multi-chip scaling, v2.2 temporal migration, or a lifecycle baseline freeze. |
