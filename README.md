...
| Active next gate | Tier 7.7f repaired finite-stream long-run scoreboard: rerun the locked 7.7c long-run matrix using the preflighted `narma10_reduced_input_u02` repaired NARMA stream, with all models rerun on the same repaired stream and no silent mixing with prior `U(0,0.5)` NARMA scores. |
| Canonical registry | 128 evidence bundles, 0 missing expected artifacts, 0 failed criteria. |
