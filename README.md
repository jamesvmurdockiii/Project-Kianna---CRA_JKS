...
| Latest hardware package | Tier 4.32a-hw prepare passed from [`controlled_test_output/tier4_32a_hw_20260506_prepared`](controlled_test_output/tier4_32a_hw_20260506_prepared): upload folder [`ebrains_jobs/cra_432a_hw`](ebrains_jobs/cra_432a_hw) is ready for EBRAINS, scoped only to `point_04c_reference` and `point_05c_lifecycle`; no hardware claim exists until returned artifacts are ingested. |
...
| Active next gate | Tier 4.32a-hw single-shard EBRAINS scale stress: upload `ebrains_jobs/cra_432a_hw` and run `cra_432a_hw/experiments/tier4_32a_hw_single_shard_scale_stress.py --mode run-hardware --output-dir tier4_32a_hw_job_output`. Replicated 8/12/16-core stress remains blocked until this hardware stress passes and is ingested. |
