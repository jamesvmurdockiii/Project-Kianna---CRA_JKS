...
[![Evidence](https://img.shields.io/badge/canonical%20evidence-58%20bundles-blue.svg)](STUDY_EVIDENCE_INDEX.md)
...
| Latest ingested hardware pass | Tier 4.30e multi-core lifecycle hardware smoke passed on board `10.11.226.145`: raw remote status `pass`, ingest `pass`, `75/75` hardware criteria, `5/5` ingest criteria, five profile builds/loads/readbacks, canonical 32-event and boundary 64-event lifecycle parity, and duplicate/stale lifecycle event rejection. |
...
| Latest prepared EBRAINS package | Tier 4.30f lifecycle sham-control hardware subset is prepared at `ebrains_jobs/cra_430f`; command: `cra_430f/experiments/tier4_30f_lifecycle_sham_hardware_subset.py --mode run-hardware --output-dir tier4_30f_hw_job_output`. |
| Active next gate | Run Tier 4.30f on EBRAINS/SpiNNaker, download returned artifacts, then ingest with the tier-specific ingest mode before making any lifecycle sham-control hardware claim. |
| Canonical registry | 58 evidence bundles, 0 missing expected artifacts, 0 failed criteria. |
...
| 4.22-4.30e | Custom SpiNNaker runtime progression from roundtrip/load tests to four-core MCPL tasks, keyed memory, routing/composition, predictive binding, confidence-gated learning, host-scheduled replay/consolidation, lifecycle static-pool metadata, multi-core lifecycle source/runtime gates, and the five-profile lifecycle hardware smoke. | Native hardware mechanism evidence for the tested capsules only; 4.29f is an audit over hardware passes, 4.30c is local split-contract evidence, 4.30d is local C/source evidence, and 4.30e is a hardware smoke only, not lifecycle task-benefit, sham-control, speedup, multi-chip, v2.2 temporal migration, or baseline-freeze evidence. Tier 4.30f is prepared but not hardware evidence until returned artifacts are ingested and pass. |
