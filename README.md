...
[![Evidence](https://img.shields.io/badge/canonical%20evidence-86%20bundles-blue.svg)](STUDY_EVIDENCE_INDEX.md)
...
| Latest public benchmark mechanism gate | Tier 7.0i passed from [`controlled_test_output/tier7_0i_20260508_recurrence_topology_specificity_gate`](controlled_test_output/tier7_0i_20260508_recurrence_topology_specificity_gate): structured bounded recurrence improved valid 8000-step aggregate geomean MSE versus v2.2 (`0.09964414908204765` vs `0.19348969000027122`) and destructive controls separated, but topology controls did not separate. Generic 7.0h bounded recurrent state remained slightly better (`0.09530752189727928`), and topology shams/no-recurrence controls matched or beat the structured candidate, so no topology-specific recurrence claim, hardware transfer, or baseline freeze is authorized. |
| Active next gate | Tier 7.0j generic bounded recurrent-state promotion/regression gate: decide whether the narrower generic recurrent-state interface earns a v2.3 software baseline through compact regression, while explicitly excluding topology-specific recurrence and ESN-superiority claims. |
| Canonical registry | 86 evidence bundles, 0 missing expected artifacts, 0 failed criteria. |
