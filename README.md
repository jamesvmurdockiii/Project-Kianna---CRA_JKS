...
[![Evidence](https://img.shields.io/badge/canonical%20evidence-154%20bundles-blue.svg)](STUDY_EVIDENCE_INDEX.md)
...
| Area | Current state |
| --- | --- |
| Software baseline | **`v2.6`** — frozen by Tier 7.7z-r0 (11/11). Edge-of-chaos recurrent dynamics: decay=0, sr=1.0, antisym=0.3 + ridge readout. PR 2→7 (3.5x), sham Δ=4.5, MSE 56% lower. Beats ESN 2.8x; lag-ridge still wins on NARMA10. v2.5 planning mechanism carried forward. |
...
| Active next gate | Tier 7.7t low-rank state repair campaign contract: see [`docs/TIER_7_7_LOW_RANK_REPAIR_PLAN.md`](docs/TIER_7_7_LOW_RANK_REPAIR_PLAN.md). The current blocker is dynamic but low-effective-dimensionality state collapse, with participation ratio near `~2`; Tier 7.7 remains active until this is repaired, bounded, or formally routed to morphology. Tier 7.8 morphology and Tier 7.9 lifecycle/evolution are queued behind that decision. |
| Canonical registry | 142 evidence bundles, 0 missing expected artifacts, 0 failed criteria. |
