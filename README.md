...
[![Evidence](https://img.shields.io/badge/canonical%20evidence-85%20bundles-blue.svg)](STUDY_EVIDENCE_INDEX.md)
...
| Latest public benchmark mechanism gate | Tier 7.0h passed as an executed software gate from [`controlled_test_output/tier7_0h_20260508_bounded_recurrent_interface_gate`](controlled_test_output/tier7_0h_20260508_bounded_recurrent_interface_gate): the bounded recurrent candidate improved valid 8000-step aggregate geomean MSE from v2.2 `0.19348969000027122` to `0.09530752189727928`, beat lag-only and random-reservoir online controls, and narrowed the ESN gap, but the permuted-recurrence sham stayed close (`1.036590722013174` margin), so no baseline freeze, hardware transfer, or recurrence-topology claim is authorized. |
| Active next gate | Tier 7.0i topology-specificity repair/gate: separate the useful recurrent-state gain from recurrence-permutation shams before promoting bounded nonlinear recurrence or moving it native/on-chip. |
| Canonical registry | 85 evidence bundles, 0 missing expected artifacts, 0 failed criteria. |
