...
| Active next gate | Run full Tier 5.45a healthy-NEST rebaseline scoring with `make tier5-45a` or equivalent sharded condition runs merged with `--merge-input-dirs`: compare `v2.6`, conservative organism defaults, each of the 15 opt-in organism mechanisms, the full stack, and external baselines under zero-fallback NEST. `make tier5-45a-smoke` is runner validation only and is not canonical evidence. |
