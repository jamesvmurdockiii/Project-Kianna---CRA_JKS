...
| Active next gate | Tier 7.4g held-out policy/action confirmation + reference separation: confirm the C-MAPSS action-cost signal, test reference separation against v2.2, and analyze the NAB non-confirmation without tuning on held-out streams. |
| Canonical registry | 112 evidence bundles, 0 missing expected artifacts, 0 failed criteria. |
...
- Evidence registry generation: 112 canonical bundles, 0 failed criteria.
...
@software{cra_2026,
  ...
  note         = {112 canonical evidence bundles; bounded SpiNNaker hardware validation}
}
```
