...
| Active next gate | Tier 4.32a-hw-replicated: prepare the 8/12/16-core replicated-shard stress with the same repaired MCPL value/meta reply path, shard-aware keys, compact readback, and stale/duplicate/timeout counters. Static reef partitioning, multi-chip work, and native-scale baseline freeze remain blocked until replicated stress passes and is ingested. |
