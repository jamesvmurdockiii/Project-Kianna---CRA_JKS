...
| Latest replicated-shard package | Tier 4.32a-hw-replicated prepare passed `14/14` from [`controlled_test_output/tier4_32a_hw_replicated_20260507_prepared`](controlled_test_output/tier4_32a_hw_replicated_20260507_prepared). Upload folder: [`ebrains_jobs/cra_432a_rep`](ebrains_jobs/cra_432a_rep). JobManager command: `cra_432a_rep/experiments/tier4_32a_hw_replicated_shard_stress.py --mode run-hardware --output-dir tier4_32a_replicated_job_output`. |
...
| Active next gate | Tier 4.32a-hw-replicated: run the prepared 8/12/16-core replicated-shard stress on EBRAINS, then ingest returned artifacts. Static reef partitioning, multi-chip work, and native-scale baseline freeze remain blocked until replicated stress passes and is ingested. |
