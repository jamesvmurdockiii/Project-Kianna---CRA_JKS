...
| Active next gate | Tier 4.32g-r0 multi-chip lifecycle route/source repair audit. Do not package 4.32g hardware until lifecycle event/trophic/mask-sync inter-chip route entries, counters, readback fields, and local tests are source-proven. Speedup, benchmark, lifecycle-scaling, true two-partition, multi-shard, and native-scale baseline-freeze claims remain blocked. |
| Canonical registry | 77 evidence bundles, 0 missing expected artifacts, 0 failed criteria. |
