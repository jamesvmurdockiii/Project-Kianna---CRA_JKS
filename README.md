...
| Latest CRA-native temporal-interface contract | Tier 7.7p passed from [`controlled_test_output/tier7_7p_20260509_cra_native_temporal_interface_internalization_contract`](controlled_test_output/tier7_7p_20260509_cra_native_temporal_interface_internalization_contract): `17/17` criteria passed. It locks `cra_native_sparse_temporal_expansion` as the next candidate: causal temporal expansion with delay/trace and nonlinear branch units must beat or cleanly separate from random-projection and nonlinear-lag controls before any promotion. Contract only: no implementation, model score, promotion, freeze, usefulness claim, external-baseline superiority, or hardware/native transfer. |
...
| Active next gate | Tier 7.7q CRA-native temporal-interface internalization scoring gate: implement and score the locked native temporal expansion candidate against v2.5, random-projection, nonlinear-lag, ablations, and shuffle controls. |
| Canonical registry | 139 evidence bundles, 0 missing expected artifacts, 0 failed criteria. |
