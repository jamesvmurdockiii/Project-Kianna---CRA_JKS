...
| Active next gate | Tier 4.32g two-chip lifecycle traffic/resource hardware smoke. Package and run only the predeclared lifecycle event/trophic/mask-sync traffic/resource counter smoke; speedup, benchmark, lifecycle-scaling, true two-partition, multi-shard, and native-scale baseline-freeze claims remain blocked. |
