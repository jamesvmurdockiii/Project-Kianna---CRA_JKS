...
| Active next gate | Tier 7.5b curriculum/environment generator implementation preflight: materialize deterministic train/validation/hidden-holdout stream manifests and dry-run baseline compatibility without scoring CRA or exposing held-out labels. |
| Canonical registry | 115 evidence bundles, 0 missing expected artifacts, 0 failed criteria. |
...
- Evidence registry generation: 115 canonical bundles, 0 failed criteria.
...
@software{cra_2026,
  ...
  note         = {115 canonical evidence bundles; bounded SpiNNaker hardware validation}
}
```
