...
| Active next gate | Tier 7.1d C-MAPSS failure analysis / adapter repair: decide whether the compact failure is caused by the scalar PCA1 adapter, readout policy, missing multichannel interface, or a real CRA limitation before adding mechanisms or moving hardware. |
| Canonical registry | 91 evidence bundles, 0 missing expected artifacts, 0 failed criteria. |
