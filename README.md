...
| Active next gate | Tier 7.0h bounded nonlinear recurrent continuous-state/interface candidate. Tier 7.0g selected this mechanism from the measured public benchmark gap: v2.2 ranks second at `8000` valid steps but remains about `9.6x` behind ESN, so the next test must target nonlinear recurrent state and readout/interface strength rather than sleep/replay, lifecycle, or hardware transfer. |
