...
from online rows, and documenting scoring-interface feasibility. Tier 7.1h then
passed compact NAB scoring: v2.3 ranked second, beat v2.2 and all three v2.3
shams, but did not beat the fixed random-reservoir online residual baseline and
...
repair candidate, but Tier 7.1l did not confirm it on held-out NAB streams:
v2.3 reduced false positives but did not beat rolling z-score or v2.2 and
...
freezing `CRA_EVIDENCE_BASELINE_v2.4`. Tier 7.4d then passed as the
held-out/public cost-aware policy/action usefulness contract. Tier 7.4e then
passed the source/split/cost/schema scoring preflight. The next step is Tier
7.4f: the locked held-out scoring gate. Stop tuning C-MAPSS/NAB adapter
policies for now.
```
...
Boundary: compact software scoring only; no full NAB benchmark claim, no public usefulness proof, no freeze, no hardware/native transfer.
...
smoke emission. Tier 7.1h produced a partial compact NAB signal: v2.3 beat v2.2
...
recall. Tier 7.1l locked that policy and tested held-out streams; the result
was not confirmed because v2.3 ranked fifth, did not beat rolling z-score or
v2.2, and separated only two of three shams. None of these tiers authorizes a
freeze or native transfer.
```
...
v2.3 policy expected utility mean: 18.046296296296294.
...
7.4b expected-utility advantage survives
...
```text
NAB held-out alarm/action costs
C-MAPSS maintenance/action costs
standardized dynamical action-cost diagnostics
diagnostic-only synthetic stressors
fixed cost model
train/calibration/test leakage rules
fair baselines and shams
bootstrap/effect-size/per-stream statistics
failure classes and nonclaims
```
...
```text
NAB and C-MAPSS source/preflight artifacts
disjoint held-out splits
fixed cost model
online/offline label separation
baseline and sham inventories
scoring schemas and statistical support rows
```
...
```text
C-MAPSS maintenance action utility: v2.4 ranked first and beat the strongest
external baseline.
NAB held-out alarm action utility: not confirmed; EWMA remained stronger.
Reference separation: C-MAPSS did not separate from v2.2 with a positive paired
CI, so this is not an incremental v2.4-specific advantage claim.
...
action-cost signal is preserved; broad public usefulness, incremental
v2.4-over-v2.2, freeze, and hardware/native transfer are blocked.
```
...
```text
Generated task families, difficulty schedule, hidden holdout splits, baselines,
leakage guards, metrics, pass/fail gates, and future artifacts are locked before
implementation/scoring. No scoring or usefulness claim is authorized.
```
...
```text
Deterministic generated streams, split manifests, hidden-label hashes, schema
contracts, baseline compatibility rows, and leakage checks are materialized
without scoring CRA or exposing hidden holdout labels.
```
...
Boundary: generated synthetic diagnostic only; no public usefulness claim, no
...
generated families, but near-oracle generator-feature alignment risk is high on
6/6 families.
Boundary: attribution/decision evidence only; no public usefulness claim, no
freeze, and no hardware/native transfer.
```
...
Result: 7.6b local scaffold signal preserved as diagnostic evidence, but
promotion/freeze/hardware transfer are blocked by high feature-alignment risk,
...
projection?
Boundary: contract only; no score, promotion, freeze, public usefulness claim,
or hardware/native transfer.
```
...
30. Tier 4.32d-r1 inter-chip MCPL route repair/local QA. COMPLETE: local pass
    ...
    package.
31. Tier 4.32d two-chip split-role MCPL smoke. COMPLETE: prepared pass 15/15
    at `controlled_test_output/tier4_32d_20260507_prepared/`, then returned
    EBRAINS hardware pass ingested at
    `controlled_test_output/tier4_32d_20260507_hardware_pass_ingested/`.
    Board `10.11.215.169`; source/learning chip `(0,0)`; remote state chip
    `(1,0)`; 32 events; 96/96 lookup replies; zero stale replies, duplicates,
    timeouts, or synthetic fallback. Boundary: communication/readback hardware
...
35. Tier 4.32g two-chip lifecycle traffic/resource hardware smoke. COMPLETE:
    hardware pass ingested at
    ...
    proving the cache-proof r2 package ran. Board target `10.11.205.177`,
    source learning core `(0,0,p7)`, remote lifecycle core `(1,0,p4)`, raw
    status `pass`, ingest status `pass`, source event/trophic requests `1/1`,
    ...
    zero stale/duplicate/missing-ack counters, reset/pause controls passed,
    payload lengths `>=149`, `30` returned artifacts preserved, zero synthetic
    fallback, and stale-package detection false. Earlier failed/stale returns
    remain preserved at
...
ZIP SHA256: 74bef434a34db25c7bf72e668ea4cd52afe5f2cf8e44367c55a82bfd91a5a34f
FD001 profile: 20631 train rows, 13096 test rows, 100 train units, 100 test units,
               100 RUL labels, 26 columns
Boundary: source/data preflight only, not C-MAPSS scoring, not public usefulness
