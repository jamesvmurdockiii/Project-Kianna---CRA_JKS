...
policy/action selection as the next general mechanism gate. The next step is
Tier 7.4a; stop tuning C-MAPSS for now.
```
...
Tier 7.4a must predeclare and then document:

```text
state -> action -> delayed consequence task family
asymmetric action costs: false positives, missed detections, delayed rewards
uncertainty-gated action selection and no-action options
external baselines and trivial policy controls
shams/ablations for confidence, memory, recurrent state, and policy learning
compact regression requirements before any mechanism promotion
boundary: general policy/action mechanism, not a NAB-specific threshold repair
```
