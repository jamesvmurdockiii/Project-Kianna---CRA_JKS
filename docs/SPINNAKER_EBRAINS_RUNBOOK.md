...
```text
Tier 4.28e - Native Failure Envelope / Operating Boundary
  Point A: 38/38 PASS (highest-pressure passing config, board 10.11.193.65)
  Point B: BOUNDARY CONFIRMED (64 events, 64 succeeded/14 rejected)
  Point C: 38/38 PASS (high pending-pressure config, board 10.11.194.1)
```
...
Tier 4.29b - Native Routing/Composition Gate
  Status: HARDWARE PASS, INGESTED (cra_429d)
  Seeds: 42/43/44 on boards 10.11.194.81 / 10.11.195.1 / 10.11.195.129
  Criteria: 52/52 per seed (156/156 total)
  Weight=32781, bias=3, pending=32/32, lookups=96/96, stale=0, timeouts=0
  ...
  Previous: cra_429c FAILED on EBRAINS (48/52 per seed).
    Root cause: C runtime readback emitted context-slot counters for all
    profiles; route_core updated route-slot counters but host read zeros.
    Fixed in host_interface.c, rebuilt, bumped to cra_429d per Rule 10.
```
...
Tier 4.29b - Native Routing/Composition Gate
  ...
  Criteria: 52/52 per seed (156/156 total)
  Weight=32781, bias=3, pending=32/32, lookups=96/96, stale=0, timeouts=0
  Previous: cra_429c FAILED on EBRAINS (48/52 per seed). Fixed readback bug.
```

Latest passed EBRAINS upload package:

```text
Tier 4.28e Point A - Native Failure Envelope Point A
upload = ebrains_jobs/cra_428n
status = returned hardware pass ingested
runner = experiments/tier4_28d_hard_noisy_switching_four_core_mcpl.py
```
