...
| 10 | Long-horizon planning public/usefulness transfer | Tier 7.6 froze bounded v2.5 software evidence; Tier 7.7a locked the public/standardized scoreboard contract; Tier 7.7b passed with standardized progress versus v2.3, driven by Mackey-Glass, while Lorenz/NARMA10 and strong external baselines still block broad usefulness; Tier 7.7c locked the long-run/failure-localization matrix. | Next step is Tier 7.7d scoring of the locked length sweep. Do not add new mechanisms until the Mackey-Glass-only signal and external-baseline gap are decomposed. |
