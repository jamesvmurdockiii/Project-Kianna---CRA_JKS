#!/usr/bin/env python3
"""Tier 4.22v tiny native memory-route reentry/composition custom-runtime smoke gate.
...
Tier 4.22v keeps the Tier 4.22u command surface and makes the task pressure
harder: four keyed slots, longer interleaving, independent context/route/memory
updates, and reentry recalls must still produce
feature=context[key]*route[key]*memory[key]*cue on chip before delayed credit.
...
TIER = "Tier 4.22v - Tiny Native Memory-Route Reentry/Composition Custom-Runtime Smoke"
RUNNER_REVISION = "tier4_22v_native_memory_route_reentry_composition_smoke_20260501_0001"
TIER4_22U_LATEST = CONTROLLED / "tier4_22u_latest_manifest.json"
DEFAULT_OUTPUT = CONTROLLED / "tier4_22v_20260501_native_memory_route_reentry_composition_smoke_prepared"
UPLOAD_PACKAGE_NAME = "cra_422ae"
STABLE_EBRAINS_UPLOAD = ROOT / "ebrains_jobs" / UPLOAD_PACKAGE_NAME
DEPRECATED_EBRAINS_UPLOADS = [ROOT / "ebrains_jobs" / "cra_422ae_old"]
...
NATIVE_MEMORY_ROUTED_PLAN = [
    ("ctx_A", 1, 1, 1, 1, "ctx_A_seed_all_pos"),
    ("ctx_B", -1, 1, -1, 1, "ctx_B_seed_context_memory_neg"),
    ("ctx_C", 1, -1, -1, -1, "ctx_C_seed_route_memory_neg"),
    ("ctx_D", -1, -1, 1, -1, "ctx_D_seed_context_route_neg"),
    ("ctx_A", None, None, None, -1, "ctx_A_recall_after_three_writes"),
    ("ctx_C", None, None, None, 1, "ctx_C_recall_route_memory_composition"),
    ("ctx_B", None, None, 1, 1, "ctx_B_memory_flip_only"),
    ("ctx_D", None, 1, None, -1, "ctx_D_route_flip_only"),
    ("ctx_A", None, -1, None, 1, "ctx_A_route_flip_only"),
    ("ctx_B", None, None, None, -1, "ctx_B_reentry_after_memory_flip"),
    ("ctx_C", None, None, 1, 1, "ctx_C_memory_flip_only"),
    ("ctx_D", 1, None, None, 1, "ctx_D_context_flip_only"),
    ("ctx_A", None, None, None, -1, "ctx_A_reentry_after_route_flip"),
    ("ctx_B", None, -1, None, 1, "ctx_B_route_flip_only"),
    ("ctx_C", -1, None, None, -1, "ctx_C_context_flip_only"),
    ("ctx_D", None, None, -1, 1, "ctx_D_memory_flip_only"),
]


def build_native_memory_route_reentry_composition_sequence() -> list[dict[str, Any]]:
    slots: dict[str, int] = {}
...
TASK_SEQUENCE = build_native_memory_route_reentry_composition_sequence()
...
def generate_task_reference(
    ...
    return {
        "status": "pass",
        "task": "native_memory_route_reentry_composition_signed_micro_task",
        "fixed_point": "s16.15",
...
def write_reference_artifacts(output_dir: Path, reference: dict[str, Any]) -> dict[str, str]:
    reference_json = output_dir / "tier4_22v_task_reference.json"
    reference_csv = output_dir / "tier4_22v_task_reference_rows.csv"
...
def prepare_bundle(output_dir: Path) -> tuple[Path, str, dict[str, str]]:
    ...
    command = f"{UPLOAD_PACKAGE_NAME}/experiments/tier4_22v_native_memory_route_reentry_composition_smoke.py --mode run-hardware --output-dir tier4_22v_job_output"
    readme = bundle / "README_TIER4_22V_JOB.md"
    readme.write_text(
        "# Tier 4.22v EBRAINS Tiny Native Memory-Route Reentry/Composition Custom-Runtime Smoke Job\n\n"
...
def write_report(path: Path, result: dict[str, Any]) -> None:
    ...
    for key in [
        "tier4_22u_status", "mode", "reference_status", "reference_sequence_length", "reference_accuracy", "reference_tail_accuracy",
...
def write_latest(output_dir: Path, manifest: Path, report: Path, status: str, mode: str) -> None:
    ...
    payload = {
        ...
        "report": str(report),
        "canonical": False,
        "claim": "Latest Tier 4.22v tiny native memory-route reentry/composition custom-runtime micro-task; pass means a tiny native memory-route reentry/composition pending-queue on-chip loop matched local fixed-point reference and task metrics only.",
    }
    write_json(CONTROLLED / "tier4_22v_latest_manifest.json", payload)


def finalize(output_dir: Path, result: dict[str, Any]) -> int:
    manifest = output_dir / "tier4_22v_results.json"
    report = output_dir / "tier4_22v_report.md"
...
def _base_local_result(args: argparse.Namespace, output_dir: Path) -> tuple[str, str | None, dict[str, Any], dict[str, str], dict[str, Any], list[dict[str, Any]]]:
    tier4_22u_status, tier4_22u_manifest = latest_status(TIER4_22U_LATEST)
    ...
    return tier4_22u_status, tier4_22u_manifest, reference, reference_artifacts, main_syntax, source_checks


def local(args: argparse.Namespace, output_dir: Path) -> int:
    tier4_22u_status, tier4_22u_manifest, reference, reference_artifacts, main_syntax, source_checks = _base_local_result(args, output_dir)
    metrics = reference.get("metrics", {})
    criteria = [
        criterion("Tier 4.22u native memory-route smoke pass exists", tier4_22u_status, "== pass", tier4_22u_status == "pass"),
    ...
    result = {
        ...
        "summary": {
            "mode": "local",
            "tier4_22u_status": tier4_22u_status,
            "tier4_22u_manifest": tier4_22u_manifest,
            ...
            **bridge_summary(reference),
            "next_step_if_passed": f"Prepare {UPLOAD_PACKAGE_NAME} and run the Tier 4.22v tiny native memory-route reentry/composition micro-task on EBRAINS.",
        },
        "criteria": criteria,
        "reference": reference,
...
def run_hardware(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    env_path = output_dir / "tier4_22v_environment.json"
    target_path = output_dir / "tier4_22v_target_acquisition.json"
    load_path = output_dir / "tier4_22v_load_result.json"
    task_path = output_dir / "tier4_22v_task_micro_loop_result.json"
    task_csv = output_dir / "tier4_22v_task_micro_loop_rows.csv"
    ...
    criteria = [
        ...
        criterion("Tier 4.22u native memory-route smoke pass exists or fresh bundle", tier4_22u_status, "== pass OR missing in fresh EBRAINS bundle", tier4_22u_status in {"pass", "missing"}),
        criterion("local task fixed-point reference generated", reference.get("status"), "== pass", reference.get("status") == "pass"),
        *bridge_criteria(reference),
...
def _is_route_slot_writes_false_fail(raw_manifest: dict[str, Any]) -> tuple[bool, str, int]:
    """Detect the first 4.22v returned-run bookkeeping false fail.
...
def ingest(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    raw_manifest_path = source / "tier4_22v_results.json"
    if not raw_manifest_path.exists():
        raise SystemExit(f"missing tier4_22v_results.json in ingest dir: {source}")
    raw_manifest = read_json(raw_manifest_path)

    artifacts: dict[str, str] = {}
    artifacts["raw_remote_manifest_json"] = _copy_latest(source, output_dir, "tier4_22v_results.json", target_name="remote_tier4_22v_results_raw.json")
    artifacts["raw_remote_report_md"] = _copy_latest(source, output_dir, "tier4_22v_report.md", target_name="remote_tier4_22v_report_raw.md")
    exact_artifact_names = {
        "tier4_22v_environment.json": "environment_json",
        "tier4_22v_target_acquisition.json": "target_acquisition_json",
        "tier4_22v_load_result.json": "load_result_json",
        "tier4_22v_task_micro_loop_result.json": "task_micro_loop_result_json",
        "tier4_22v_task_micro_loop_rows.csv": "task_micro_loop_rows_csv",
        "tier4_22v_task_reference.json": "reference_json",
        "tier4_22v_task_reference_rows.csv": "reference_csv",
        "tier4_22v_latest_manifest.json": "remote_latest_manifest_json",
    }
    ...
    ingested["summary"].update(
        {
            ...
            "claim_boundary": "Ingested returned EBRAINS artifacts; a pass is a tiny signed native memory-route reentry/composition custom-runtime micro-task only, not full CRA task learning or speedup evidence.",
        }
    )
...
def main() -> int:
    ...
    default = DEFAULT_OUTPUT if args.mode == "prepare" else CONTROLLED / f"tier4_22v_{stamp}_{args.mode.replace('-', '_')}"
