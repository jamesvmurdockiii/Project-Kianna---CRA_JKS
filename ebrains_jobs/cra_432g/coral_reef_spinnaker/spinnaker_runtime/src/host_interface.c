...
static void _handle_lifecycle_event_request_emit(sdp_msg_t *msg) {
    uint32_t event_index = msg->arg1;
    uint8_t event_type = (uint8_t)(msg->arg2 & 0xFF);
    uint32_t target_slot = msg->arg3;
    cra_lifecycle_send_event_request_stub(event_index, event_type, target_slot);
    uint8_t reply[2] = {CMD_LIFECYCLE_EVENT, 0};
    sdp_send_reply(msg, reply, 2);
}

static void _handle_lifecycle_trophic_request_emit(sdp_msg_t *msg) {
    uint32_t target_slot = msg->arg1;
    int32_t trophic_delta_raw = (int32_t)msg->arg2;
    cra_lifecycle_send_trophic_update_stub(target_slot, trophic_delta_raw);
    uint8_t reply[2] = {CMD_LIFECYCLE_TROPHIC_UPDATE, 0};
    sdp_send_reply(msg, reply, 2);
}
#endif
...
static void _handle_lifecycle_event(sdp_msg_t *msg) {
    ...
    int rc = cra_lifecycle_handle_event_request(
        event_index,
        event_type,
        target_slot,
        parent_slot,
...
#ifdef CRA_RUNTIME_PROFILE_LEARNING_CORE
        case CMD_LIFECYCLE_EVENT:          _handle_lifecycle_event_request_emit(msg);          break;
        case CMD_LIFECYCLE_TROPHIC_UPDATE: _handle_lifecycle_trophic_request_emit(msg);        break;
#endif
...
uint8_t host_if_pack_state_summary(uint8_t *payload, uint8_t max_len) {
    cra_state_summary_t summary;
    const uint8_t required_len = 149;
    ...
    _write_u32(&payload[105], summary.lifecycle_event_requests_sent);
    _write_u32(&payload[109], summary.lifecycle_trophic_requests_sent);
    _write_u32(&payload[113], summary.lifecycle_event_acks_received);
    _write_u32(&payload[117], summary.lifecycle_mask_syncs_sent);
    _write_u32(&payload[121], summary.lifecycle_mask_syncs_received);
    _write_u32(&payload[125], summary.lifecycle_last_seen_event_count);
    _write_u32(&payload[129], summary.lifecycle_last_seen_active_mask_bits);
    _write_u32(&payload[133], summary.lifecycle_last_seen_lineage_checksum);
    _write_u32(&payload[137], summary.lifecycle_duplicate_events);
    _write_u32(&payload[141], summary.lifecycle_stale_events);
    _write_u32(&payload[145], summary.lifecycle_missing_acks);
