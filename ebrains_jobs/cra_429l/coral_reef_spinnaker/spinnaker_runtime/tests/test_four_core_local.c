...
static void inject_replies_conf(int32_t ctx_conf, int32_t route_conf, int32_t mem_conf) {
    uint32_t pending[3];
    ...
    for (uint32_t i = 0; i < n; i++) {
        ...
        switch (type) {
            case LOOKUP_TYPE_CONTEXT: value = s_ctx_val;  confidence = ctx_conf;  break;
            case LOOKUP_TYPE_ROUTE:   value = s_route_val; confidence = route_conf; break;
            case LOOKUP_TYPE_MEMORY:  value = s_mem_val;  confidence = mem_conf;  break;
            default: assert(0 && "unknown lookup type");
        }
        rc = cra_state_lookup_receive(pending[i], value, confidence, 1);
        assert(rc == 0);
    }
}

static void inject_replies(void) {
    inject_replies_conf(FP_ONE, FP_ONE, FP_ONE);
}
...
/* ------------------------------------------------------------------ */
/* Test 5: zero confidence blocks all learning                          */
/* ------------------------------------------------------------------ */
static void test_zero_confidence(void) {
    cra_state_init();
    cra_state_set_readout(0, 0);
    cra_state_set_learning_rate(FP_FROM_FLOAT(0.25f));
    ...
    assert(summary.pending_matured == 1);
    assert(summary.reward_events == 1);
    /* Zero confidence => no weight/bias change */
    assert(summary.readout_weight == 0);
    assert(summary.readout_bias   == 0);
    printf("  PASS: zero confidence blocks learning\n");
}

/* ------------------------------------------------------------------ */
/* Test 6: half confidence scales learning by 0.5                       */
/* ------------------------------------------------------------------ */
static void test_half_confidence(void) {
    cra_state_init();
    cra_state_set_readout(0, 0);
    cra_state_set_learning_rate(FP_FROM_FLOAT(0.25f));
    ...
    while (cra_state_continuous_mode() && timestep < 20) {
        cra_state_process_continuous_tick(timestep);
        inject_replies_conf(FP_FROM_FLOAT(0.5f), FP_ONE, FP_ONE);
        timestep++;
    }
    ...
    assert(summary.pending_matured == 1);
    assert(summary.reward_events == 1);
    /* effective_lr = 0.25 * 0.5 = 0.125; delta_w = delta_b = 0.125 */
    assert(summary.readout_weight == FP_FROM_FLOAT(0.125f));
    assert(summary.readout_bias   == FP_FROM_FLOAT(0.125f));
    printf("  PASS: half confidence scales learning to 0.125\n");
}
