#!/usr/bin/env python3
"""Tier 4.22u tiny native memory-route state custom-runtime smoke gate.

Tier 4.22t proved that the custom runtime can own keyed context plus keyed
route slots and compute feature=context[key]*route[key]*cue on real SpiNNaker.
Tier 4.22u takes the next deliberately small step: the host writes keyed
context, keyed route slots, and keyed memory/working-state slots, then asks the
custom runtime to retrieve all three by key and form
feature=context[key]*route[key]*memory[key]*cue on chip before scheduling
delayed credit.
...
- This is not full v2.1 on-chip memory/routing, not full CRA task learning,
  not speedup evidence, not multi-core scaling, and not final on-chip autonomy.
"""
...
TIER = "Tier 4.22u - Tiny Native Memory-Route State Custom-Runtime Smoke"
RUNNER_REVISION = "tier4_22u_native_memory_route_state_smoke_20260501_0001"
TIER4_22T_LATEST = CONTROLLED / "tier4_22t_latest_manifest.json"
DEFAULT_OUTPUT = CONTROLLED / "tier4_22u_20260501_native_memory_route_state_smoke_prepared"
UPLOAD_PACKAGE_NAME = "cra_422ad"
STABLE_EBRAINS_UPLOAD = ROOT / "ebrains_jobs" / UPLOAD_PACKAGE_NAME
DEPRECATED_EBRAINS_UPLOADS = [ROOT / "ebrains_jobs" / "cra_422ad_old"]
...
TASK_TAIL_WINDOW = 6
PENDING_GAP_DEPTH = 2
CONTEXT_KEY_IDS = {"ctx_A": 101, "ctx_B": 202, "ctx_C": 303}
NATIVE_MEMORY_ROUTED_PLAN = [
    ("ctx_A", 1, 1, 1, 1, "ctx_A_write_route_memory_pos"),
    ("ctx_A", None, None, None, -1, "ctx_A_recall_route_memory_pos"),
    ("ctx_B", -1, 1, -1, 1, "ctx_B_write_route_pos_memory_neg"),
    ("ctx_A", None, None, None, 1, "ctx_A_reentry_route_memory_pos"),
    ("ctx_B", None, -1, 1, -1, "ctx_B_recall_route_neg_memory_pos"),
    ("ctx_A", None, None, None, -1, "ctx_A_recall_route_memory_neg"),
    ("ctx_C", 1, -1, -1, 1, "ctx_C_write_route_neg_memory_neg"),
    ("ctx_B", None, None, None, 1, "ctx_B_reentry_route_neg_memory_pos"),
    ("ctx_C", None, 1, 1, -1, "ctx_C_recall_route_pos_memory_pos"),
    ("ctx_A", None, None, None, 1, "ctx_A_reentry_route_memory_pos_again"),
]


def build_native_memory_route_state_sequence() -> list[dict[str, Any]]:
    slots: dict[str, int] = {}
    route_slots: dict[str, int] = {}
    memory_slots: dict[str, int] = {}
    ...
    for cycle in range(3):
        for key, context_update, route_update, memory_update, cue, purpose in NATIVE_MEMORY_ROUTED_PLAN:
            if context_update is not None:
                slots[key] = int(context_update)
                context_writes += 1
            if route_update is not None:
                route_slots[key] = int(route_update)
                route_writes += 1
                max_route_slot_count = max(max_route_slot_count, len(route_slots))
            if memory_update is not None:
                memory_slots[key] = int(memory_update)
                memory_writes += 1
                max_memory_slot_count = max(max_memory_slot_count, len(memory_slots))
            cue_value = int(cue if cycle % 2 == 0 else -cue)
            context_value = int(slots.get(key, 1))
            context_reads += 1
            route_value = int(route_slots.get(key, 1))
            route_reads += 1
            memory_value = int(memory_slots.get(key, 1))
            memory_reads += 1
            ...
            sequence.append({
                ...
                "native_context_key": key,
                "native_context_key_id": CONTEXT_KEY_IDS[key],
                "native_context_update": context_update,
                "native_context_value": context_value,
                "native_context_confidence": 1.0,
                "native_route_update": route_update,
                "native_route_value": route_value,
                "native_route_confidence": 1.0,
                "native_memory_update": memory_update,
                "native_memory_value": memory_value,
                "native_memory_confidence": 1.0,
                "native_visible_cue": cue_value,
                "native_slot_count": len(slots),
                "native_context_writes": context_writes,
                "native_context_reads": context_reads,
                "native_route_writes": route_writes,
                "native_route_reads": route_reads,
                "native_route_slot_count": len(route_slots),
                "native_route_max_slot_count": max_route_slot_count,
                "native_memory_writes": memory_writes,
                "native_memory_reads": memory_reads,
                "native_memory_slot_count": len(memory_slots),
                "native_memory_max_slot_count": max_memory_slot_count,
                "native_feature_source": "chip_context_memory_route_lookup_feature_transform",
            })
    return sequence


TASK_SEQUENCE = build_native_memory_route_state_sequence()
...
def generate_task_reference(
    ...
    for event in sequence or TASK_SEQUENCE:
        ...
        row = {
            ...
            "native_context_key": event.get("native_context_key"),
            "native_context_key_id": event.get("native_context_key_id"),
            "native_context_update": event.get("native_context_update"),
            "native_context_value": event.get("native_context_value"),
            "native_context_value_raw": fp_from_float(float(event.get("native_context_value", 0.0))),
            "native_context_confidence": event.get("native_context_confidence"),
            "native_context_confidence_raw": fp_from_float(float(event.get("native_context_confidence", 1.0))),
            "native_route_update": event.get("native_route_update"),
            "native_route_value": event.get("native_route_value"),
            "native_route_value_raw": fp_from_float(float(event.get("native_route_value", 0.0))),
            "native_route_confidence": event.get("native_route_confidence"),
            "native_route_confidence_raw": fp_from_float(float(event.get("native_route_confidence", 1.0))),
            "native_memory_update": event.get("native_memory_update"),
            "native_memory_value": event.get("native_memory_value"),
            "native_memory_value_raw": fp_from_float(float(event.get("native_memory_value", 0.0))),
            "native_memory_confidence": event.get("native_memory_confidence"),
            "native_memory_confidence_raw": fp_from_float(float(event.get("native_memory_confidence", 1.0))),
            "native_visible_cue": event.get("native_visible_cue"),
            "native_visible_cue_raw": fp_from_float(float(event.get("native_visible_cue", 0.0))),
            "native_slot_count": event.get("native_slot_count"),
            "native_context_writes": event.get("native_context_writes"),
            "native_context_reads": event.get("native_context_reads"),
            "native_route_writes": event.get("native_route_writes"),
            "native_route_reads": event.get("native_route_reads"),
            "native_route_slot_count": event.get("native_route_slot_count"),
            "native_route_max_slot_count": event.get("native_route_max_slot_count"),
            "native_memory_writes": event.get("native_memory_writes"),
            "native_memory_reads": event.get("native_memory_reads"),
            "native_memory_slot_count": event.get("native_memory_slot_count"),
            "native_memory_max_slot_count": event.get("native_memory_max_slot_count"),
            "native_feature_source": event.get("native_feature_source"),
    ...
    return {
        "status": "pass",
        "task": "native_memory_route_state_signed_micro_task",
        "fixed_point": "s16.15",
        "equation": "chip feature=context_slot_lookup(key)*route_slot_lookup(key)*memory_slot_lookup(key)*visible_cue; schedule prediction=w*feature+bias; keep pending across gap; mature oldest with target; w+=lr*(target-prediction)*feature; b+=lr*(target-prediction)",
        "native_context": {
            "context_keys": sorted({str(row.get("native_context_key")) for row in rows if row.get("native_context_key") is not None}),
            "context_key_ids": sorted({int(row.get("native_context_key_id")) for row in rows if row.get("native_context_key_id") is not None}),
            "context_writes": int(max([int(row.get("native_context_writes") or 0) for row in rows], default=0)),
            "context_reads": int(max([int(row.get("native_context_reads") or 0) for row in rows], default=0)),
            "max_slot_count": int(max([int(row.get("native_slot_count") or 0) for row in rows], default=0)),
            "feature_source": "chip_context_memory_route_lookup_feature_transform",
        },
        "native_route": {
            "route_slot_writes": int(max([int(row.get("native_route_writes") or 0) for row in rows], default=0)),
            "route_slot_reads": int(max([int(row.get("native_route_reads") or 0) for row in rows], default=0)),
            "max_route_slot_count": int(max([int(row.get("native_route_slot_count") or 0) for row in rows], default=0)),
            "route_values": sorted({int(row.get("native_route_value")) for row in rows if row.get("native_route_value") is not None}),
            "feature_source": "chip_context_memory_route_lookup_feature_transform",
        },
        "native_memory": {
            "memory_slot_writes": int(max([int(row.get("native_memory_writes") or 0) for row in rows], default=0)),
            "memory_slot_reads": int(max([int(row.get("native_memory_reads") or 0) for row in rows], default=0)),
            "max_memory_slot_count": int(max([int(row.get("native_memory_slot_count") or 0) for row in rows], default=0)),
            "memory_values": sorted({int(row.get("native_memory_value")) for row in rows if row.get("native_memory_value") is not None}),
            "feature_source": "chip_context_memory_route_lookup_feature_transform",
        },
        "learning_rate": float(learning_rate),
        "learning_rate_raw": lr_raw,
...
def write_reference_artifacts(output_dir: Path, reference: dict[str, Any]) -> dict[str, str]:
    reference_json = output_dir / "tier4_22u_task_reference.json"
    reference_csv = output_dir / "tier4_22u_task_reference_rows.csv"
...
def bridge_summary(reference: dict[str, Any]) -> dict[str, Any]:
    context = reference.get("native_context", {}) if isinstance(reference, dict) else {}
    route = reference.get("native_route", {}) if isinstance(reference, dict) else {}
    memory = reference.get("native_memory", {}) if isinstance(reference, dict) else {}
    return {
        "native_context_keys": context.get("context_keys"),
        "native_context_key_ids": context.get("context_key_ids"),
        "native_context_writes": context.get("context_writes"),
        "native_context_reads": context.get("context_reads"),
        "native_context_max_slot_count": context.get("max_slot_count"),
        "native_route_slot_writes": route.get("route_slot_writes"),
        "native_route_slot_reads": route.get("route_slot_reads"),
        "native_route_max_slot_count": route.get("max_route_slot_count"),
        "native_route_values": route.get("route_values"),
        "native_memory_slot_writes": memory.get("memory_slot_writes"),
        "native_memory_slot_reads": memory.get("memory_slot_reads"),
        "native_memory_max_slot_count": memory.get("max_memory_slot_count"),
        "native_memory_values": memory.get("memory_values"),
        "native_feature_source": context.get("feature_source") or route.get("feature_source") or memory.get("feature_source"),
    }
...
def bridge_criteria(reference: dict[str, Any], *, label: str = "reference") -> list[dict[str, Any]]:
    ...
    return [
        ...
        criterion(f"{label} native context reads observed", context.get("context_reads"), "== 30", int(context.get("context_reads") or 0) == 30),
        criterion(f"{label} native context retained three keyed slots", context.get("max_slot_count"), ">= 3", int(context.get("max_slot_count") or 0) >= 3),
        ...
        criterion(f"{label} native keyed route reads observed", route.get("route_slot_reads"), "== 30", int(route.get("route_slot_reads") or 0) == 30),
        criterion(f"{label} native keyed route retained three slots", route.get("max_route_slot_count"), ">= 3", int(route.get("max_route_slot_count") or 0) >= 3),
        ...
        criterion(f"{label} native keyed memory reads observed", memory.get("memory_slot_reads"), "== 30", int(memory.get("memory_slot_reads") or 0) == 30),
        criterion(f"{label} native keyed memory retained three slots", memory.get("max_memory_slot_count"), ">= 3", int(memory.get("max_memory_slot_count") or 0) >= 3),
        ...
        criterion(
            ...
            "== chip_context_memory_route_lookup_feature_transform",
            (context.get("feature_source") or route.get("feature_source") or memory.get("feature_source")) == "chip_context_memory_route_lookup_feature_transform",
        ),
    ]
...
def task_source_checks(
    ...
    *,
    ...
    checks.extend(
        [
            criterion(
                f"{label} native context, route, and memory command constants exist",
                "CMD_WRITE_CONTEXT/CMD_WRITE_ROUTE_SLOT/CMD_WRITE_MEMORY_SLOT/CMD_SCHEDULE_MEMORY_ROUTE_CONTEXT_PENDING",
                "custom runtime must expose native context plus memory-route state primitives",
            ...
            criterion(
                ...
                "cra_state_write_context(key, value, confidence, g_timestep)" in host_source and "cra_state_read_context(key, &context_value, &context_confidence)" in host_source,
            ),
            criterion(
            ...
            criterion(
                ...
                "cra_state_write_memory_slot(key, value, confidence, g_timestep)" in host_source and "cra_state_read_memory_slot(key, &memory_value, &memory_confidence)" in host_source,
            ),
            criterion(
                f"{label} native keyed memory-route schedule forms feature on chip",
                "feature = FP_MUL(FP_MUL(FP_MUL(context_value, route_value), memory_value), cue)",
                "host must send key+cue, while the runtime retrieves context+route+memory slots by key and computes the scalar feature",
                "feature = FP_MUL(FP_MUL(FP_MUL(context_value, route_value), memory_value), cue)" in host_source and "CMD_SCHEDULE_MEMORY_ROUTE_CONTEXT_PENDING" in controller_source,
            ),
            criterion(
...
def prepare_bundle(output_dir: Path) -> tuple[Path, str, dict[str, str]]:
    ...
    command = f"{UPLOAD_PACKAGE_NAME}/experiments/tier4_22u_native_memory_route_state_smoke.py --mode run-hardware --output-dir tier4_22u_job_output"
    readme = bundle / "README_TIER4_22U_JOB.md"
    readme.write_text(
...
def task_micro_loop(hostname: str, args: argparse.Namespace, *, dest_cpu: int, reference: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        for expected in expected_rows:
            ...
            route_write = None
            memory_write = None
            if expected.get("native_context_update") is not None:
                context_write = ctrl.write_context(
                    key=int(expected["native_context_key_id"]),
                    value=float(expected["native_context_update"]),
                    confidence=float(expected.get("native_context_confidence", 1.0)),
                    dest_x=args.dest_x,
                    dest_y=args.dest_y,
                    dest_cpu=dest_cpu,
                )
                time.sleep(float(args.command_delay_seconds))
            if expected.get("native_route_update") is not None:
                route_write = ctrl.write_route_slot(
                    key=int(expected["native_context_key_id"]),
                    value=float(expected["native_route_update"]),
                    confidence=float(expected.get("native_route_confidence", 1.0)),
                    dest_x=args.dest_x,
                    dest_y=args.dest_y,
                    dest_cpu=dest_cpu,
                )
                time.sleep(float(args.command_delay_seconds))
            if expected.get("native_memory_update") is not None:
                memory_write = ctrl.write_memory_slot(
                    key=int(expected["native_context_key_id"]),
                    value=float(expected["native_memory_update"]),
                    confidence=float(expected.get("native_memory_confidence", 1.0)),
                    dest_x=args.dest_x,
                    dest_y=args.dest_y,
                    dest_cpu=dest_cpu,
                )
                time.sleep(float(args.command_delay_seconds))
            schedule = ctrl.schedule_memory_route_context_pending_decision(
                key=int(expected["native_context_key_id"]),
                cue=float(expected["native_visible_cue"]),
            ...
            row = {
                ...
                "native_context_key": expected.get("native_context_key"),
                "native_context_key_id": expected.get("native_context_key_id"),
                "native_context_update": expected.get("native_context_update"),
                "native_route_key_id": expected.get("native_context_key_id"),
                "native_route_update": expected.get("native_route_update"),
                "native_route_value": expected.get("native_route_value"),
                "native_memory_key_id": expected.get("native_context_key_id"),
                "native_memory_update": expected.get("native_memory_update"),
                "native_memory_value": expected.get("native_memory_value"),
                "native_visible_cue": expected.get("native_visible_cue"),
                ...
                "expected_context_value_raw": expected["native_context_value_raw"],
                "observed_context_value_raw": schedule.get("context_value_raw"),
                "context_value_raw_delta": int_or(schedule.get("context_value_raw"), 10**12) - int(expected["native_context_value_raw"]),
                "expected_context_confidence_raw": expected["native_context_confidence_raw"],
                "observed_context_confidence_raw": schedule.get("context_confidence_raw"),
                "context_confidence_raw_delta": int_or(schedule.get("context_confidence_raw"), 10**12) - int(expected["native_context_confidence_raw"]),
                "expected_route_value_raw": expected["native_route_value_raw"],
                "observed_route_value_raw": schedule.get("route_value_raw"),
                "route_value_raw_delta": int_or(schedule.get("route_value_raw"), 10**12) - int(expected["native_route_value_raw"]),
                "expected_route_confidence_raw": expected["native_route_confidence_raw"],
                "observed_route_confidence_raw": schedule.get("route_confidence_raw"),
                "route_confidence_raw_delta": int_or(schedule.get("route_confidence_raw"), 10**12) - int(expected["native_route_confidence_raw"]),
                "expected_memory_value_raw": expected["native_memory_value_raw"],
                "observed_memory_value_raw": schedule.get("memory_value_raw"),
                "memory_value_raw_delta": int_or(schedule.get("memory_value_raw"), 10**12) - int(expected["native_memory_value_raw"]),
                "expected_memory_confidence_raw": expected["native_memory_confidence_raw"],
                "observed_memory_confidence_raw": schedule.get("memory_confidence_raw"),
                "memory_confidence_raw_delta": int_or(schedule.get("memory_confidence_raw"), 10**12) - int(expected["native_memory_confidence_raw"]),
                "observed_keyed_memory_key": schedule.get("keyed_memory_key"),
                "keyed_memory_key_match": int_or(schedule.get("keyed_memory_key"), -1) == int(expected["native_context_key_id"]),
        ...
        for key_id in sorted({int(row["native_context_key_id"]) for row in expected_rows}):
        ...
        ok = (
            ...
            and int_or(final_state.get("slot_writes"), -1) == int(reference.get("native_context", {}).get("context_writes") or -2)
            and int_or(final_state.get("slot_hits"), -1) >= len(expected_rows)
            and int_or(final_state.get("slot_misses"), 0) == 0
            and int_or(final_state.get("active_slots"), -1) >= int(reference.get("native_context", {}).get("max_slot_count") or -2)
            and all(bool(state.get("success")) for state in final_route_slot_states)
            and int(observed_route_slot_writes) == int(reference.get("native_route", {}).get("route_slot_writes") or -2)
            and int(observed_active_route_slots) >= int(reference.get("native_route", {}).get("max_route_slot_count") or -2)
            ...
            and int(observed_memory_slot_writes) == int(reference.get("native_memory", {}).get("memory_slot_writes") or -2)
            and int(observed_active_memory_slots) >= int(reference.get("native_memory", {}).get("max_memory_slot_count") or -2)
...
def write_report(path: Path, result: dict[str, Any]) -> None:
    ...
    for key in [
        "tier4_22t_status", "mode", "reference_status", "reference_sequence_length", "reference_accuracy", "reference_tail_accuracy",
        ...
        "native_context_keys", "native_context_key_ids", "native_context_writes", "native_context_reads", "native_context_max_slot_count",
        "native_route_slot_writes", "native_route_slot_reads", "native_route_max_slot_count", "native_route_values", "native_feature_source",
        "native_memory_slot_writes", "native_memory_slot_reads", "native_memory_max_slot_count", "native_memory_values",
...
def write_latest(output_dir: Path, manifest: Path, report: Path, status: str, mode: str) -> None:
    ...
    payload = {
        ...
        "report": str(report),
        "canonical": False,
        "claim": "Latest Tier 4.22u tiny native memory-route state custom-runtime micro-task; pass means a tiny native memory-route state pending-queue on-chip loop matched local fixed-point reference and task metrics only.",
    }
    write_json(CONTROLLED / "tier4_22u_latest_manifest.json", payload)


def finalize(output_dir: Path, result: dict[str, Any]) -> int:
    manifest = output_dir / "tier4_22u_results.json"
    report = output_dir / "tier4_22u_report.md"
...
def _base_local_result(args: argparse.Namespace, output_dir: Path) -> tuple[str, str | None, dict[str, Any], dict[str, str], dict[str, Any], list[dict[str, Any]]]:
    tier4_22t_status, tier4_22t_manifest = latest_status(TIER4_22T_LATEST)
    ...
    return tier4_22t_status, tier4_22t_manifest, reference, reference_artifacts, main_syntax, source_checks


def local(args: argparse.Namespace, output_dir: Path) -> int:
    tier4_22t_status, tier4_22t_manifest, reference, reference_artifacts, main_syntax, source_checks = _base_local_result(args, output_dir)
    metrics = reference.get("metrics", {})
    criteria = [
        criterion("Tier 4.22t native keyed route-state smoke pass exists", tier4_22t_status, "== pass", tier4_22t_status == "pass"),
    ...
    result = {
        ...
        "summary": {
            "mode": "local",
            "tier4_22t_status": tier4_22t_status,
            "tier4_22t_manifest": tier4_22t_manifest,
            ...
            **bridge_summary(reference),
            "next_step_if_passed": f"Prepare {UPLOAD_PACKAGE_NAME} and run the Tier 4.22u tiny native memory-route state micro-task on EBRAINS.",
        },
        "criteria": criteria,
        "reference": reference,
...
def run_hardware(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    env_path = output_dir / "tier4_22u_environment.json"
    target_path = output_dir / "tier4_22u_target_acquisition.json"
    load_path = output_dir / "tier4_22u_load_result.json"
    task_path = output_dir / "tier4_22u_task_micro_loop_result.json"
    task_csv = output_dir / "tier4_22u_task_micro_loop_rows.csv"
    ...
    criteria = [
        ...
        criterion("Tier 4.22t native keyed route-state smoke pass exists or fresh bundle", tier4_22t_status, "== pass OR missing in fresh EBRAINS bundle", tier4_22t_status in {"pass", "missing"}),
        criterion("local task fixed-point reference generated", reference.get("status"), "== pass", reference.get("status") == "pass"),
        *bridge_criteria(reference),
        ...
        *_source_checks_for_root(ROOT, label="runtime"),
        ...
        criterion("all context writes succeeded", [row.get("context_write_success") for row in rows if row.get("native_context_update") is not None], "all True", bool(rows) and all(row.get("context_write_success") for row in rows)),
        criterion("all route-slot writes succeeded", [row.get("route_write_success") for row in rows if row.get("native_route_update") is not None], "all True", bool(rows) and all(row.get("route_write_success") for row in rows)),
        criterion("all memory-slot writes succeeded", [row.get("memory_write_success") for row in rows if row.get("native_memory_update") is not None], "all True", bool(rows) and all(row.get("memory_write_success") for row in rows)),
        ...
        criterion("context slot writes final", final_state.get("slot_writes"), f"== {reference.get('native_context', {}).get('context_writes')}", int_or(final_state.get("slot_writes"), -1) == int(reference.get("native_context", {}).get("context_writes") or -2)),
        ...
        criterion("active context slots final", final_state.get("active_slots"), f">= {reference.get('native_context', {}).get('max_slot_count')}", int_or(final_state.get("active_slots"), -1) >= int(reference.get("native_context", {}).get("max_slot_count") or 10**9)),
        ...
        criterion("observed route-slot writes final", observed_route_slot_writes, f"== {reference.get('native_route', {}).get('route_slot_writes')}", observed_route_slot_writes == int(reference.get("native_route", {}).get("route_slot_writes") or -2)),
        criterion("observed active route slots final", observed_active_route_slots, f">= {reference.get('native_route', {}).get('max_route_slot_count')}", observed_active_route_slots >= int(reference.get("native_route", {}).get("max_route_slot_count") or 10**9)),
        ...
        criterion("observed memory-slot writes final", observed_memory_slot_writes, f"== {reference.get('native_memory', {}).get('memory_slot_writes')}", observed_memory_slot_writes == int(reference.get("native_memory", {}).get("memory_slot_writes") or -2)),
        criterion("observed active memory slots final", observed_active_memory_slots, f">= {reference.get('native_memory', {}).get('max_memory_slot_count')}", observed_active_memory_slots >= int(reference.get("native_memory", {}).get("max_memory_slot_count") or 10**9)),
    ...
    result = {
        ...
        "summary": {
            ...
            **bridge_summary(reference),
            ...
            "next_step_if_passed": "Ingest returned Tier 4.22u files; if it passes, continue with the next native custom-runtime integration gate, likely bounded memory/routing composition.",
        },
        "criteria": criteria,
        "reference": reference,
...
def _is_route_slot_writes_false_fail(raw_manifest: dict[str, Any]) -> tuple[bool, str, int]:
    """Detect the first 4.22u returned-run bookkeeping false fail.
    ...
    expected_route_slot_writes = int(reference.get("native_route", {}).get("route_slot_writes") or -1)
...
def ingest(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    raw_manifest_path = source / "tier4_22u_results.json"
    if not raw_manifest_path.exists():
        raise SystemExit(f"missing tier4_22u_results.json in ingest dir: {source}")
    raw_manifest = read_json(raw_manifest_path)

    artifacts: dict[str, str] = {}
    artifacts["raw_remote_manifest_json"] = _copy_latest(source, output_dir, "tier4_22u_results.json", target_name="remote_tier4_22u_results_raw.json")
    artifacts["raw_remote_report_md"] = _copy_latest(source, output_dir, "tier4_22u_report.md", target_name="remote_tier4_22u_report_raw.md")
    exact_artifact_names = {
        "tier4_22u_environment.json": "environment_json",
        "tier4_22u_target_acquisition.json": "target_acquisition_json",
        "tier4_22u_load_result.json": "load_result_json",
        "tier4_22u_task_micro_loop_result.json": "task_micro_loop_result_json",
        "tier4_22u_task_micro_loop_rows.csv": "task_micro_loop_rows_csv",
        "tier4_22u_task_reference.json": "reference_json",
        "tier4_22u_task_reference_rows.csv": "reference_csv",
        "tier4_22u_latest_manifest.json": "remote_latest_manifest_json",
    }
    ...
    ingested["summary"].update(
        {
            ...
            "observed_route_slot_writes": observed_route_slot_writes if false_fail_corrected else raw_manifest.get("summary", {}).get("observed_route_slot_writes"),
            ...
            "claim_boundary": "Ingested returned EBRAINS artifacts; a pass is a tiny signed native memory-route state custom-runtime micro-task only, not full CRA task learning or speedup evidence.",
        }
    )
...
def main() -> int:
    ...
    default = DEFAULT_OUTPUT if args.mode == "prepare" else CONTROLLED / f"tier4_22u_{stamp}_{args.mode.replace('-', '_')}"
