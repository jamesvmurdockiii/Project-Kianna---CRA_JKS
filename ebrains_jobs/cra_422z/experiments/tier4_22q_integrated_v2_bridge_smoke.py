#!/usr/bin/env python3
"""Tier 4.22q tiny integrated-v2 bridge custom-runtime smoke gate.

Tier 4.22p proved that a tiny A-B-A reentry pending queue could run on the
custom runtime on real SpiNNaker. Tier 4.22q takes the next deliberately small
step: a host-side v2-style keyed-context plus routing transform produces a
signed decision stream, while the custom runtime owns the pending horizon,
pre-update prediction, and delayed readout update.
...
- This is not native v2.1 on-chip memory/routing, not full CRA task learning,
...
TIER = "Tier 4.22q - Tiny Integrated V2 Bridge Custom-Runtime Smoke"
RUNNER_REVISION = "tier4_22q_integrated_v2_bridge_smoke_20260501_0001"
TIER4_22P_LATEST = CONTROLLED / "tier4_22p_latest_manifest.json"
DEFAULT_OUTPUT = CONTROLLED / "tier4_22q_20260501_integrated_v2_bridge_smoke_prepared"
UPLOAD_PACKAGE_NAME = "cra_422z"
STABLE_EBRAINS_UPLOAD = ROOT / "ebrains_jobs" / UPLOAD_PACKAGE_NAME
DEPRECATED_EBRAINS_UPLOADS = [ROOT / "ebrains_jobs" / "cra_422z_old"]
...
def build_integrated_v2_bridge_sequence() -> list[dict[str, Any]]:
    slots: dict[str, int] = {}
    route = 1
    ...
    for cycle in range(3):
        for key, context_update, route_update, cue, purpose in V2_BRIDGE_PLAN:
            if context_update is not None:
                slots[key] = int(context_update)
                bridge_updates += 1
            if route_update is not None:
                route = int(route_update)
                route_updates += 1
            cue_value = int(cue if cycle % 2 == 0 else -cue)
            context_value = int(slots.get(key, 1))
            feature = float(context_value * route * cue_value)
            sequence.append({
                "step": len(sequence) + 1,
                "feature": feature,
                "target": feature,
                "purpose": f"{purpose}_cycle{cycle + 1}",
                "bridge_context_key": key,
                "bridge_context_update": context_update,
                "bridge_context_value": context_value,
                "bridge_route_update": route_update,
                "bridge_route_value": route,
                "bridge_visible_cue": cue_value,
                "bridge_slot_count": len(slots),
                "bridge_context_updates": bridge_updates,
                "bridge_route_updates": route_updates,
                "bridge_feature_source": "host_keyed_context_route_transform",
            })
    return sequence


TASK_SEQUENCE = build_integrated_v2_bridge_sequence()
...
def generate_task_reference(
    ...
    for event in sequence or TASK_SEQUENCE:
        ...
        row = {
            ...
            "bridge_slot_count": event.get("bridge_slot_count"),
            "bridge_context_updates": event.get("bridge_context_updates"),
            "bridge_route_updates": event.get("bridge_route_updates"),
    ...
    return {
        "status": "pass",
        "task": "integrated_v2_bridge_signed_micro_task",
        "fixed_point": "s16.15",
        "equation": "host bridge feature=context_slot*route_state*visible_cue; schedule prediction=w*feature+bias; keep pending across gap; mature oldest with target; w+=lr*(target-prediction)*feature; b+=lr*(target-prediction)",
        "bridge": {
            "context_keys": sorted({str(row.get("bridge_context_key")) for row in rows if row.get("bridge_context_key") is not None}),
            "context_updates": int(max([int(row.get("bridge_context_updates") or 0) for row in rows], default=0)),
            "route_updates": int(max([int(row.get("bridge_route_updates") or 0) for row in rows], default=0)),
            "max_slot_count": int(max([int(row.get("bridge_slot_count") or 0) for row in rows], default=0)),
            "feature_source": "host_keyed_context_route_transform",
        },
        "learning_rate": float(learning_rate),
        "learning_rate_raw": lr_raw,
        "pending_gap_depth": int(pending_gap_depth),
...
def write_reference_artifacts(output_dir: Path, reference: dict[str, Any]) -> dict[str, str]:
    reference_json = output_dir / "tier4_22q_task_reference.json"
    reference_csv = output_dir / "tier4_22q_task_reference_rows.csv"
...
def bridge_summary(reference: dict[str, Any]) -> dict[str, Any]:
    bridge = reference.get("bridge", {}) if isinstance(reference, dict) else {}
    return {
        "bridge_context_keys": bridge.get("context_keys"),
        "bridge_context_updates": bridge.get("context_updates"),
        "bridge_route_updates": bridge.get("route_updates"),
        "bridge_max_slot_count": bridge.get("max_slot_count"),
        "bridge_feature_source": bridge.get("feature_source"),
    }
...
def bridge_criteria(reference: dict[str, Any], *, label: str = "reference") -> list[dict[str, Any]]:
    bridge = reference.get("bridge", {}) if isinstance(reference, dict) else {}
    return [
        criterion(f"{label} bridge context updates observed", bridge.get("context_updates"), "> 0", int(bridge.get("context_updates") or 0) > 0),
        criterion(f"{label} bridge route updates observed", bridge.get("route_updates"), "> 0", int(bridge.get("route_updates") or 0) > 0),
        criterion(f"{label} bridge retained three keyed slots", bridge.get("max_slot_count"), ">= 3", int(bridge.get("max_slot_count") or 0) >= 3),
        criterion(
            f"{label} bridge feature source declared",
            bridge.get("feature_source"),
            "== host_keyed_context_route_transform",
            bridge.get("feature_source") == "host_keyed_context_route_transform",
        ),
    ]
...
def prepare_bundle(output_dir: Path) -> tuple[Path, str, dict[str, str]]:
    ...
    command = f"{UPLOAD_PACKAGE_NAME}/experiments/tier4_22q_integrated_v2_bridge_smoke.py --mode run-hardware --output-dir tier4_22q_job_output"
    readme = bundle / "README_TIER4_22Q_JOB.md"
    readme.write_text(
...
def write_report(path: Path, result: dict[str, Any]) -> None:
    ...
    for key in [
        "tier4_22p_status", "mode", "reference_status", "reference_sequence_length", "reference_accuracy", "reference_tail_accuracy",
        ...
        "bridge_context_updates", "bridge_route_updates", "bridge_max_slot_count",
...
def write_latest(output_dir: Path, manifest: Path, report: Path, status: str, mode: str) -> None:
    ...
    payload = {
        ...
        "report": str(report),
        "canonical": False,
        "claim": "Latest Tier 4.22q tiny integrated-v2 bridge custom-runtime micro-task; pass means a tiny integrated v2 bridge pending-queue on-chip loop matched local fixed-point reference and task metrics only.",
    }
    write_json(CONTROLLED / "tier4_22q_latest_manifest.json", payload)


def finalize(output_dir: Path, result: dict[str, Any]) -> int:
    manifest = output_dir / "tier4_22q_results.json"
    report = output_dir / "tier4_22q_report.md"
...
def _base_local_result(args: argparse.Namespace, output_dir: Path) -> tuple[str, str | None, dict[str, Any], dict[str, str], dict[str, Any], list[dict[str, Any]]]:
    tier4_22p_status, tier4_22p_manifest = latest_status(TIER4_22P_LATEST)
    ...
    return tier4_22p_status, tier4_22p_manifest, reference, reference_artifacts, main_syntax, source_checks


def local(args: argparse.Namespace, output_dir: Path) -> int:
    tier4_22p_status, tier4_22p_manifest, reference, reference_artifacts, main_syntax, source_checks = _base_local_result(args, output_dir)
    metrics = reference.get("metrics", {})
    criteria = [
        criterion("Tier 4.22p A-B-A reentry micro-task pass exists", tier4_22p_status, "== pass", tier4_22p_status == "pass"),
        ...
        *bridge_criteria(reference),
        criterion("reference final weight positive after integrated bridge stream", reference.get("final_readout_weight_raw"), "> 0", int(reference.get("final_readout_weight_raw") or 0) > 0),
    ...
    result = {
        ...
        "summary": {
            "mode": "local",
            "tier4_22p_status": tier4_22p_status,
            "tier4_22p_manifest": tier4_22p_manifest,
            ...
            **bridge_summary(reference),
            "next_step_if_passed": f"Prepare {UPLOAD_PACKAGE_NAME} and run the Tier 4.22q tiny integrated-v2 bridge micro-task on EBRAINS.",
        },
        "criteria": criteria,
        "reference": reference,
...
def run_hardware(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    env_path = output_dir / "tier4_22q_environment.json"
    target_path = output_dir / "tier4_22q_target_acquisition.json"
    load_path = output_dir / "tier4_22q_load_result.json"
    task_path = output_dir / "tier4_22q_task_micro_loop_result.json"
    task_csv = output_dir / "tier4_22q_task_micro_loop_rows.csv"
    ...
    criteria = [
        ...
        criterion("Tier 4.22p A-B-A reentry micro-task pass exists or fresh bundle", tier4_22p_status, "== pass OR missing in fresh EBRAINS bundle", tier4_22p_status in {"pass", "missing"}),
        criterion("local task fixed-point reference generated", reference.get("status"), "== pass", reference.get("status") == "pass"),
        *bridge_criteria(reference),
    ...
    result = {
        ...
        "summary": {
            ...
            **bridge_summary(reference),
            ...
            "claim_boundary": "Tiny integrated host-v2/custom-runtime bridge smoke only; not native/on-chip v2 memory/routing, full CRA task learning, speedup evidence, scaling, or final autonomy.",
...
def ingest(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    raw_manifest_path = source / "tier4_22q_results.json"
    if not raw_manifest_path.exists():
        raise SystemExit(f"missing tier4_22q_results.json in ingest dir: {source}")
    raw_manifest = read_json(raw_manifest_path)

    artifacts: dict[str, str] = {}
    artifacts["raw_remote_manifest_json"] = _copy_latest(source, output_dir, "tier4_22q_results.json", target_name="remote_tier4_22q_results_raw.json")
    artifacts["raw_remote_report_md"] = _copy_latest(source, output_dir, "tier4_22q_report.md", target_name="remote_tier4_22q_report_raw.md")
    exact_artifact_names = {
        "tier4_22q_environment.json": "environment_json",
        "tier4_22q_target_acquisition.json": "target_acquisition_json",
        "tier4_22q_load_result.json": "load_result_json",
        "tier4_22q_task_micro_loop_result.json": "task_micro_loop_result_json",
        "tier4_22q_task_micro_loop_rows.csv": "task_micro_loop_rows_csv",
        "tier4_22q_task_reference.json": "reference_json",
        "tier4_22q_task_reference_rows.csv": "reference_csv",
        "tier4_22q_latest_manifest.json": "remote_latest_manifest_json",
    }
    ...
    ingested["summary"].update(
        {
            ...
            "claim_boundary": "Ingested returned EBRAINS artifacts; a pass is a tiny signed integrated v2 bridge custom-runtime micro-task only, not full CRA task learning or speedup evidence.",
        }
    )
...
def main() -> int:
    ...
    default = DEFAULT_OUTPUT if args.mode == "prepare" else CONTROLLED / f"tier4_22q_{stamp}_{args.mode.replace('-', '_')}"
