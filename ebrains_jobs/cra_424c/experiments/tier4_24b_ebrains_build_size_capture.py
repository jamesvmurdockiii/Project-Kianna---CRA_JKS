...
def prepare(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    command_build_only = f"{UPLOAD_PACKAGE_NAME}/experiments/tier4_24b_ebrains_build_size_capture.py --mode run-hardware --skip-load"
    command_with_load = f"{UPLOAD_PACKAGE_NAME}/experiments/tier4_24b_ebrains_build_size_capture.py --mode run-hardware"
...
def run_hardware(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    build_gnu_dir = RUNTIME / "build" / "gnu"
    elf_path = build_dir / "coral_reef.elf"
    elf_path_gnu = build_gnu_dir / "coral_reef.elf"
    aplx_path = build_dir / "coral_reef.aplx"
    
    # spinnaker_tools.mk may place .elf in build/gnu/ instead of build/
    if not elf_path.exists() and elf_path_gnu.exists():
        elf_path = elf_path_gnu

    # File sizes
    elf_size = elf_path.stat().st_size if elf_path.exists() else 0
    aplx_size = aplx_path.stat().st_size if aplx_path.exists() else 0
    elf_actual_path = str(elf_path) if elf_path.exists() else ""
    ...
    build_metrics = {
        ...
        "elf_path_actual": elf_actual_path,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--out-dir", default="", dest="output_dir", help="Output directory for results (default: tier4_24b_job_output/)")
...
def main() -> int:
    ...
    output_dir = Path(args.output_dir) if args.output_dir else Path("tier4_24b_job_output")
