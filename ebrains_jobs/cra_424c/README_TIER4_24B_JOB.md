...
Upload the `cra_424c` folder itself so the JobManager path starts with `cra_424c/`.
...
```text
cra_424c/experiments/tier4_24b_ebrains_build_size_capture.py --mode run-hardware --skip-load
```

**With hardware load (if board is available):**

```text
cra_424c/experiments/tier4_24b_ebrains_build_size_capture.py --mode run-hardware
```
