...
```bash
make RUNTIME_PROFILE=full
make RUNTIME_PROFILE=decoupled_memory_route
```

`full` is the default for legacy/custom-runtime smoke work and host tests.
`decoupled_memory_route` is the Tier 4.22w profile: it keeps reset/read-state,
delayed readout maturation, keyed context, keyed route-slot, keyed memory-slot,
and decoupled memory-route scheduling while excluding unused neuron/synapse/router
command handlers from that per-core image.
