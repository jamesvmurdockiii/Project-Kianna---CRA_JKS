...
Tests the native SpiNNaker keyed context + route + memory composition mechanism.
Uses non-neutral route values (+1.0 and -1.0) to create a routing-gated composition
test with explicit controls: wrong-context, wrong-route, both-wrong, overwrite,
and shuffle events. Uses a four-core MCPL distributed runner.
...
Claim boundary:
  Native keyed context * route * memory composition works for multi-slot tables.
...
# Neutral memory key (memory value = 1.0, no effect on multiplication)
NEUTRAL_MEMORY_KEY = 1

# Route slot keys and initial values
ROUTE_SLOTS = {
    101: 1.0,   # positive regime: feature sign unchanged
    102: -1.0,  # negative regime: feature sign flipped
}

# Context slot keys and initial values (same as 4.29a)
CONTEXT_SLOTS = {
    201: 1.0,
    202: 1.0,
    203: 1.0,
...
def _generate_task_sequence() -> list[dict[str, Any]]:
    """Generate the 32-event signed stream with routing, shuffle, wrong-key, and overwrite controls."""
    events: list[dict[str, Any]] = []
    ctx_slots: dict[int, float] = dict(CONTEXT_SLOTS)
    route_slots: dict[int, float] = dict(ROUTE_SLOTS)

    def add_event(step: int, key: int, route_key: int, cue: float, purpose: str) -> None:
        ctx_val = ctx_slots.get(key, 0.0)
        route_val = route_slots.get(route_key, 0.0)
        feature = ctx_val * route_val * cue
        target = feature  # target matches composed feature (0 for misses)
        events.append({
            "step": step,
            "context_key": key,
            "route_key": route_key,
    ...
    # Phase 1: route 101 and 102 interleaved with context keys 201-204
    add_event(1,  201, 101,  1.0, "phase1_r101_ctx201")
    add_event(2,  201, 102, -1.0, "phase1_r102_ctx201")
    add_event(3,  202, 101,  1.0, "phase1_r101_ctx202")
    add_event(4,  202, 102, -1.0, "phase1_r102_ctx202")
    add_event(5,  203, 101,  1.0, "phase1_r101_ctx203")
    add_event(6,  203, 102, -1.0, "phase1_r102_ctx203")
    add_event(7,  204, 101,  1.0, "phase1_r101_ctx204")
    add_event(8,  204, 102, -1.0, "phase1_r102_ctx204")

    # Phase 1: wrong-context events
    add_event(9,  999, 101,  1.0, "phase1_wrong_ctx_1")
    add_event(10, 998, 102, -1.0, "phase1_wrong_ctx_2")

    # Phase 1: wrong-route events
    add_event(11, 201, 999,  1.0, "phase1_wrong_route_1")
    add_event(12, 202, 998, -1.0, "phase1_wrong_route_2")

    # Phase 1: both-wrong events
    add_event(13, 997, 997,  1.0, "phase1_both_wrong_1")
    add_event(14, 996, 996, -1.0, "phase1_both_wrong_2")

    # Phase 1: negative-context reads
    add_event(15, 205, 101,  1.0, "phase1_r101_ctx205")
    add_event(16, 206, 102, -1.0, "phase1_r102_ctx206")

    # --- OVERWRITES between phases ---
    ctx_slots[OVERWRITE_KEY] = OVERWRITE_NEW
    route_slots[ROUTE_OVERWRITE_KEY] = ROUTE_OVERWRITE_NEW

    # Phase 2: overwrite verification (context 201 now -1.0, route 102 now +1.0)
    add_event(17, 201, 102,  1.0, "phase2_overwrite_verify")
    add_event(18, 201, 101, -1.0, "phase2_ctx_overwrite_verify")
    add_event(19, 202, 102,  1.0, "phase2_route_overwrite_verify")

    # Phase 2: scattered reads
    add_event(20, 203, 101, -1.0, "phase2_r101_ctx203")
    add_event(21, 204, 102,  1.0, "phase2_r102_ctx204")
    add_event(22, 205, 101, -1.0, "phase2_r101_ctx205")
    add_event(23, 206, 102,  1.0, "phase2_r102_ctx206")
    add_event(24, 207, 101, -1.0, "phase2_r101_ctx207")
    add_event(25, 208, 102,  1.0, "phase2_r102_ctx208")

    # Phase 2: wrong-context events
    add_event(26, 995, 101, -1.0, "phase2_wrong_ctx_1")
    add_event(27, 994, 102,  1.0, "phase2_wrong_ctx_2")

    # Phase 2: wrong-route events
    add_event(28, 207, 999, -1.0, "phase2_wrong_route_1")
    add_event(29, 208, 998,  1.0, "phase2_wrong_route_2")

    # Phase 2: both-wrong events
    add_event(30, 995, 997, -1.0, "phase2_both_wrong_1")
    add_event(31, 994, 996,  1.0, "phase2_both_wrong_2")

    # Phase 2: final re-read
    add_event(32, 202, 101, -1.0, "phase2_r101_ctx202")
...
def _compute_reference() -> dict[str, Any]:
    ...
    # Count wrong-context events
    wrong_ctx_events = [e for e in TASK_SEQUENCE if e["context_key"] in WRONG_KEYS]
    wrong_ctx_count = len(wrong_ctx_events)

    # Count wrong-route events
    wrong_route_events = [e for e in TASK_SEQUENCE if e["route_key"] not in ROUTE_SLOTS]
    wrong_route_count = len(wrong_route_events)

    # Count context overwrite events (events using overwritten key after step 16)
    ctx_overwrite_events = [
        e for e in TASK_SEQUENCE
        if e["context_key"] == OVERWRITE_KEY and e["step"] > 16
    ]

    # Count route overwrite events (events using overwritten route after step 16)
    ...
    route_misses = len(TASK_SEQUENCE) - route_hits
    ...
    return {
        ...
        "route_slot_writes": len(ROUTE_SLOTS) + 1,  # 2 initial + 1 overwrite
