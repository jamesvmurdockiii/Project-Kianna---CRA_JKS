...
Extends 4.28c from delayed-cue to hard_noisy_switching: regime switches,
sensory noise, and noisy trials. Uses context slots as oracle regime signs
so the current single-weight runtime can handle the task without mechanism
changes. Tests whether the four-core MCPL scaffold survives a harder task
structure.
...
Claim boundary:
  ...
  Host pre-writes regime context; not autonomous regime detection.
  Not multi-chip scaling, not speedup evidence, not v2.1 mechanism transfer.
"""
...
def _compute_hard_noisy_switching_reference(
    learning_rate: float = 0.25, surprise_threshold: float = 2.0
) -> dict[str, Any]:
    """Simulate fixed-point hard_noisy_switching learning on the host with surprise gating."""
    weight = 0
    bias = 0
    ...
    for idx, event in enumerate(TASK_SEQUENCE):
        # Hardware effective feature = regime_sign * cue_sign (context value * cue)
        ...
        while pending_queue and pending_queue[0]["due_step"] <= idx + 1:
            ...
            # Surprise gating: skip update if |error| >= surprise_threshold
...
TIER = "Tier 4.28d — Hard Noisy Switching Four-Core MCPL Hardware Probe"
RUNNER_REVISION = "tier4_28d_hard_noisy_switching_20260503_0001"
UPLOAD_PACKAGE_NAME = "cra_428j"
DEPRECATED_EBRAINS_UPLOADS = [ROOT / "ebrains_jobs" / "cra_428f"]
STABLE_EBRAINS_UPLOAD = ROOT / "ebrains_jobs" / UPLOAD_PACKAGE_NAME
DEPRECATED_EBRAINS_UPLOADS = [ROOT / "ebrains_jobs" / "cra_428i", ROOT / "ebrains_jobs" / "cra_428f_old"]
...
def generate_hard_noisy_switching_sequence(
    *,
    ...
    for start in range(0, steps - max_delay, hard_period):
        ...
        # Context key = 1000 + event_index, value = regime sign
...
TASK_SEQUENCE = generate_hard_noisy_switching_sequence(steps=128, seed=42)
...
def build_aplx_for_profile(profile: str, output_dir: Path) -> dict[str, Any]:
    ...
    (output_dir / f"tier4_28d_build_{profile}_stdout.txt").write_text(build["stdout"])
    (output_dir / f"tier4_28d_build_{profile}_stderr.txt").write_text(build["stderr"])
...
def mode_local(args: argparse.Namespace) -> dict[str, Any]:
    ...
    default_dir = f"tier4_28d_seed{args.seed}_job_output"
    ...
    # Host reference: simulate fixed-point hard_noisy_switching learning
    print("\n[6/5] Running host reference for hard_noisy_switching...")
    ref = _compute_hard_noisy_switching_reference()
    ...
    criteria = [
        ...
        {"name": "host reference weight positive", "value": ref["weight_raw"], "rule": "> 0", "passed": ref["weight_raw"] > 0},
        {"name": "host reference tail accuracy reasonable", "value": ref["tail_accuracy"], "rule": "> 0.5", "passed": ref["tail_accuracy"] > 0.5},
    ]
    ...
    output_path = output_dir / "tier4_28d_local_results.json"
...
def mode_prepare(args: argparse.Namespace) -> dict[str, Any]:
    ...
    # 4. Write README
    print("\n[4/4] Writing README...")
    command_single = f"{UPLOAD_PACKAGE_NAME}/experiments/tier4_28d_hard_noisy_switching_four_core_mcpl.py --mode run-hardware --seed 42"
    command_multi  = f"{UPLOAD_PACKAGE_NAME}/experiments/tier4_28d_hard_noisy_switching_four_core_mcpl.py --mode run-hardware --seeds 42,43,44"
...
"""
    ...
    readme = bundle / "README_TIER4_28D_JOB.md"
    ...
    output_path = output_dir / "tier4_28d_prepare_results.json"
    output_path.write_text(json.dumps(result, indent=2))
    print(f"\nArtifact: {output_path}")
    print(f"\n{'=' * 60}")
    print(f"OVERALL: {status.upper()}")
...
def four_core_hardware_loop(
    ...
    try:
        ...
        # For hard_noisy_switching, write per-event regime context slots
        ctx_writes = []
        for event in TASK_SEQUENCE:
        ...
        # Also write default route/memory slots
        route_writes = []
...
def mode_run_hardware(args: argparse.Namespace) -> dict[str, Any]:
    ...
    write_json(output_dir / "tier4_28d_environment.json", env_report)
    write_json(output_dir / "tier4_28d_target_acquisition.json", base.public_target_acquisition(target))
    ...
    try:
        if target.get("status") == "pass" and hostname:
            ...
            for name, load_info in loads.items():
                write_json(output_dir / f"tier4_28d_{name}_load.json", load_info)
        ...
        if (target.get("status") == "pass" and hostname
            ...
            write_json(output_dir / "tier4_28d_task.json", task_result)
    except Exception as exc:
        hardware_exception = {
    ...
    criteria = [
        ...
        criterion("learning_core weight positive and near reference", learning_final.get("readout_weight_raw"), f"within +/- {abs(ref_weight)//4} of {ref_weight}", abs(int(learning_final.get("readout_weight_raw") or 0) - ref_weight) <= abs(ref_weight) // 4),
    ...
    write_json(output_dir / "tier4_28d_hardware_results.json", result)
...
def mode_ingest(args: argparse.Namespace) -> dict[str, Any]:
    print("Tier 4.28d — Ingest hardware results")
    print("=" * 60)
    ...
    hw_results_path = hw_dir / "tier4_28d_hardware_results.json"
    ...
    output_path = output_dir / "tier4_28d_ingest_results.json"
    output_path.write_text(json.dumps(result, indent=2))
    ...
    (output_dir / "tier4_28d_ingest_report.md").write_text(report)

    print(f"OVERALL: {status.upper()}")
    print(f"Passed: {passed}/{total}")
    print(f"Ingest report: {output_dir / 'tier4_28d_ingest_report.md'}")
    return result
    ...
    try:
        if mode == "local":
            ...
            if args.seeds:
                ...
                combined_path = Path(f"tier4_28d_multi_seed_job_output") / "tier4_28d_combined_results.json"
        ...
        crash_path = Path(f"tier4_28d_seed{args.seed}_job_output") / "tier4_28d_crash_report.json"
