...
#endif
static void mcpl_lookup_callback(uint key, uint payload);
...
/*! MCPL packet RX — inter-core lookup request or reply.
    Registered unconditionally for all profiles (distributed cores use MCPL
    for lookup traffic even when they have no neuron spike traffic). */
static void mcpl_lookup_callback(uint key, uint payload) {
    cra_state_mcpl_lookup_receive(key, payload);
}
...
void c_main(void) {
    ...
    cra_state_mcpl_init(sark_core_id());
#endif
...
#endif
    spin1_callback_on(MCPL_PACKET_RECEIVED, mcpl_lookup_callback, 0);
