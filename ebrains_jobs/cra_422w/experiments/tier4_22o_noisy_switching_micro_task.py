#!/usr/bin/env python3
"""Tier 4.22o tiny noisy-switching custom-runtime micro-task gate.

Tier 4.22n proved that a tiny delayed pending queue could run on the custom
runtime on real SpiNNaker. Tier 4.22o takes the next deliberately small step: a
noisy switching stream where the target rule flips after an initial regime
...
TIER = "Tier 4.22o - Tiny Noisy-Switching Custom-Runtime Micro-Task"
RUNNER_REVISION = "tier4_22o_noisy_switching_micro_task_20260501_0001"
TIER4_22N_LATEST = CONTROLLED / "tier4_22n_latest_manifest.json"
DEFAULT_OUTPUT = CONTROLLED / "tier4_22o_20260501_noisy_switching_micro_task_prepared"
UPLOAD_PACKAGE_NAME = "cra_422w"
STABLE_EBRAINS_UPLOAD = ROOT / "ebrains_jobs" / UPLOAD_PACKAGE_NAME
DEPRECATED_EBRAINS_UPLOADS = [ROOT / "ebrains_jobs" / "cra_422w_old"]

FP_SHIFT = 15
FP_ONE = 1 << FP_SHIFT
TASK_LEARNING_RATE = 0.375
TASK_TAIL_WINDOW = 4
PENDING_GAP_DEPTH = 2
TASK_SEQUENCE = [
    {"step": 1, "feature": 1.0, "target": 1.0, "purpose": "regime_A_pre_switch"},
    {"step": 2, "feature": -1.0, "target": -1.0, "purpose": "regime_A_pre_switch"},
    {"step": 3, "feature": 1.0, "target": 1.0, "purpose": "regime_A_pre_switch"},
    {"step": 4, "feature": -1.0, "target": 1.0, "purpose": "regime_A_label_noise"},
    {"step": 5, "feature": 1.0, "target": 1.0, "purpose": "regime_A_pre_switch"},
    {"step": 6, "feature": -1.0, "target": -1.0, "purpose": "regime_A_pre_switch"},
    {"step": 7, "feature": 1.0, "target": -1.0, "purpose": "regime_B_after_switch"},
    {"step": 8, "feature": -1.0, "target": 1.0, "purpose": "regime_B_after_switch"},
    {"step": 9, "feature": 1.0, "target": 1.0, "purpose": "regime_B_label_noise"},
    {"step": 10, "feature": -1.0, "target": 1.0, "purpose": "regime_B_after_switch"},
    {"step": 11, "feature": 1.0, "target": -1.0, "purpose": "regime_B_after_switch"},
    {"step": 12, "feature": -1.0, "target": 1.0, "purpose": "regime_B_after_switch"},
    {"step": 13, "feature": 1.0, "target": -1.0, "purpose": "regime_B_after_switch"},
    {"step": 14, "feature": -1.0, "target": 1.0, "purpose": "regime_B_after_switch"},
]
...
def generate_task_reference(
    ...
    return {
        "status": "pass",
        "task": "noisy_switching_signed_micro_task",
        "fixed_point": "s16.15",
...
def write_reference_artifacts(output_dir: Path, reference: dict[str, Any]) -> dict[str, str]:
    reference_json = output_dir / "tier4_22o_task_reference.json"
    reference_csv = output_dir / "tier4_22o_task_reference_rows.csv"
...
def prepare_bundle(output_dir: Path) -> tuple[Path, str, dict[str, str]]:
    ...
    for script in [
        "tier4_22o_noisy_switching_micro_task.py",
    ...
    command = f"{UPLOAD_PACKAGE_NAME}/experiments/tier4_22o_noisy_switching_micro_task.py --mode run-hardware --output-dir tier4_22o_job_output"
    readme = bundle / "README_TIER4_22O_JOB.md"
    readme.write_text(
...
def write_report(path: Path, result: dict[str, Any]) -> None:
    ...
    for key in [
        "tier4_22n_status", "mode", "reference_status", "reference_sequence_length", "reference_accuracy", "reference_tail_accuracy",
...
def write_latest(output_dir: Path, manifest: Path, report: Path, status: str, mode: str) -> None:
    ...
    payload = {
        ...
        "report": str(report),
        "canonical": False,
        "claim": "Latest Tier 4.22o tiny noisy-switching custom-runtime micro-task; pass means a tiny delayed switching pending-queue on-chip loop matched local fixed-point reference and task metrics only.",
    }
    write_json(CONTROLLED / "tier4_22o_latest_manifest.json", payload)


def finalize(output_dir: Path, result: dict[str, Any]) -> int:
    manifest = output_dir / "tier4_22o_results.json"
    report = output_dir / "tier4_22o_report.md"
...
def _base_local_result(args: argparse.Namespace, output_dir: Path) -> tuple[str, str | None, dict[str, Any], dict[str, str], dict[str, Any], list[dict[str, Any]]]:
    tier4_22n_status, tier4_22n_manifest = latest_status(TIER4_22N_LATEST)
    ...
    return tier4_22n_status, tier4_22n_manifest, reference, reference_artifacts, main_syntax, source_checks


def local(args: argparse.Namespace, output_dir: Path) -> int:
    tier4_22n_status, tier4_22n_manifest, reference, reference_artifacts, main_syntax, source_checks = _base_local_result(args, output_dir)
    metrics = reference.get("metrics", {})
    criteria = [
        criterion("Tier 4.22n delayed-cue micro-task pass exists", tier4_22n_status, "== pass", tier4_22n_status == "pass"),
        ...
        criterion("reference sequence length", reference.get("sequence_length"), "== 14", reference.get("sequence_length") == 14),
        ...
        criterion("reference final weight negative after switch", reference.get("final_readout_weight_raw"), "< 0", int(reference.get("final_readout_weight_raw") or 0) < 0),
        criterion("reference final bias reflects noise", reference.get("final_readout_bias_raw"), "== -1536", int(reference.get("final_readout_bias_raw") if reference.get("final_readout_bias_raw") is not None else 0) == -1536),
    ] + source_checks
    ...
    result = {
        ...
        "summary": {
            "mode": "local",
            "tier4_22n_status": tier4_22n_status,
            "tier4_22n_manifest": tier4_22n_manifest,
            ...
            "next_step_if_passed": "Prepare cra_422w and run the Tier 4.22o tiny noisy-switching micro-task on EBRAINS.",
        },
        "criteria": criteria,
        "reference": reference,
...
def run_hardware(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    env_path = output_dir / "tier4_22o_environment.json"
    target_path = output_dir / "tier4_22o_target_acquisition.json"
    load_path = output_dir / "tier4_22o_load_result.json"
    task_path = output_dir / "tier4_22o_task_micro_loop_result.json"
    task_csv = output_dir / "tier4_22o_task_micro_loop_rows.csv"
    ...
    criteria = [
        ...
        criterion("Tier 4.22n delayed-cue micro-task pass exists or fresh bundle", tier4_22n_status, "== pass OR missing in fresh EBRAINS bundle", tier4_22n_status in {"pass", "missing"}),
        criterion("local task fixed-point reference generated", reference.get("status"), "== pass", reference.get("status") == "pass"),
    ...
    result = {
        ...
        "summary": {
            ...
            "claim_boundary": "Tiny on-chip noisy-switching micro-task only; not full CRA task learning or speedup evidence.",
...
def ingest(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    raw_manifest_path = source / "tier4_22o_results.json"
    if not raw_manifest_path.exists():
        raise SystemExit(f"missing tier4_22o_results.json in ingest dir: {source}")
    raw_manifest = read_json(raw_manifest_path)

    artifacts: dict[str, str] = {}
    artifacts["raw_remote_manifest_json"] = _copy_latest(source, output_dir, "tier4_22o_results.json", target_name="remote_tier4_22o_results_raw.json")
    artifacts["raw_remote_report_md"] = _copy_latest(source, output_dir, "tier4_22o_report.md", target_name="remote_tier4_22o_report_raw.md")
    exact_artifact_names = {
        "tier4_22o_environment.json": "environment_json",
        "tier4_22o_target_acquisition.json": "target_acquisition_json",
        "tier4_22o_load_result.json": "load_result_json",
        "tier4_22o_task_micro_loop_result.json": "task_micro_loop_result_json",
        "tier4_22o_task_micro_loop_rows.csv": "task_micro_loop_rows_csv",
        "tier4_22o_task_reference.json": "reference_json",
        "tier4_22o_task_reference_rows.csv": "reference_csv",
        "tier4_22o_latest_manifest.json": "remote_latest_manifest_json",
    }
    ...
    ingested["summary"].update(
        {
            ...
            "claim_boundary": "Ingested returned EBRAINS artifacts; a pass is a tiny signed noisy-switching custom-runtime micro-task only, not full CRA task learning or speedup evidence.",
        }
    )
...
def main() -> int:
    ...
    default = DEFAULT_OUTPUT if args.mode == "prepare" else CONTROLLED / f"tier4_22o_{stamp}_{args.mode.replace('-', '_')}"
