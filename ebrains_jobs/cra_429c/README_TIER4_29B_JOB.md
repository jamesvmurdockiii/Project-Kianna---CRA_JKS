...
Upload the `cra_429c` folder itself so the JobManager path starts
with `cra_429c/`.
...
```text
cra_429c/experiments/tier4_29b_native_routing_composition_gate.py --mode run-hardware --seeds 42,43,44
```

## Alternative: single-seed command

```text
cra_429c/experiments/tier4_29b_native_routing_composition_gate.py --mode run-hardware --seed 42
```
