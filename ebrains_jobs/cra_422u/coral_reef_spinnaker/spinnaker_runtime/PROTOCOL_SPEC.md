...
**Version:** 0.6  
**Date:** 2026-05-01  
**Status:** EXPERIMENTAL SIDECAR - Tier 4.22i board load/command round-trip passed; Tier 4.22j minimal closed-loop learning smoke passed after raw false-fail correction; Tier 4.22l tiny fixed-point learning parity passed on EBRAINS; Tier 4.22m minimal task micro-loop is prepared
...
Tier 4.22m is prepared as the next task-like gate using the same command
surface. It runs a 12-event signed fixed-pattern stream, scores the pre-update
readout prediction sign, matures one delayed-credit horizon per event, and
checks the final compact state against the local fixed-point reference. This is
minimal task micro-loop evidence only until returned EBRAINS artifacts pass.
