#!/usr/bin/env python3
"""Tier 4.22r tiny native context-state custom-runtime smoke gate.

Tier 4.22q proved that a tiny host-v2 bridge could feed signed features into
the custom runtime on real SpiNNaker. Tier 4.22r takes the next deliberately
small step: the host writes bounded keyed context slots, then asks the custom
runtime to retrieve the keyed context and form feature=context*cue on chip
before scheduling delayed credit.
...
TIER = "Tier 4.22r - Tiny Native Context-State Custom-Runtime Smoke"
RUNNER_REVISION = "tier4_22r_native_context_state_smoke_20260501_0001"
TIER4_22Q_LATEST = CONTROLLED / "tier4_22q_latest_manifest.json"
DEFAULT_OUTPUT = CONTROLLED / "tier4_22r_20260501_native_context_state_smoke_prepared"
UPLOAD_PACKAGE_NAME = "cra_422aa"
STABLE_EBRAINS_UPLOAD = ROOT / "ebrains_jobs" / UPLOAD_PACKAGE_NAME
DEPRECATED_EBRAINS_UPLOADS = [ROOT / "ebrains_jobs" / "cra_422aa_old"]
...
NATIVE_CONTEXT_PLAN = [
    ("ctx_A", 1, 1, "ctx_A_write_pos"),
    ("ctx_A", None, -1, "ctx_A_recall_neg"),
    ("ctx_B", -1, 1, "ctx_B_write_pos"),
    ("ctx_A", None, 1, "ctx_A_reentry_pos"),
    ("ctx_B", None, -1, "ctx_B_recall_neg"),
    ("ctx_A", None, -1, "ctx_A_recall_neg_again"),
    ("ctx_C", 1, 1, "ctx_C_write_pos"),
    ("ctx_B", None, 1, "ctx_B_reentry_pos"),
    ("ctx_C", None, -1, "ctx_C_recall_neg"),
    ("ctx_A", None, 1, "ctx_A_reentry_pos_again"),
]


def build_native_context_state_sequence() -> list[dict[str, Any]]:
    slots: dict[str, int] = {}
    sequence: list[dict[str, Any]] = []
    context_writes = 0
    context_reads = 0
    for cycle in range(3):
        for key, context_update, cue, purpose in NATIVE_CONTEXT_PLAN:
            ...
            context_reads += 1
            feature = float(context_value * cue_value)
            sequence.append({
                "step": len(sequence) + 1,
                "feature": feature,
                "target": feature,
...
TASK_SEQUENCE = build_native_context_state_sequence()
...
def generate_task_reference(
    ...
    return {
        "status": "pass",
        "task": "native_context_state_signed_micro_task",
        "fixed_point": "s16.15",
        "equation": "chip feature=context_slot_lookup(key)*visible_cue; schedule prediction=w*feature+bias; keep pending across gap; mature oldest with target; w+=lr*(target-prediction)*feature; b+=lr*(target-prediction)",
        "native_context": {
            ...
            "feature_source": "chip_context_lookup_feature_transform",
        },
        "learning_rate": float(learning_rate),
        "learning_rate_raw": lr_raw,
        "pending_gap_depth": int(pending_gap_depth),
...
def write_reference_artifacts(output_dir: Path, reference: dict[str, Any]) -> dict[str, str]:
    reference_json = output_dir / "tier4_22r_task_reference.json"
    reference_csv = output_dir / "tier4_22r_task_reference_rows.csv"
...
def bridge_summary(reference: dict[str, Any]) -> dict[str, Any]:
    bridge = reference.get("native_context", {}) if isinstance(reference, dict) else {}
    return {
        "native_context_keys": bridge.get("context_keys"),
        "native_context_key_ids": bridge.get("context_key_ids"),
        "native_context_writes": bridge.get("context_writes"),
        "native_context_reads": bridge.get("context_reads"),
        "native_context_max_slot_count": bridge.get("max_slot_count"),
        "native_feature_source": bridge.get("feature_source"),
    }
...
def bridge_criteria(reference: dict[str, Any], *, label: str = "reference") -> list[dict[str, Any]]:
    bridge = reference.get("native_context", {}) if isinstance(reference, dict) else {}
    return [
        criterion(f"{label} native context writes observed", bridge.get("context_writes"), "> 0", int(bridge.get("context_writes") or 0) > 0),
        criterion(f"{label} native context reads observed", bridge.get("context_reads"), "== 30", int(bridge.get("context_reads") or 0) == 30),
        criterion(f"{label} native context retained three keyed slots", bridge.get("max_slot_count"), ">= 3", int(bridge.get("max_slot_count") or 0) >= 3),
        criterion(
            f"{label} native feature source declared",
            bridge.get("feature_source"),
            "== chip_context_lookup_feature_transform",
            bridge.get("feature_source") == "chip_context_lookup_feature_transform",
        ),
    ]
...
def task_source_checks(
    ...
    *,
    ...
    checks.extend(
        [
            criterion(
                f"{label} native context command constants exist",
                "CMD_WRITE_CONTEXT/CMD_READ_CONTEXT/CMD_SCHEDULE_CONTEXT_PENDING",
                "custom runtime must expose the first native v2 state primitive",
                "CMD_WRITE_CONTEXT" in config_source and "CMD_READ_CONTEXT" in config_source and "CMD_SCHEDULE_CONTEXT_PENDING" in config_source,
            ),
            criterion(
                ...
                "cra_state_write_context(key, value, confidence, g_timestep)" in host_source and "cra_state_read_context(key, &value, &confidence)" in host_source,
            ),
            criterion(
                f"{label} native context schedule forms feature on chip",
                "feature = FP_MUL(context_value, cue)",
                "host must send key+cue, while the runtime retrieves context and computes the scalar feature",
                "feature = FP_MUL(context_value, cue)" in host_source and "CMD_SCHEDULE_CONTEXT_PENDING" in controller_source,
            ),
            criterion(
...
def prepare_bundle(output_dir: Path) -> tuple[Path, str, dict[str, str]]:
    ...
    command = f"{UPLOAD_PACKAGE_NAME}/experiments/tier4_22r_native_context_state_smoke.py --mode run-hardware --output-dir tier4_22r_job_output"
    readme = bundle / "README_TIER4_22R_JOB.md"
    readme.write_text(
    ...
    try:
        ...
        for expected in expected_rows:
            ...
            if expected.get("native_context_update") is not None:
                ...
                time.sleep(float(args.command_delay_seconds))
            schedule = ctrl.schedule_context_pending_decision(
                key=int(expected["native_context_key_id"]),
...
def write_report(path: Path, result: dict[str, Any]) -> None:
    ...
    for key in [
        "tier4_22q_status", "mode", "reference_status", "reference_sequence_length", "reference_accuracy", "reference_tail_accuracy",
        ...
        "native_context_keys", "native_context_key_ids", "native_context_writes", "native_context_reads", "native_context_max_slot_count", "native_feature_source",
...
def write_latest(output_dir: Path, manifest: Path, report: Path, status: str, mode: str) -> None:
    ...
    payload = {
        ...
        "report": str(report),
        "canonical": False,
        "claim": "Latest Tier 4.22r tiny native context-state custom-runtime micro-task; pass means a tiny native context-state pending-queue on-chip loop matched local fixed-point reference and task metrics only.",
    }
    write_json(CONTROLLED / "tier4_22r_latest_manifest.json", payload)


def finalize(output_dir: Path, result: dict[str, Any]) -> int:
    manifest = output_dir / "tier4_22r_results.json"
    report = output_dir / "tier4_22r_report.md"
...
def _base_local_result(args: argparse.Namespace, output_dir: Path) -> tuple[str, str | None, dict[str, Any], dict[str, str], dict[str, Any], list[dict[str, Any]]]:
    tier4_22q_status, tier4_22q_manifest = latest_status(TIER4_22Q_LATEST)
    ...
    return tier4_22q_status, tier4_22q_manifest, reference, reference_artifacts, main_syntax, source_checks


def local(args: argparse.Namespace, output_dir: Path) -> int:
    tier4_22q_status, tier4_22q_manifest, reference, reference_artifacts, main_syntax, source_checks = _base_local_result(args, output_dir)
    metrics = reference.get("metrics", {})
    criteria = [
        criterion("Tier 4.22q integrated bridge smoke pass exists", tier4_22q_status, "== pass", tier4_22q_status == "pass"),
        ...
        *bridge_criteria(reference),
        criterion("reference final weight positive after native context stream", reference.get("final_readout_weight_raw"), "> 0", int(reference.get("final_readout_weight_raw") or 0) > 0),
    ...
    result = {
        ...
        "summary": {
            "mode": "local",
            "tier4_22q_status": tier4_22q_status,
            "tier4_22q_manifest": tier4_22q_manifest,
            ...
            **bridge_summary(reference),
            "next_step_if_passed": f"Prepare {UPLOAD_PACKAGE_NAME} and run the Tier 4.22r tiny native context-state micro-task on EBRAINS.",
        },
        "criteria": criteria,
        "reference": reference,
...
def run_hardware(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    env_path = output_dir / "tier4_22r_environment.json"
    target_path = output_dir / "tier4_22r_target_acquisition.json"
    load_path = output_dir / "tier4_22r_load_result.json"
    task_path = output_dir / "tier4_22r_task_micro_loop_result.json"
    task_csv = output_dir / "tier4_22r_task_micro_loop_rows.csv"
    ...
    write_csv(task_csv, [{k: v for k, v in row.items() if k not in {"context_write", "schedule", "mature", "state_after_schedule", "state_after_mature"}} for row in task_result.get("rows", [])] if isinstance(task_result, dict) else [])
    ...
    criteria = [
        ...
        criterion("Tier 4.22q integrated bridge smoke pass exists or fresh bundle", tier4_22q_status, "== pass OR missing in fresh EBRAINS bundle", tier4_22q_status in {"pass", "missing"}),
        criterion("local task fixed-point reference generated", reference.get("status"), "== pass", reference.get("status") == "pass"),
        *bridge_criteria(reference),
    ...
    result = {
        ...
        "summary": {
            ...
            **bridge_summary(reference),
            ...
            "claim_boundary": "Tiny native keyed-context state primitive only; not full native/on-chip v2 memory/routing, full CRA task learning, speedup evidence, scaling, or final autonomy.",
...
def ingest(args: argparse.Namespace, output_dir: Path) -> int:
    ...
    raw_manifest_path = source / "tier4_22r_results.json"
    if not raw_manifest_path.exists():
        raise SystemExit(f"missing tier4_22r_results.json in ingest dir: {source}")
    raw_manifest = read_json(raw_manifest_path)

    artifacts: dict[str, str] = {}
    artifacts["raw_remote_manifest_json"] = _copy_latest(source, output_dir, "tier4_22r_results.json", target_name="remote_tier4_22r_results_raw.json")
    artifacts["raw_remote_report_md"] = _copy_latest(source, output_dir, "tier4_22r_report.md", target_name="remote_tier4_22r_report_raw.md")
    exact_artifact_names = {
        "tier4_22r_environment.json": "environment_json",
        "tier4_22r_target_acquisition.json": "target_acquisition_json",
        "tier4_22r_load_result.json": "load_result_json",
        "tier4_22r_task_micro_loop_result.json": "task_micro_loop_result_json",
        "tier4_22r_task_micro_loop_rows.csv": "task_micro_loop_rows_csv",
        "tier4_22r_task_reference.json": "reference_json",
        "tier4_22r_task_reference_rows.csv": "reference_csv",
        "tier4_22r_latest_manifest.json": "remote_latest_manifest_json",
    }
    ...
    ingested["summary"].update(
        {
            ...
            "claim_boundary": "Ingested returned EBRAINS artifacts; a pass is a tiny signed native context-state custom-runtime micro-task only, not full CRA task learning or speedup evidence.",
        }
    )
...
def main() -> int:
    ...
    default = DEFAULT_OUTPUT if args.mode == "prepare" else CONTROLLED / f"tier4_22r_{stamp}_{args.mode.replace('-', '_')}"
