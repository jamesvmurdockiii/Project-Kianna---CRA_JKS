#!/usr/bin/env python3
"""
Tier 4.28a — Four-Core MCPL Repeatability (seeds 42, 43, 44).

Extends 4.27a from SDP to MCPL for inter-core lookup, and from single-seed
to three-seed repeatability. The core loading sequence, schedule format, and
readback schema are identical to 4.27a; only the .aplx images change (built
with USE_MCPL_LOOKUP=1).

Modes:
  local        — build four MCPL profiles, verify ITCM, validate source wiring
...
TIER = "Tier 4.28a — Four-Core MCPL Repeatability"
RUNNER_REVISION = "tier4_28a_four_core_mcpl_repeatability_20260502_0003"
UPLOAD_PACKAGE_NAME = "cra_428d"
...
def build_aplx_for_profile(profile: str, output_dir: Path) -> dict[str, Any]:
    ...
    (output_dir / f"tier4_28a_build_{profile}_stdout.txt").write_text(build["stdout"])
    (output_dir / f"tier4_28a_build_{profile}_stderr.txt").write_text(build["stderr"])
...
def mode_local(args: argparse.Namespace) -> dict[str, Any]:
    ...
    default_dir = f"tier4_28a_seed{args.seed}_job_output"
    ...
    output_path = output_dir / "tier4_28a_local_results.json"
...
def mode_prepare(args: argparse.Namespace) -> dict[str, Any]:
    ...
    # 4. Write README
    print("\n[4/4] Writing README...")
    command = f"{UPLOAD_PACKAGE_NAME}/experiments/tier4_28a_four_core_mcpl_repeatability.py --mode run-hardware --seed {args.seed}"
...
Default output dir is `tier4_28a_seed{args.seed}_job_output`.
...
"""
    ...
    readme = bundle / "README_TIER4_28a_JOB.md"
    ...
    output_path = output_dir / "tier4_28a_prepare_results.json"
    output_path.write_text(json.dumps(result, indent=2))
    print(f"\nArtifact: {output_path}")
    print(f"\n{'=' * 60}")
    print(f"OVERALL: {status.upper()}")
...
def mode_run_hardware(args: argparse.Namespace) -> dict[str, Any]:
    ...
    write_json(output_dir / "tier4_28a_environment.json", env_report)
    write_json(output_dir / "tier4_28a_target_acquisition.json", base.public_target_acquisition(target))
    ...
    try:
        if target.get("status") == "pass" and hostname:
            ...
            for name, load_info in loads.items():
                write_json(output_dir / f"tier4_28a_{name}_load.json", load_info)
        ...
        if (target.get("status") == "pass" and hostname
            ...
            write_json(output_dir / "tier4_28a_task.json", task_result)
    except Exception as exc:
        hardware_exception = {
    ...
    write_json(output_dir / "tier4_28a_hardware_results.json", result)
...
def mode_ingest(args: argparse.Namespace) -> dict[str, Any]:
    print("Tier 4.28a — Ingest hardware results")
    print("=" * 60)
    ...
    hw_results_path = hw_dir / "tier4_28a_hardware_results.json"
    ...
    output_path = output_dir / "tier4_28a_ingest_results.json"
    output_path.write_text(json.dumps(result, indent=2))
    ...
    (output_dir / "tier4_28a_ingest_report.md").write_text(report)

    print(f"OVERALL: {status.upper()}")
    print(f"Passed: {passed}/{total}")
    print(f"Ingest report: {output_dir / 'tier4_28a_ingest_report.md'}")
    return result
...
def main() -> int:
    ...
    try:
        ...
        crash_path = Path(f"tier4_28a_seed{args.seed}_job_output") / "tier4_28a_crash_report.json"
