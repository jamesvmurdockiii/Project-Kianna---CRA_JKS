...
def four_core_hardware_loop(
    ...
    ok = ctx_ctrl.write_context(
        CONTEXT_KEY, fp_from_float(CONTEXT_VALUE), fp_from_float(1.0), dest_x, dest_y, CONTEXT_CORE_P
    )
    ctx_writes.append({"key": CONTEXT_KEY, "success": ok.get("success") is True})

    ok = route_ctrl.write_route_slot(
        ROUTE_KEY, fp_from_float(ROUTE_VALUE), fp_from_float(1.0), dest_x, dest_y, ROUTE_CORE_P
    )
    route_writes.append({"key": ROUTE_KEY, "success": ok.get("success") is True})

    ok = mem_ctrl.write_memory_slot(
        MEMORY_KEY, fp_from_float(MEMORY_VALUE), fp_from_float(1.0), dest_x, dest_y, MEMORY_CORE_P
    )
    ...
    for i, entry in enumerate(schedule):
        ok = learning_ctrl.write_schedule_entry(
            ...
            cue=fp_from_float(entry["cue"]),
            target=fp_from_float(entry["target"]),
            delay=entry["delay"],
            dest_x=dest_x,
            dest_y=dest_y,
    ...
    criteria = evaluate_control_criteria(control, ref, all_refs={"no_replay": no_replay_ref}, hw_state=learning_final)
...
def mode_run_hardware(args: argparse.Namespace) -> dict:
    ...
    # Compute no-replay reference for directional checks
    no_replay_events = BASE_EVENTS.copy()
    no_replay_ref = run_host_reference(int(args.seed), no_replay_events)
    ...
    try:
        ...
        if (target.get("status") == "pass" and hostname
            ...
            for control in CONTROLS:
                print(f"\n  -> Control: {control['name']}")
                result = four_core_hardware_loop(hostname, args, target, loads, control, no_replay_ref)
