...
`readout_bias_raw=-1536`. The first returned package, `cra_422w`, built,
loaded, scheduled, and matured all 14 events but failed at the signed regime
switch because `FP_MUL` used a 32-bit intermediate. The current package,
`cra_422x`, repairs fixed-point multiplication with an `int64_t` intermediate
and adds host regressions for the large signed product and pending switch
update. This remains prepared/local evidence only until returned EBRAINS
artifacts pass.
