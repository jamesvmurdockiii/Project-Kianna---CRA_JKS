 ...
 *
 * Uses the official SARK router API (rtr_alloc, rtr_mc_set, rtr_free)
 * to manage exact-match entries in the chip-level multicast router. Each
 * neuron gets one entry that matches its full 32-bit spike key and routes to
 * the local application core (determined at runtime via sark_core_id()).
 *
 * Because rtr_alloc(1) returns an index into the 1024-entry CAM, we keep a
 * small software index (also a linked list in SDRAM) to map neuron_id -> CAM
 * index so that death can free the correct entry.
 */
#include "router.h"
#include "config.h"
#include <sark.h>
...
int router_add_neuron(uint32_t neuron_id) {
    ...
    // Allocate one CAM entry via the official SARK router allocator.
    // SARK returns zero on failure; allocatable application entries start at 1.
    uint32_t cam_index = rtr_alloc(1);
    if (cam_index == 0) {
        return -1;  // CAM full
    }
    if (!rtr_mc_set(cam_index, key, mask, route)) {
        rtr_free(cam_index, 1);
        return -1;  // CAM full
    }
